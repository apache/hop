...
public class SharedObjectsTest {
  ...
  @Test
  public void writeToFileTest() throws HopException, IOException {
    doCallRealMethod().when( sharedObjectsMock ).writeToFile( any( FileObject.class ), anyString() );

    when( sharedObjectsMock.initOutputStreamUsingHopVFS( any( FileObject.class ) ) ).thenThrow(
        new RuntimeException() );

    try {
      sharedObjectsMock.writeToFile( any( FileObject.class ), anyString() );
    } catch ( HopException e ) {
      // NOP: catch block throws an HopException after calling sharedObjectsMock method
    }
  ...
  public void testCopyBackupVfs() throws Exception {
    ...
    FileObject baseDir = HopVFS.getFileObject( dirName );
    try {
      ...
      FileObject backup = HopVFS.getFileObject( backupFileName );
      Assert.assertFalse( backup.exists() );

      String contents = HopVFS.getTextFileContent( fileName, "utf8" );
      ...
      String contentsBackup = HopVFS.getTextFileContent( backupFileName, "utf8" );
