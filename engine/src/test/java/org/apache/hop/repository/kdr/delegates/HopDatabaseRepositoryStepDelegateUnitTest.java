...
/**
 */
public class HopDatabaseRepositoryStepDelegateUnitTest {
  ...
  public void getStepTypeIDs_WhenNeedToUseNotAllValues() throws Exception {
    ...
    HopDatabaseRepository rep = new HopDatabaseRepository();
    rep.connectionDelegate = mock( HopDatabaseRepositoryConnectionDelegate.class );
    when( rep.connectionDelegate.getDatabaseMeta() ).thenReturn( mock( DatabaseMeta.class ) );

    HopDatabaseRepositoryStepDelegate delegate = new HopDatabaseRepositoryStepDelegate( rep );
  ...
  @Test
  public void testGetStepTypeCodeToIdMap() throws HopException {
    HopDatabaseRepository repository = mock( HopDatabaseRepository.class );
    HopDatabaseRepositoryConnectionDelegate connectionDelegate =
      mock( HopDatabaseRepositoryConnectionDelegate.class );
    ...
    when( connectionDelegate.getDatabaseMeta() ).thenReturn( databaseMeta );
    HopDatabaseRepositoryStepDelegate kettleDatabaseRepositoryStepDelegate =
      new HopDatabaseRepositoryStepDelegate( repository );
    Map map = mock( Map.class );
    when( connectionDelegate
      .getValueToIdMap( kettleDatabaseRepositoryStepDelegate.quoteTable( HopDatabaseRepository.TABLE_R_STEP_TYPE ),
        kettleDatabaseRepositoryStepDelegate.quote( HopDatabaseRepository.FIELD_STEP_TYPE_ID_STEP_TYPE ),
          kettleDatabaseRepositoryStepDelegate.quote( HopDatabaseRepository.FIELD_STEP_TYPE_CODE ) ) ).thenReturn(
        map );
