...
public class HopDatabaseRepositoryStepDelegate extends HopDatabaseRepositoryBaseDelegate {
  ...
  public HopDatabaseRepositoryStepDelegate( HopDatabaseRepository repository ) {
    super( repository );
  }

  public synchronized ObjectId getStepTypeID( String code ) throws HopException {
    return repository.connectionDelegate.getIDWithValue(
      ...
      quote( HopDatabaseRepository.FIELD_STEP_TYPE_CODE ), code );
  }

  public ObjectId[] getStepTypeIDs( String[] codes, int amount ) throws HopException {
    ...
    return repository.connectionDelegate.getIDsWithValues(
      quoteTable( HopDatabaseRepository.TABLE_R_STEP_TYPE ),
      quote( HopDatabaseRepository.FIELD_STEP_TYPE_ID_STEP_TYPE ),
      quote( HopDatabaseRepository.FIELD_STEP_TYPE_CODE ), codes );
  }

  public Map<String, LongObjectId> getStepTypeCodeToIdMap() throws HopException {
    return repository.connectionDelegate.getValueToIdMap(
        quoteTable( HopDatabaseRepository.TABLE_R_STEP_TYPE ),
        quote( HopDatabaseRepository.FIELD_STEP_TYPE_ID_STEP_TYPE ),
        quote( HopDatabaseRepository.FIELD_STEP_TYPE_CODE ) );
  }

  public synchronized ObjectId getStepID( String name, ObjectId id_transformation ) throws HopException {
    return repository.connectionDelegate.getIDWithValue(
      quoteTable( HopDatabaseRepository.TABLE_R_STEP ), quote( HopDatabaseRepository.FIELD_STEP_ID_STEP ),
      quote( HopDatabaseRepository.FIELD_STEP_NAME ), name,
      quote( HopDatabaseRepository.FIELD_STEP_ID_TRANSFORMATION ), id_transformation );
  }

  public synchronized String getStepTypeCode( ObjectId id_database_type ) throws HopException {
    return repository.connectionDelegate.getStringWithID(
      quoteTable( HopDatabaseRepository.TABLE_R_STEP_TYPE ),
      quote( HopDatabaseRepository.FIELD_STEP_TYPE_ID_STEP_TYPE ), id_database_type,
      quote( HopDatabaseRepository.FIELD_STEP_TYPE_CODE ) );
  }

  public RowMetaAndData getStep( ObjectId id_step ) throws HopException {
    return repository.connectionDelegate.getOneRow(
      quoteTable( HopDatabaseRepository.TABLE_R_STEP ), quote( HopDatabaseRepository.FIELD_STEP_ID_STEP ),
      id_step );
  }

  public RowMetaAndData getStepType( ObjectId id_step_type ) throws HopException {
    return repository.connectionDelegate.getOneRow(
      quoteTable( HopDatabaseRepository.TABLE_R_STEP_TYPE ),
      quote( HopDatabaseRepository.FIELD_STEP_TYPE_ID_STEP_TYPE ), id_step_type );
  }

  public RowMetaAndData getStepAttribute( ObjectId id_step_attribute ) throws HopException {
    return repository.connectionDelegate.getOneRow(
      quoteTable( HopDatabaseRepository.TABLE_R_STEP_ATTRIBUTE ),
      quote( HopDatabaseRepository.FIELD_STEP_ATTRIBUTE_ID_STEP_ATTRIBUTE ), id_step_attribute );
  }
  ...
  public StepMeta loadStepMeta( ObjectId stepId, List<DatabaseMeta> databases,
    List<PartitionSchema> partitionSchemas ) throws HopException {
    ...
    try {
      ...
      if ( r != null ) {
        ...
        stepMeta.setName( r.getString( HopDatabaseRepository.FIELD_STEP_NAME, null ) );
        stepMeta.setDescription( r.getString( HopDatabaseRepository.FIELD_STEP_DESCRIPTION, null ) );

        long id_step_type = r.getInteger( HopDatabaseRepository.FIELD_STEP_ID_STEP_TYPE, -1L );
        RowMetaAndData steptyperow = getStepType( new LongObjectId( id_step_type ) );

        stepMeta.setStepID( steptyperow.getString( HopDatabaseRepository.FIELD_STEP_TYPE_CODE, null ) );
        stepMeta.setDistributes( r.getBoolean( HopDatabaseRepository.FIELD_STEP_DISTRIBUTE, true ) );
        int copies = (int) r.getInteger( HopDatabaseRepository.FIELD_STEP_COPIES, 1 );
        String copiesString = r.getString( HopDatabaseRepository.FIELD_STEP_COPIES_STRING, null );
        ...
        int x = (int) r.getInteger( HopDatabaseRepository.FIELD_STEP_GUI_LOCATION_X, 0 );
        int y = (int) r.getInteger( HopDatabaseRepository.FIELD_STEP_GUI_LOCATION_Y, 0 );
        stepMeta.setLocation( new Point( x, y ) );
        stepMeta.setDraw( r.getBoolean( HopDatabaseRepository.FIELD_STEP_GUI_DRAW, false ) );
      ...
      throw new HopException( BaseMessages.getString( PKG, "StepMeta.Exception.StepCouldNotBeLoaded", String
  ...
  private void readRepCompatibleStepMeta( StepMetaInterface stepMetaInterface,
    HopDatabaseRepository repository, ObjectId objectId, List<DatabaseMeta> databases ) throws HopException {
    stepMetaInterface.readRep( repository, objectId, databases, null );
  }

  public void saveStepMeta( StepMeta stepMeta, ObjectId transformationId ) throws HopException {
    try {
      ...
      throw new HopException( BaseMessages.getString( PKG, "StepMeta.Exception.UnableToSaveStepInfo", String
        .valueOf( transformationId ) ), e );
    }
  }

  @SuppressWarnings( "deprecation" )
  private void compatibleSaveRep( StepMetaInterface stepMetaInterface, HopDatabaseRepository repository,
    ObjectId id_transformation, ObjectId objectId ) throws HopException {
    stepMetaInterface.saveRep( repository, id_transformation, objectId );
  }

  public void saveStepErrorMeta( StepErrorMeta meta, ObjectId id_transformation, ObjectId id_step ) throws HopException {
  ...
  public StepErrorMeta loadStepErrorMeta( VariableSpace variables, StepMeta stepMeta, List<StepMeta> steps ) throws HopException {
  ...
  public StepPartitioningMeta loadStepPartitioningMeta( ObjectId id_step ) throws HopException {
   ...
   *
   */
  public void saveStepPartitioningMeta( StepPartitioningMeta meta, ObjectId id_transformation, ObjectId id_step ) throws HopException {
  ...
  public synchronized ObjectId insertStep( ObjectId id_transformation, String name, String description,
    ...
    boolean gui_draw, String copiesString ) throws HopException {
    ...
    table.addValue( new ValueMetaInteger( HopDatabaseRepository.FIELD_STEP_ID_STEP ), id );
    table.addValue( new ValueMetaInteger( HopDatabaseRepository.FIELD_STEP_ID_TRANSFORMATION ), id_transformation );
    table.addValue( new ValueMetaString( HopDatabaseRepository.FIELD_STEP_NAME ), name );
    table.addValue( new ValueMetaString( HopDatabaseRepository.FIELD_STEP_DESCRIPTION ), description );
    table.addValue( new ValueMetaInteger( HopDatabaseRepository.FIELD_STEP_ID_STEP_TYPE ), id_step_type );
    table.addValue( new ValueMetaBoolean( HopDatabaseRepository.FIELD_STEP_DISTRIBUTE ), Boolean.valueOf( distribute ) );
    table.addValue( new ValueMetaInteger( HopDatabaseRepository.FIELD_STEP_COPIES ), new Long( copies ) );
    table.addValue( new ValueMetaInteger( HopDatabaseRepository.FIELD_STEP_GUI_LOCATION_X ), new Long( gui_location_x ) );
    table.addValue( new ValueMetaInteger( HopDatabaseRepository.FIELD_STEP_GUI_LOCATION_Y ), new Long( gui_location_y ) );
    table.addValue( new ValueMetaBoolean( HopDatabaseRepository.FIELD_STEP_GUI_DRAW ), Boolean.valueOf( gui_draw ) );
    table.addValue( new ValueMetaString( HopDatabaseRepository.FIELD_STEP_COPIES_STRING ), copiesString );

    repository.connectionDelegate.getDatabase().prepareInsert( table.getRowMeta(), HopDatabaseRepository.TABLE_R_STEP );
  ...
  public synchronized int getNrSteps( ObjectId id_transformation ) throws HopException {
    int retval = 0;
    ...
    String sql =
      "SELECT COUNT(*) FROM "
        ...
        + quote( HopDatabaseRepository.FIELD_STEP_ID_TRANSFORMATION ) + " = ? ";
  ...
  public synchronized int getNrStepAttributes( ObjectId id_step ) throws HopException {
    int retval = 0;
  ...
  private void saveAttributesMap( ObjectId transformationId, ObjectId stepId,
    Map<String, Map<String, String>> attributesMap ) throws HopException {
  ...
  private Map<String, Map<String, String>> loadStepAttributesMap( ObjectId stepId ) throws HopException {
    ...
    for ( Object[] attributeRow : attributeRows ) {
      String code = rowMeta.getString( attributeRow, HopDatabaseRepository.FIELD_STEP_ATTRIBUTE_CODE, null );
      if ( code != null && code.startsWith( STEP_ATTRIBUTE_PREFIX ) ) {
        String value =
          rowMeta.getString( attributeRow, HopDatabaseRepository.FIELD_STEP_ATTRIBUTE_VALUE_STR, null );
        if ( value != null ) {
