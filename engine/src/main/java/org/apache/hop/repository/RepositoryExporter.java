...
public class RepositoryExporter implements IRepositoryExporterFeedback {
  ...
  private synchronized List<ExportFeedback> exportAllObjectsInternal( ProgressMonitorListener monitorOuter,
      String xmlFilename, RepositoryDirectoryInterface root, String exportType, boolean feedback ) throws HopException {
    ...
    FileObject output = HopVFS.getFileObject( xmlFilename );
    ...
    try {
      ...
      switch ( type ) {
        ...
        default: {
          ...
          throw new HopException( "Unsupported export type: " + type );
        }
      }
  ...
  private void exportJobs( ProgressMonitorDecorator monitor, RepositoryDirectoryInterface dirTree, ExportWriter writer,
      boolean feedback ) throws HopException {
    try {
      ...
      throw new HopException( "Error while exporting repository jobs", e );
    } finally {
      writer.closeJob();
    }
  }
  ...
  private List<ImportValidationFeedback> validateObject( Object subject, boolean boolFeedback ) throws HopException {
    if ( !hasRules ) {
  ...
  private void convertFromFileRepository( JobMeta jobMeta ) {
    ...
    if ( repository instanceof HopFileRepository ) {

      HopFileRepository fileRep = (HopFileRepository) repository;
        ...
        if ( entry instanceof JobEntryTrans ) {
          ...
          if ( trans.getSpecificationMethod() == ObjectLocationSpecificationMethod.FILENAME ) {
            try {
              ...
              FileObject fileObject = HopVFS.getFileObject( meta.getFilename() );
  ...
  private void convertFromFileRepository( TransMeta transMeta ) {
    ...
    if ( repository instanceof HopFileRepository ) {
      ...
      for ( StepMeta stepMeta : transMeta.getSteps() ) {
        ...
        try {
          ExtensionPointHandler.callExtensionPoint( log, HopExtensionPoint.RepositoryExporterPatchTransStep.id, metaInjectObjectArray );
  ...
  private String calcRepositoryDirectory( HopFileRepository fileRep, FileObject fileObject ) throws FileSystemException {
  ...
  private void exportTransformations( ProgressMonitorDecorator monitor, RepositoryDirectoryInterface dirTree,
      ExportWriter writer, boolean feedback ) throws HopException {
    try {
      writer.openTrans();
      ...
      throw new HopException( "Error while exporting repository transformations", e );
    } finally {
      writer.closeTrans();
    }
  }
  ...
  private class ExportWriter implements IExportWriter {
    ...
    ExportWriter( FileObject writeTo ) throws HopException {
    ...
    @Override
    public void writeJob( String str ) throws HopException {
      delegate.writeJob( str );
    }

    @Override
    public void writeTrans( String str ) throws HopException {
      delegate.writeTrans( str );
    }

    @Override
    public void closeJob() throws HopException {
      delegate.closeJob();
    }

    @Override
    public void closeTrans() throws HopException {
      delegate.closeTrans();
    }
    ...
    @Override
    public void openJob() throws HopException {
      delegate.openJob();
    }

    @Override
    public void openTrans() throws HopException {
      delegate.openTrans();
    }
  }

  interface IExportWriter {
    void writeJob( String str ) throws HopException;

    void writeTrans( String str ) throws HopException;

    void openJob() throws HopException;

    void closeJob() throws HopException;

    void openTrans() throws HopException;

    void closeTrans() throws HopException;
  ...
  private class ExportStFileWriter implements IExportWriter {
    ...
    ExportStFileWriter( FileObject writeTo ) throws HopException {
      this.writeTo = writeTo;
      // we will write even it is empty file
      write();
    }

    private void write() throws HopException {
      if ( !start ) {
        this.prepareToWrite();
        this.writeXmlRoot();
        start = true;
    ...
    private void prepareToWrite() throws HopException {
      try {
    ...
    private void writeXmlRoot() throws HopException {
      try {
