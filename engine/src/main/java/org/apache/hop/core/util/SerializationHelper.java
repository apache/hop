...
public class SerializationHelper {
  ...
  /**
   * This method will perform the work that used to be done by hand in each hop input meta for:
   * readData(Node transformNode). We handle all primitive types, complex user types, arrays, lists
   * and any number of nested object levels, via recursion of this method.
   *
      ...
      if (field.getType().isArray()) {
        try {
          ...
          for (int i = 0; i < childrenNodes.getLength(); i++) {
            ...
            // ignore TEXT_NODE, they'll cause us to have a larger count than reality, even if they
            // are empty
  ...
  /**
   * This method will perform the work that used to be done by hand in each hop input meta for:
   * getXml(). We handle all primitive types, complex user types, arrays, lists and any number of
   * nested object levels, via recursion of this method.
   *
   * @param object
   * @param buffer
   */
    ...
    for (Field field : fields) {
      ...
      try {
        ...
        if (field.getType().isPrimitive()
          ...
          buffer
              .append(
                  "<"
                      + field.getName()
                      + " class=\""
                      + fieldValue.getClass().getComponentType().getName()
                      + "\">")
              .append(Const.CR);
          ...
          indent(buffer, indentLevel);
          buffer
              .append("<" + field.getName() + " class=\"" + listClass.getName() + "\">")
              .append(Const.CR);
          ...
          // if we don't now what it is, let's treat it like a first class citizen and try to write
          // it out
          // open node (add class name attribute)
          indent(buffer, indentLevel);
          buffer
              .append("<" + field.getName() + " class=\"" + fieldValue.getClass().getName() + "\">")
              .append(Const.CR);
