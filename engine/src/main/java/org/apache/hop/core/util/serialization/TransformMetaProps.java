...
/**
 * A slim representation of ITransformMeta properties, used as a way to leverage alternative
 * serialization strategies (e.g. {@link MetaXmlSerializer}
 *
 * <p>Public methods allow conversion: {@link #from(ITransformMeta)} and {@link #to(ITransformMeta)}
 *
 * <p>
 *
 * <p>Internally, the TransformMetaProps holds a list of {@link PropGroup} elements, each
 * corresponding to a MetadataInjection group.
 *
 * <p>InjectionDeep not yet supported.
 */
...
public class TransformMetaProps<Meta extends ITransformMeta> {
  ...
  /**
   * Retuns an instance of this class with transformMeta properties mapped to a list of {@link
   * PropGroup}
   */
   ...
   *
   * <p>This method mutates the transformMeta, as opposed to returning a new instance, to match more
   * cleanly to Hop's {@link ITransformMeta#loadXml} design, which loads props into an instance.
   */
  ...
  /**
   * Allows specifying a variable variables to be used when applying property values to a
   * transformMeta.
   */
  ...
  private void populateGroups(Meta meta, BeanInjectionInfo<Meta> info, BeanInjector injector) {
    groups =
        info.getGroups().stream() // get metadata injection groups
            .flatMap(group -> group.getProperties().stream()) // expand to all properties
            .map(getProp(meta, injector)) // map to  property/value
            .collect(groupingBy(Prop::getGroup))
            .entrySet()
            .stream() // group by group name
            .map(
                entry ->
                    new PropGroup(
                        entry.getKey(), entry.getValue())) // map the set of properties to a group
            .collect(Collectors.toList());
  }

  /**
   * Collects the list of declared fields with the {@link Sensitive} annotation for the class.
   * Values for these fields will be encrypted.
   *
   * <p>Checks the top level fields of clazz, and recurses into {@link InjectionDeep} classes.
   */
  @VisibleForTesting
  ...
  private Function<BeanInjectionInfo.Property, Prop> getProp(
      ITransformMeta transformMeta, BeanInjector injector) {
    return prop ->
        new Prop(prop.getKey(), getPropVal(transformMeta, injector, prop), prop.getGroupKey());
  }

  @SuppressWarnings("unchecked")
  private List<Object> getPropVal(
      ITransformMeta transformMeta, BeanInjector injector, BeanInjectionInfo.Property prop) {
    try {
      List ret;

      Object o = injector.getPropVal(transformMeta, prop.getKey());
      ...
      return maybeEncrypt(prop.getKey(), ret);
    } catch (Exception e) {
  ...
  private void assignValueForProp(
      BeanInjectionInfo.Property beanInfoProp,
      ITransformMeta transformMetaInterface,
      BeanInjector injector) {
    List<Prop> props =
        groups.stream()
            .filter(group -> beanInfoProp.getGroupKey().equals(group.name))
            .flatMap(group -> group.props.stream())
            .filter(prop -> beanInfoProp.getKey().equals(prop.name))
            .collect(Collectors.toList());
  ...
  private static class PropGroup {
    ...
    @SuppressWarnings("unused")
    public PropGroup() {} // needed for deserialization
  ...
  /**
   * Represents a single property from a ITransformMeta impl. Values are captured as a List<Object>
   * to consistently handle both List properties and single items.
   */
  private static class Prop {
    ...
    @SuppressWarnings("unused")
    public Prop() {} // needed for deserialization
