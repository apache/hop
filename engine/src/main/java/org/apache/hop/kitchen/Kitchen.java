...
public class Kitchen {
  ...
  public static void main( String[] a ) throws Exception {
    ...
    final Future<Map.Entry<HopPluginException, Future<HopException>>> repositoryRegisterFuture =
      executor.submit( new Callable<Map.Entry<HopPluginException, Future<HopException>>>() {

        @Override
        public Map.Entry<HopPluginException, Future<HopException>> call() throws Exception {
          PluginRegistry.addPluginType( repositoryPluginType );
          try {
            HopClientEnvironment.getInstance().setClient( HopClientEnvironment.ClientType.KITCHEN );
            HopClientEnvironment.init();
          } catch ( HopPluginException e ) {
            return new AbstractMap.SimpleImmutableEntry<HopPluginException, Future<HopException>>( e, null );
          }

          Future<HopException> kettleEnvironmentInitFuture =
            executor.submit( new Callable<HopException>() {
          ...
          return new AbstractMap.SimpleImmutableEntry<HopPluginException, Future<HopException>>( null, kettleEnvironmentInitFuture );
        }
      } );
    ...
    Map.Entry<HopPluginException, Future<HopException>> repositoryRegisterResults =
      repositoryRegisterFuture.get();
    // It's a singleton map with one key-value pair (a Pair collection)
    HopPluginException repositoryRegisterException = repositoryRegisterResults.getKey();
    if ( repositoryRegisterException != null ) {
      throw repositoryRegisterException;
    }
    Future<HopException> kettleInitFuture = repositoryRegisterResults.getValue();

    if ( !Utils.isEmpty( optionLogfile ) ) {
      fileAppender = new FileLoggingEventListener( optionLogfile.toString(), true );
      HopLogStore.getAppender().addLoggingEventListener( fileAppender );
    } else {
      fileAppender = null;
    }
    ...
    try {
      ...
      if ( !Utils.isEmpty( optionVersion ) ) {
        ...
        if ( a.length == 1 ) {
          exitJVM( CommandExecutorCodes.Pan.HOP_VERSION_PRINT.getCode() );
        }
      }
      ...
      if ( fileAppender != null ) {
        fileAppender.close();
        HopLogStore.getAppender().removeLoggingEventListener( fileAppender );
      }
    }
   ...
   * @throws HopException Error parsing command line arguments
   */
  public static void configureLogging( final CommandLineOption maxLogLinesOption,
                                       final CommandLineOption maxLogTimeoutOption ) throws HopException {
    int maxLogLines = parseIntArgument( maxLogLinesOption, 0 );
    if ( Utils.isEmpty( maxLogLinesOption.getArgument() ) ) {
      maxLogLines = Const.toInt( EnvUtil.getSystemProperty( Const.HOP_MAX_LOG_SIZE_IN_LINES ), 5000 );
    }
    int maxLogTimeout = parseIntArgument( maxLogTimeoutOption, 0 );
    if ( Utils.isEmpty( maxLogTimeoutOption.getArgument() ) ) {
      maxLogTimeout = Const.toInt( EnvUtil.getSystemProperty( Const.HOP_MAX_LOG_TIMEOUT_IN_MINUTES ), 1440 );
    }
    HopLogStore.init( maxLogLines, maxLogTimeout );
  }
   ...
   * @throws HopException Error parsing provided argument as an integer
   */
  protected static int parseIntArgument( final CommandLineOption option, final int def ) throws HopException {
    if ( !Utils.isEmpty( option.getArgument() ) ) {
      try {
        return Integer.parseInt( option.getArgument().toString() );
      } catch ( NumberFormatException ex ) {
        throw new HopException( BaseMessages.getString( PKG, "Kitchen.Error.InvalidNumberArgument", option
