...
public class ProcessFiles extends BaseStep implements StepInterface {
  ...
  public boolean processRow( StepMetaInterface smi, StepDataInterface sdi ) throws HopException {
    ...
    try {
      ...
      data.sourceFile = HopVFS.getFileObject( sourceFilename, getTransMeta() );
      ...
      if ( meta.getOperationType() != ProcessFilesMeta.OPERATION_TYPE_DELETE ) {
        ...
        data.targetFile = HopVFS.getFileObject( targetFilename, getTransMeta() );
      ...
      switch ( meta.getOperationType() ) {
        ...
        case ProcessFilesMeta.OPERATION_TYPE_MOVE:
          if ( ( ( meta.isOverwriteTargetFile() && data.targetFile.exists() ) || !data.targetFile.exists() )
            && !meta.simulate ) {
            data.sourceFile.moveTo( HopVFS.getFileObject( targetFilename, getTransMeta() ) );
            if ( log.isDetailed() ) {
              logDetailed( BaseMessages.getString(
