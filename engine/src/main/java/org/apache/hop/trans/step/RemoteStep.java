...
public class RemoteStep implements Cloneable, XMLInterface, Comparable<RemoteStep> {
  ...
  public RemoteStep( Node node ) throws HopException {
  ...
  private Object[] getRowOfData( RowMetaInterface rowMeta ) throws HopFileException {
    Object[] rowData = null;
  ...
  public synchronized BlockingRowSet openReaderSocket( final BaseStep baseStep ) throws IOException,
    ...
    HopException lastException = null;
    ...
    while ( !connected
      ...
      try {
        ...
        lastException =
          new HopException( "Unable to open socket to server " + realHostname + " port " + portNumber, e );
      }
      if ( lastException != null ) {
        ...
        try {
          ...
          throw new HopException( "Interrupted while trying to connect to server socket: " + e.toString() );
        }
      }
    }
    ...
    if ( lastException != null ) {
      ...
      if ( inputStream == null ) {
        throw new HopException( "Unable to connect to the SocketWriter in the "
    ...
    Runnable runnable = new Runnable() {
      public void run() {
        try {
          ...
          if ( rowMeta == null ) {
            throw new HopEOFException(); // leave now.
          }
