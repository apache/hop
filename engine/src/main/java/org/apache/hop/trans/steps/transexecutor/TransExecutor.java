...
public class TransExecutor extends BaseStep implements StepInterface {
  ...
  public boolean processRow( StepMetaInterface smi, StepDataInterface sdi ) throws HopException {
    try {
      ...
      throw new HopException( BaseMessages.getString( PKG, "TransExecutor.UnexpectedError" ), e );
    }
  }
  ...
  private void executeTransformation( List<String> incomingFieldValues ) throws HopException {
    TransExecutorData transExecutorData = getData();
  ...
  void discardLogLines( TransExecutorData transExecutorData ) {
    ...
    if ( executorTrans != null ) {
      HopLogStore.discardLines( executorTrans.getLogChannelId(), false );
      LoggingRegistry.getInstance().removeIncludingChildren( executorTrans.getLogChannelId() );
    }
  }

  @VisibleForTesting
  Trans createInternalTrans() throws HopException {
  ...
  @VisibleForTesting
  void collectTransResults( Result result ) throws HopException {
