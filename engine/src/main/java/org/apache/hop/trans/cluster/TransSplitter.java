...
import org.apache.hop.trans.steps.socketreader.SocketReaderMeta;
import org.apache.hop.trans.steps.socketwriter.SocketWriterMeta;
...
public class TransSplitter {
   ...
   */
  public TransSplitter( TransMeta transMeta ) throws HopException {
    this( transMeta, new TransMetaFactoryImpl() );
  }

  protected TransSplitter( TransMeta transMeta, TransMetaFactory transMetaFactory ) throws HopException {
    this();
    ...
    if ( clusterSchema == null ) {
      throw new HopException( "No clustering is used in this transformation." );
    }
    if ( clusterSchema.isDynamic() ) {
      ...
      if ( masterSlaveServer == null ) {
        throw new HopException( "You always need at least one master in a cluster schema." );
      }
  ...
  private void checkClusterConfiguration() throws HopException {
    ...
    for ( int i = 0; i < steps.size(); i++ ) {
      ...
      if ( clusterSchema != null ) {
        ...
        if ( clusterSchema.findMaster() == null ) {
          throw new HopException( "No master server was specified in cluster schema [" + clusterSchema + "]" );
        }
        ...
        if ( count <= 0 ) {
          throw new HopException( "At least one slave server is required to be present in cluster schema ["
            + clusterSchema + "]" );
        }
      }
    }
  ...
  private TransMeta getSlaveTransformation( ClusterSchema clusterSchema,
      SlaveServer slaveServer ) throws HopException {
  ...
  public SlaveServer getMasterServer() throws HopException {
    ...
    throw new HopException( "No master server could be found in the original transformation" );
  }

  public void splitOriginalTransformation() throws HopException {
    clear();
    ...
    try {
            ...
            if ( !previousStep.isClustered() ) {
              ...
              for ( int s = 0; s < slaveServers.size(); s++ ) {
                ...
                if ( !targetSlaveServer.isMaster() ) {
                  ...
                  // throw new
                  // HopException("The number of step copies on the master has to be 1 or equal
      ...
      throw new HopException( "Unexpected problem while generating master transformation", e );
    }
  }
  ...
  private int getSlaveServerNumber( ClusterSchema clusterSchema, SlaveServer slaveServer ) throws HopException {
    int index = 0;
   ...
   * @throws HopException
   */
  private void generateSlavePartitionSchemas() throws HopException {
