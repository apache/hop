...
package org.apache.hop.job.entries.exportrepository;
...
public class JobEntryExportRepository extends JobEntryBase implements Cloneable, JobEntryInterface {
  ...
  public void loadXML( Node entrynode, List<DatabaseMeta> databases, List<SlaveServer> slaveServers,
    ...
    try {
      ...
      throw new HopXMLException( BaseMessages.getString( PKG, "JobExportRepository.Meta.UnableLoadXML" ), xe );
    }
  }
  ...
  public void loadRep( Repository rep, IMetaStore metaStore, ObjectId id_jobentry, List<DatabaseMeta> databases,
    ...
    try {
      ...
      throw new HopException( BaseMessages.getString( PKG, "JobExportRepository.Meta.UnableLoadRep", ""
        + id_jobentry ), dbe );
    }
  }
  ...
  public void saveRep( Repository rep, IMetaStore metaStore, ObjectId id_job ) throws HopException {
    try {
      ...
      throw new HopException( BaseMessages.getString( PKG, "JobExportRepository.Meta.UnableSaveRep", ""
        + id_job ), dbe );
    }
  }
  ...
  public Result execute( Result previousResult, int nr ) {
    ...
    try {
      file = HopVFS.getFileObject( realoutfilename, this );
      if ( file.exists() ) {
        if ( export_type.equals( Export_All )
          || export_type.equals( Export_Jobs ) || export_type.equals( Export_Trans )
          || export_type.equals( Export_One_Folder ) ) {
          if ( iffileexists.equals( If_FileExists_Fail ) ) {
            ...
            String parentFolder = HopVFS.getFilename( file.getParent() );
            String shortFilename = file.getName().getBaseName();
            shortFilename = buildUniqueFilename( shortFilename );
            file = HopVFS.getFileObject( parentFolder + Const.FILE_SEPARATOR + shortFilename, this );
            if ( log.isDetailed() ) {
      ...
      realoutfilename = HopVFS.getFilename( this.file );
      ...
      if ( !repdir.isRoot() ) {
        if ( repdir.toString().lastIndexOf( "/" ) == 0 ) {
          ...
          if ( newfolder ) {
            ...
            this.file = HopVFS.getFileObject( foldername, this );
            if ( !this.file.exists() ) {
          ...
          this.file = HopVFS.getFileObject( filename, this );
  ...
  private void addFileToResultFilenames( String fileaddentry, LogChannelInterface log, Result result, Job parentJob ) {
    try {
      ResultFile resultFile =
        new ResultFile( ResultFile.FILE_TYPE_GENERAL, HopVFS.getFileObject( fileaddentry, this ), parentJob
          .getJobname(), toString() );
