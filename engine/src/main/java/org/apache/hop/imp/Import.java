...
import org.apache.hop.repository.CanLimitDirs;
...
import org.apache.hop.repository.RepositoryImportFeedbackInterface;
  ...
  public static void main( String[] a ) throws HopException {
    ...
    if ( !Utils.isEmpty( optionVersion ) ) {
      ...
      log.logBasic( BaseMessages.getString(
        PKG, "Import.Log.HopVersion", buildVersion.getVersion(), buildVersion.getRevision(), buildVersion
          .getBuildDate() ) );
      if ( a.length == 1 ) {
        exitJVM( 6 );
      }
    ...
    try {
      ...
      if ( !limitDirs.isEmpty() ) {
        if ( importer instanceof CanLimitDirs ) {
          ...
          throw new HopException( BaseMessages.getString( PKG, "Import.CouldntLimitDirs", importer.getClass()
              .getCanonicalName() ) );
        }
      }
  ...
  protected static int parseIntArgument( final CommandLineOption option, final int def ) throws HopException {
    if ( !Utils.isEmpty( option.getArgument() ) ) {
      try {
        ...
        throw new HopException( BaseMessages.getString( PKG, "Import.Error.InvalidNumberArgument", option
