...
package org.apache.hop.pipeline.steps.tableinput;
...
public class TableInput extends BaseStep implements StepInterface {
  ...
  public TableInput( StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, PipelineMeta pipelineMeta,
                     Pipeline pipeline ) {
  ...
  private RowMetaAndData readStartDate() throws HopException {
    ...
    if ( rowSet != null ) {
      ...
      if ( parametersMeta.size() == 0 ) {
        throw new HopException( "Expected to read parameters from step ["
          + data.infoStream.getStepname() + "] but none were found." );
      }
    } else {
      throw new HopException( "Unable to find rowset to read from, perhaps step ["
