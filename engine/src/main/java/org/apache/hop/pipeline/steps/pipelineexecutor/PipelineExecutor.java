...
package org.apache.hop.pipeline.steps.pipelineexecutor;
 ...
 * {@link org.apache.hop.pipeline.steps.jobexecutor.JobExecutor JobExecutor}.
 ...
 * @since 18-mar-2013
 */
public class PipelineExecutor extends BaseStep implements StepInterface {
  ...
  public PipelineExecutor( StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, PipelineMeta pipelineMeta,
                           Pipeline pipeline ) {
   ...
   * MappingInput and one MappingOutput step in the PipelineExecutor.
   */
  public boolean processRow( StepMetaInterface smi, StepDataInterface sdi ) throws HopException {
    try {
      ...
      RowSet executorStepOutputRowSet = pipelineExecutorData.getExecutorStepOutputRowSet();
      if ( pipelineExecutorData.getExecutorStepOutputRowMeta() != null && executorStepOutputRowSet != null ) {
        putRowTo( pipelineExecutorData.getExecutorStepOutputRowMeta(), row, executorStepOutputRowSet );
      }
  ...
  private void initOnFirstProcessingIteration() throws HopException {
    PipelineExecutorData pipelineExecutorData = getData();
    // internal pipeline's first step has exactly the same input
    ...
    if ( meta.getResultFilesTargetStepMeta() != null ) {
      ...
      pipelineExecutorData.setResultFilesRowSet( findOutputRowSet( meta.getResultFilesTargetStepMeta().getName() ) );
    }
    ...
    if ( meta.getOutputRowsSourceStepMeta() != null ) {
      ...
      pipelineExecutorData.setResultRowsRowSet( findOutputRowSet( meta.getOutputRowsSourceStepMeta().getName() ) );
    }

    // executor's self output is exactly its input
    if ( meta.getExecutorsOutputStepMeta() != null ) {
      pipelineExecutorData.setExecutorStepOutputRowMeta( getInputRowMeta().clone() );
      pipelineExecutorData.setExecutorStepOutputRowSet( findOutputRowSet( meta.getExecutorsOutputStepMeta().getName() ) );
    }
  ...
  private void executePipeline( List<String> incomingFieldValues ) throws HopException {
    ...
    getPipeline().addActiveSubPipeline( getStepname(), executorPipeline );
  ...
  Pipeline createInternalPipeline() throws HopException {
    ...
    PipelineStepUtil.initServletConfig( getPipeline(), executorPipeline );
  ...
  void collectPipelineResults( Result result ) throws HopException {
    ...
    if ( meta.getOutputRowsSourceStepMeta() != null && pipelineResultsRowSet != null ) {
