...
/**
 * Meta-data for the Pipeline Executor step.
 *
 * @author Matt
 * @since 18-mar-2013
 */
public class PipelineExecutorMeta extends StepWithMappingMeta implements StepMetaInterface, ISubPipelineAwareMeta {
  ...
  public void loadXML( Node stepnode, IMetaStore metaStore ) throws HopXMLException {
    try {
      ...
      //
      Node mappingParametersNode = XMLHandler.getSubNode( stepnode, PipelineExecutorParameters.XML_TAG );
  ...
  protected void addFieldToRow( RowMetaInterface row, String fieldName, int type, int length, int precision )
    ...
    if ( !Utils.isEmpty( fieldName ) ) {
      try {
        ...
        throw new HopStepException( BaseMessages.getString( PKG, "PipelineExecutorMeta.ValueMetaInterfaceCreation",
          fieldName ), e );
      }
    }
  }
  ...
  public void check( List<CheckResultInterface> remarks, PipelineMeta pipelineMeta, StepMeta stepinfo, RowMetaInterface prev,
    ...
    if ( prev == null || prev.size() == 0 ) {
      cr =
        new CheckResult( CheckResultInterface.TYPE_RESULT_WARNING, BaseMessages.getString( PKG,
          "PipelineExecutorMeta.CheckResult.NotReceivingAnyFields" ), stepinfo );
      remarks.add( cr );
    } else {
      cr =
        new CheckResult( CheckResultInterface.TYPE_RESULT_OK, BaseMessages.getString( PKG,
          "PipelineExecutorMeta.CheckResult.StepReceivingFields", prev.size() + "" ), stepinfo );
      remarks.add( cr );
    }
    ...
    if ( input.length > 0 ) {
      cr =
        new CheckResult( CheckResultInterface.TYPE_RESULT_OK, BaseMessages.getString( PKG,
          "PipelineExecutorMeta.CheckResult.StepReceivingFieldsFromOtherSteps" ), stepinfo );
      remarks.add( cr );
    } else {
      cr =
        new CheckResult( CheckResultInterface.TYPE_RESULT_ERROR, BaseMessages.getString( PKG,
          "PipelineExecutorMeta.CheckResult.NoInputReceived" ), stepinfo );
      remarks.add( cr );
    }
  }
  ...
  public StepInterface getStep( StepMeta stepMeta, StepDataInterface stepDataInterface, int cnr, PipelineMeta tr,
                                Pipeline pipeline ) {
    return new PipelineExecutor( stepMeta, stepDataInterface, cnr, tr, pipeline );
  }
  ...
  /**
   * @return The objects referenced in the step, like a mapping, a pipeline, ...
   */
