...
public class ContentEditorWidget implements IContentEditorWidget {
  ...
  private void addSourceViewer() {
    ...
    this.sourceViewer = new ProjectionViewer(control, ruler, null, false, styles);
    ...
    if (dark) {
      org.eclipse.swt.graphics.Color lineNumFg =
          new org.eclipse.swt.graphics.Color(control.getDisplay(), 130, 130, 130);
  ...
  private void addToolbar() {
    ...
    IToolbarContainer toolbarContainer =
        ToolbarFacade.createToolbarContainer(control, SWT.WRAP | SWT.RIGHT | SWT.HORIZONTAL);
   ...
   * start of the text, tracking a bracket stack. The top of the stack at the cursor position is the
  ...
  private static int[] findEnclosingBrackets(String text, int offset) {
    ...
    while (!stack.isEmpty()) {
      int[] top = stack.pop();
      int openPos = top[0];
      int closePos = findMatchForward(text, openPos, OPEN_BRACKETS[top[1]], CLOSE_BRACKETS[top[1]]);
      if (closePos >= offset) {
    ...
    styledText.addListener(
        ...
        event -> {
          ...
          contextMenuWidgets.enableMenuItem(
              ID_CONTEXT_MENU_UNDO, sourceViewer.canDoOperation(ITextOperationTarget.UNDO));
          contextMenuWidgets.enableMenuItem(
              ID_CONTEXT_MENU_REDO, sourceViewer.canDoOperation(ITextOperationTarget.REDO));
          contextMenuWidgets.enableMenuItem(
              ID_CONTEXT_MENU_CUT, sourceViewer.canDoOperation(ITextOperationTarget.CUT));
          contextMenuWidgets.enableMenuItem(
  ...
  void installBlockGuidePainter() {
    ...
    boolean dark = PropsUi.getInstance().isDarkMode();
    Color guideColor =
        new Color(st.getDisplay(), dark ? 60 : 200, dark ? 60 : 200, dark ? 60 : 200);
  ...
  void installCollapsePlaceholderPainter() {
    ...
    Color placeholderColor =
        new Color(st.getDisplay(), dark ? 140 : 120, dark ? 140 : 120, dark ? 140 : 120);
  ...
  private void updateFoldingRegions() {
    ProjectionAnnotationModel model = sourceViewer.getProjectionAnnotationModel();
    if (model == null) return;
  ...
  /**
   * When language is XML, typing '>' after an open tag inserts the closing tag and places the caret
   * between '>' and '</tagname>'.
   */
