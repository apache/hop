...
public class Database implements IVariables, ILoggingObject, AutoCloseable {
    ...
    public static SqlParameterSpec parseSqlParameterSpec(String sourceSql) {
        ...
        while (index < sql.length()) {
            ...
            if (c == '?') {
                ...
                if (isPostgresJsonbOperator(sql, index)) {
    ...
    /**
     * Consume a quoted section of SQL, starting at the given index. The section is terminated by the
     * same quote character, unless it is escaped by doubling it.
     *
     * @param sql the SQL string
     * @param startIndex the index of the opening quote character
     * @param quoteCharacter the quote character (' or " or `)
     * @param allowDoubledQuoteEscape true if doubled quotes are allowed as escape
     * @return the index of the character after the closing quote, or sql.length() if not found
     */
    ...
    /**
     * Consume a bracketed identifier of SQL, starting at the given index. The section is terminated by
     * the closing bracket, unless it is escaped by doubling it.
     *
     * @param sql the SQL string
     * @param startIndex the index of the opening bracket character
     * @return the index of the character after the closing bracket, or sql.length() if not found
     */
    ...
    /**
     * Get the dollar-quote tag starting at the given index, if any. A dollar-quote tag is a string of
     * the form $tag$ where tag is an optional sequence of letters, digits, and underscores.
     *
     * @param sql the SQL string
     * @param startIndex the index of the opening dollar sign
     * @return the dollar-quote tag, or null if not found
     */
    ...
    /**
     * Check if the question mark at the given index is part of a PostgreSQL JSONB operator.
     *
     * @param sql the SQL string
     * @param questionMarkIndex the index of the question mark
     * @return true if the question mark is part of a JSONB operator, false otherwise
     */
    private static boolean isPostgresJsonbOperator(String sql, int questionMarkIndex) {
        ...
        int previous = findPreviousNonWhitespace(sql, questionMarkIndex - 1);
        int next = findNextNonWhitespace(sql, questionMarkIndex + 1);
        if (previous < 0 || next < 0) {
            return false;
        }

        return isLikelyJsonbLeftOperand(sql.charAt(previous))
                && isLikelyJsonbRightOperandStart(sql.charAt(next));
    }

    /**
     * Find the index of the previous non-whitespace character in the SQL string, starting from the given index.
     *
     * @param sql the SQL string
     * @param index the starting index
     * @return the index of the previous non-whitespace character, or -1 if not found
     */
    private static int findPreviousNonWhitespace(String sql, int index) {
    ...
    /**
     * Find the index of the next non-whitespace character in the SQL string, starting from the given index.
     *
     * @param sql the SQL string
     * @param index the starting index
     * @return the index of the next non-whitespace character, or -1 if not found
     */
    private static int findNextNonWhitespace(String sql, int index) {
        while (index < sql.length() && Character.isWhitespace(sql.charAt(index))) {
            index++;
        }
        return index >= sql.length() ? -1 : index;
    }

    /**
     * Check if the character is likely to be part of a JSONB left operand.
     *
     * @param c the character to check
     * @return true if the character is likely to be part of a JSONB left operand, false otherwise
     */
    private static boolean isLikelyJsonbLeftOperand(char c) {
    ...
    /**
     * Check if the character is likely to be the start of a JSONB right operand.
     *
     * @param c the character to check
     * @return true if the character is likely to be the start of a JSONB right operand, false otherwise
     */
    private static boolean isLikelyJsonbRightOperandStart(char c) {
        return Character.isLetterOrDigit(c)
                || c == '_'
                || c == '$'
                || c == '\''
    ...
    /**
     * Count the number of parameters in the given SQL string.
     *
     * @param sql the SQL string
     * @return the number of parameters in the SQL string
     */
    public int countParameters(String sql) {
        return parseSqlParameterSpec(sql).getParameterCount();
    }
