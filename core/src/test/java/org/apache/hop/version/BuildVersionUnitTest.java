...
public class BuildVersionUnitTest {
  ...
  private void initEnvironmentVariableGetter( String version, String revision, String date, String user ) {
    try {
      when( mockEnvironmentVariableGetter.getEnvVarible( BuildVersion.HOP_BUILD_VERSION ) ).thenReturn( version );
      when( mockEnvironmentVariableGetter.getEnvVarible( BuildVersion.HOP_BUILD_REVISION ) ).thenReturn( revision );
      when( mockEnvironmentVariableGetter.getEnvVarible( BuildVersion.HOP_BUILD_DATE ) ).thenReturn( date );
      when( mockEnvironmentVariableGetter.getEnvVarible( BuildVersion.HOP_BUILD_USER ) ).thenReturn( user );
    } catch ( Exception e ) {
      // wont happen
    }
  }
