...
See xref:manual::hop-server/deploy-project-image.adoc#skip_hop_server_short_lived_containers[Skip Hop Server: short-lived containers].
...
To ship a git-hosted project *with* the image (CI builds `FROM apache/hop`, copies the project to `/your-project`, then rolls the container) see xref:manual::hop-server/deploy-project-image.adoc[Project in a Docker image].
