...
If you just want to see the flattened logging text for a channel you can use the read facade:

[source,java]
----
int lastNr = HopLogStore.getMaxNrLines();
List<String> loggingText = HopLogStore.getLogBufferText( logChannelId, false, 0, lastNr);
----

Use `HopLogStore.getLogBufferText(...)` instead of the deprecated `HopLogStore.getAppender()`, which exposes an internal implementation detail.
