...
Hop Web has two themes: "light" and "dark". The theme is a per-user setting stored in the audit folder. Users choose it in Configuration → GUI options (Dark mode); the page reloads to apply it. Use `/ui` for light and `/ui-dark` for dark, or let the app redirect based on the saved preference.
...
`docker run -p 8080:8080 hop-web`
...
For the operator-facing volume layout, Compose example, and upgrade procedure, see xref:{page-component-version}@manual::hop-web-docker.adoc[Hop Web in Docker: persistence and upgrades].
