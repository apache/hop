...
The user-facing workbench is documented in xref:{page-component-version}@manual::hop-gui/perspective-ai-advisor.adoc[AI Assistant].
...
. Keep `defaultSelected = false` for sensitive extras. A plugin-id catalog may default on. Set `summary` and `listBaselineSharing()` so the collapsed Sharing line is accurate.
