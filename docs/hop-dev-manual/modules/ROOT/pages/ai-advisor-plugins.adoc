...
The default `parseResponse` extracts ` ```hop_proposals ` JSON blocks (`AiProposalParser` in hop-core).
Override `parseResponse` only when you use a different fence (for example ` ```dv_proposals `).
...
. Emit `hop_proposals` unless you override `parseResponse`. Override `previewProposal` for custom review text.
