...
# Optionally filter which main*.hwf workflows run (basename substring or shell glob;
# comma-separated lists are supported). Quote globs so the shell does not expand them.
