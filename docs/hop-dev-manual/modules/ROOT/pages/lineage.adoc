...
| `LineageVariables` — Hop variables (`@Variable`) that describe hub configuration. Also the per-kind emitters transforms call (`LineageFileIoEmitter`, `LineageRelationalIoEmitter`, `LineageHttpIoEmitter`, `LineageRunLifecycleEmitter`, `LineageTransformSchemaEmitter`) and the shared dataset-identity helpers (`LineageFileIdentity`, `LineageRelationalIdentity`). Emitters are best-effort by construction: they no-op on missing arguments and never throw into transform execution, so call sites invoke them directly without a defensive `try`/`catch`.
