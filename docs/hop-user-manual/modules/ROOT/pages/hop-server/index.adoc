...
Hop Server is a lightweight server to run workflows and pipelines with the xref:pipeline/pipeline-run-configurations/native-remote-pipeline-engine.adoc[Remote pipeline], xref:workflow/workflow-run-configurations/native-remote-workflow-engine.adoc[Remote workflow], or xref:pipeline/pipeline-run-configurations/native-load-balancing-pipeline-engine.adoc[load-balancing] run configurations.

How you get a project onto a Hop Server is covered in xref:hop-server/deploying.adoc[Deploying Hop Server].
...
Run a Hop server
  ...
  -ws, --workflow-status=<workflowName>
                          List the status of the workflow with this name (also
                            specify the -id option)
      -xs, --cross-site-policy=<crossSitePolicy>
                          Which browser requests this server accepts, based on
                            the Sec-Fetch-Site header: 'same-site' (the
                            default) rejects cross-site requests, 'same-origin'
                            also rejects same-site ones, and 'off' disables the
                            check. Clients that send no Sec-Fetch-* headers,
                            such as hop-run or the Hop GUI, are never affected.
----
...
The project name is taken from that environment, so you do not need `-j` as well.
...
|--shutdown-timeout
|Default: 0 +
The maximum number of seconds to wait for running pipelines and workflows to finish when the server shuts down (0 shuts down immediately). Can also be set with the `HOP_SERVER_SHUTDOWN_TIMEOUT` environment variable.

|-xs
|--cross-site-policy
|Default: same-site +
Which browser requests the server accepts: `same-site`, `same-origin` or `off`. Can also be set with the `HOP_SERVER_CROSS_SITE_POLICY` environment variable. See <<Cross-site requests>>.
...
<hop-server-config>
  ...
  <!-- The maximum number of log lines kept in memory by the server.
       Leave this at 0 or leave the element out to use the HOP_MAX_LOG_SIZE_IN_LINES variable,
  ...
  <!-- The time (in minutes) it takes for a log line to be cleaned up in memory.
       Leave this at 0 or leave the element out to use the HOP_MAX_LOG_TIMEOUT_IN_MINUTES variable,
       which cleans up log lines after 1440 minutes (one day) by default.
  -->
  <max_log_timeout_minutes>1440</max_log_timeout_minutes>

  <!-- The time (in minutes) it takes for a finished or stopped pipeline or workflow execution to be
       removed from the server status.
       Leave this at 0 or leave the element out to use the HOP_SERVER_OBJECT_TIMEOUT_MINUTES
       variable, which removes executions after 1440 minutes (one day) by default.
  -->
  <object_timeout_minutes>1440</object_timeout_minutes>

  <!-- An extra folder to read metadata objects from, for example for web services.
       It is added on top of the metadata the server already has (its project, or the default project):
       on a name clash the object in this folder is used.
       Pipelines and workflows that clients send also bring their own metadata, which wins over the server's.
       See "Metadata and variables on Hop Server" in the user manual.
  -->
...
`max_log_lines`, `max_log_timeout_minutes` and `object_timeout_minutes` keep the memory use of a
long running server in check. Each of them is resolved separately, and the first of these that
supplies a value wins:

. the configuration file, when the element is set to a value larger than 0
. the matching Java system property, for example `-DHOP_SERVER_OBJECT_TIMEOUT_MINUTES=60`
...
|`1440`: finished and stopped executions are removed from the server status after one day
|===

A server that is started without a configuration file, or without these elements, therefore keeps
all of its log lines, cleans them up after a day, and removes finished executions from its status
after a day. The `Configuration details` section of the status page reports the values the server
is actually running with.

WARNING: The default keeps *every* log line of *every* execution for a day. A server that runs a lot
of short executions can run out of memory that way. Set `max_log_lines` to limit this.

[NOTE]
====
These three settings are read from Java system properties, not from the environment of the process.
Setting an environment variable on a container, for example `-e HOP_MAX_LOG_SIZE_IN_LINES=5000`,
therefore has no effect: the value from `hop-config.json` is used instead. To override them in a
...
Mind that `HOP_OPTIONS` replaces the default `-Xmx2048m`, so repeat the memory settings you need.
Alternatively, set the elements in the server configuration file, which takes precedence over both.
====
...
For a complete description of the standard Hop docker container see the xref:docker-container.adoc[Hop in Docker] page.
...
When Hop Server is started from a terminal, the process can be terminated through `CTRL-C`.

Hop Server registers a JVM shutdown hook, so it also shuts down gracefully when the process receives a `SIGTERM`, for example through `kill <pid>`, `docker stop` or a `systemd` stop.

In headless environments, Hop Server exposes a shutdown servlet on the `/shutdown` path.
An authenticated `GET` request to that path triggers a graceful shutdown of the server:
...
--
[source,shell]
curl -u cluster:cluster http://127.0.0.1:8080/shutdown
--
...
Whichever way a shutdown is triggered (`CTRL-C`, `SIGTERM` or the `/shutdown` servlet), Hop Server shuts down gracefully:

* it immediately stops accepting new work: requests that add or start pipelines, workflows or packages are refused with an HTTP `503 Service Unavailable`, while status requests keep working,
* the server status reports `Shutting down` (the status XML/JSON contains `shutting_down=Y`), which is useful as a Kubernetes readiness signal,
* it waits for the running pipelines and workflows to finish before the process exits.

By default the server waits `0` seconds, i.e. it shuts down immediately.
Use the `--shutdown-timeout` option (or the `HOP_SERVER_SHUTDOWN_TIMEOUT` environment variable) to allow running executions to finish within a maximum number of seconds.
When the timeout elapses while executions are still running, the server shuts down anyway.
...
The Hop Server servlets answer state-changing operations on `GET`: a single URL starts a pipeline, stops a workflow or removes one from the server.
That is convenient for automation, but it also means a web page an operator happens to be visiting can drive the server simply by pointing an image, a form or a link at it, using the operator's browser and their credentials.

To close that path, Hop Server rejects requests whose https://developer.mozilla.org/docs/Web/HTTP/Headers/Sec-Fetch-Site[`Sec-Fetch-Site`] header says they were started by another site, answering them with an HTTP `403 Forbidden` before authentication or any servlet runs.

Only browsers send `Sec-Fetch-*` headers.
hop-run, the Hop GUI, `curl` and your own automation send none at all, and requests without the header are always accepted, so **existing integrations are not affected**.

The `--cross-site-policy` option (or the `HOP_SERVER_CROSS_SITE_POLICY` environment variable) picks how strict the server is:
...
A link or a form on another host of the same domain — a second Hop server, an intranet portal, the same host on another port — is still accepted.

|`same-origin`
|Also rejects requests started by another host of the same domain, so only the server's own pages may drive it.

|`off`
|Does not look at `Sec-Fetch-Site` at all.
An escape hatch for a deployment that turns out to need cross-site browser access; it leaves the server open to the requests described above.

|===

[source,shell]
 ./hop-server.sh 127.0.0.1 8080 --cross-site-policy same-origin

Two consequences worth knowing about:

* Following a link to the server **from another site** lands on a `403`, including a plain click-through to the status page.
Typing the address or using a bookmark sends `Sec-Fetch-Site: none` and keeps working, as does every link on the server's own pages.
* A `Sec-Fetch-Site` value the server does not recognise, or more than one of them on the same request, is rejected as well.
A client that cannot send the header correctly can always send nothing at all.

This is a defence against one specific attack, not a substitute for the basics: keep authentication enabled, change the default `cluster`/`cluster` credentials and do not expose the server to an untrusted network.

Hop Web applies the same policy to the Hop Server API it co-deploys on `/hop/*`, configured in its own security settings — see xref:hop-gui/hop-web.adoc[Hop Web].
