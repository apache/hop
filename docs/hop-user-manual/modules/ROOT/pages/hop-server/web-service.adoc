...
|Status field
|The field that contains the status code to be returned by the service, default status code is `200` if left empty.
Only honoured on `hop/webService`: the xref:hop-server/json-api.adoc[JSON API] commits its response status before the body is streamed.
...
|This is the name of the variable which at runtime will contain the content of the request body.  This is useful when doing a POST against the webservice.

|Request header content variable
|This is the name of the variable which at runtime will contain the content of the request header.  This is return a json object containing all headers that were in the request.
