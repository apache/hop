...
Open your OpenBAO and Vault resolvers and check the option rather than assuming the upgrade covered them.
...
Against an OpenBAO with a self-signed certificate or one signed by a private CA, give the resolver that certificate through **PEM file path** or **PEM string**; without it the lookup fails and the expression is left unresolved.
====
