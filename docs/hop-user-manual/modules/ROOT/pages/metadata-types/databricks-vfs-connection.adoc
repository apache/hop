...
* **Explore** on this editor opens the xref:hop-gui/vfs-file-explorer.adoc[VFS File Explorer] at `+name:///+`. The root path is applied by the provider, so do not put it in that URI.
