...
* xref:metadata-types/ai-provider.adoc[AI Provider]: Named language-model endpoint (credentials and model) used by the AI Assistant and optionally by Language Model Chat
...
* xref:metadata-types/azure-authentication.adoc[Azure Connection]: An Azure Blob Storage connection type.
...
* xref:metadata-types/execution-information-location.adoc[Execution Information Location]: Defines where and how Apache Hop stores execution metadata, supporting local files, remote servers, a database, Neo4j, Elastic or OpenSearch for later inspection and analysis.
...
* xref:metadata-types/ftp-connection.adoc[FTP Connection]: Named FTP or FTPS server used as a VFS scheme in file paths and by the FTP actions
* xref:metadata-types/git-connection.adoc[Git Connection]: REST API endpoint and credentials for GitHub, GitLab, Bitbucket, Forgejo or Gitea
* xref:metadata-types/google-storage-authentication.adoc[Google Storage Connection]: A Google Cloud Storage connection type.
* xref:metadata-types/hdfs-connection.adoc[HDFS Connection]: Named HDFS access via HttpFS, Knox or WebHDFS (no Hadoop client JARs)
...
* xref:metadata-types/redis/redis-connection.adoc[Redis Connection]: Describes a reusable Redis connection (Standalone, Sentinel or Cluster)
