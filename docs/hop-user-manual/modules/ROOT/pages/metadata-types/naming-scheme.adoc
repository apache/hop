...
* **N indicator** -- widgets that accept a naming scheme show a small *N* on the right (same diamond plate as `$` for variables and `#` for expanded integers). Click *N* to open the matching scheme in a dialog. If several schemes of that type exist, pick one first.
...
|Type|Target kind. *General* applies to every name field unless a scheme of the widget's specific type exists (Hop field, transform, action, pipeline, workflow, metadata, variable, database table, database column, file, or folder).
...
. Focus a name widget -- transform name, action name, a field-name cell, a table name, a file name, …
...
To review or change the rules instead of applying them, click the *N* indicator. That opens the scheme editor (or a picker when more than one scheme matches the field type).
