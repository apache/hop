...
The connection editor has *Explore* (the classic table picker) and *Open in Database* (the xref:hop-gui/perspective-database.adoc[Database perspective], to browse schemas and run SQL). From that perspective, *Edit connection* opens this metadata editor.
