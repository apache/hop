...
The receiving (probing) pipeline (`+${PROJECT_HOME}/pipeline-probe/pipeline-probe-example.hpl+`) has a xref:pipeline/transforms/pipeline-data-probe.adoc[Pipeline Data Probe] as input. This pipeline will then denormalize the received data to field, count the number of books per genre, sort the results and writes the final data out to a file (`+${PROJECT_HOME}/books-per-genre/probe-data.csv+`).
