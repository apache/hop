...
| `+${HOP_CONFIG_FOLDER}/projects/+` in the *image* config.
...
If `projectHome` still points at `+${HOP_CONFIG_FOLDER}/projects/samples+`, you are editing starter samples inside config.
...
services:
  hop-web:
    ...
    environment:
      ...
      HOP_WEB_ADMIN_PASSWORD: ${HOP_WEB_ADMIN_PASSWORD}
    volumes:
