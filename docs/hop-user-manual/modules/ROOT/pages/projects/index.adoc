...
TIP: In Hop Web Docker, keep *project homes* on a host folder or volume as well. The image's `default` and `samples` projects are only a starter layout. See xref:tech-manual::hop-web-docker.adoc[Hop Web in Docker: persistence and upgrades].
