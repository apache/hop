...
E.g. a `CRM` database connection is created in the project, using a number of variables for hostname (`+${CRM_HOST}+`) and database name (`+${CRM_DBNAME}+`), with corresponding variables for the port, username, password and so on.
* the metadata object _configuration_ is defined on an environment level, where a number of variable _values_ are created the contain the ip address for `+${CRM_HOST}+` etc.
