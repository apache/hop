...
:description: Hop Gui provides a number of perspectives for a variety of tasks. You'll spend most of your time in the data orchestration perspective, building workflows and pipelines, but other perspectives for metadata management, file management and more offer tons of functionality.
...
* image:icons/database.svg[width="24px"] xref:hop-gui/perspective-database.adoc[Database] (also as a floating window or bottom-dock tab)

* image:icons/ai-provider.svg[width="24px"] xref:hop-gui/perspective-ai-advisor.adoc[AI Assistant] (also as a floating window or bottom-dock tab; off by default)
...
TIP: Project-wide search is not a left-toolbar perspective. Open it with `CTRL-F` (*Edit → Find*). See xref:hop-gui/perspective-search.adoc[Search].