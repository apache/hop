...
IMPORTANT: This perspective only shows executions written by the Neo4j logging in `NEO4J_LOGGING_CONNECTION`. Executions written by a xref:metadata-types/neo4j/neo4j-location-type.adoc[Neo4j] xref:metadata-types/execution-information-location.adoc[execution information location] are stored with a different set of properties and do not appear here: they are shown in the xref:hop-gui/perspective-execution-information.adoc[Execution Information perspective], which gains extra Lineage, Error path and Cypher tabs when the location is a Neo4j one. Both write `Execution` nodes and `EXECUTES` relationships, so point them at separate databases if you use both.
