...
The git commit perspective (`CTRL+SHIFT+O`, `CMD+SHIFT+O` on macOS) lists all files with changes in two groups: 'Staged' (added to git's index with 'Add') and 'Unstaged' (everything else, including files git doesn't track yet).

The toolbar and right-click options act on the files whose checkbox is ticked, not on the highlighted row.
Files git already knows about are ticked for you, so a commit takes your work in progress along: untick a file (or unstage it) to leave it out.
Files that are not tracked yet are listed but never ticked: committing a file for the first time is something you decide, tick it yourself to include it.

[options="header", cols="1,2,5,1"]
|===
|icon|action|description|shortcut
|image:icons/refresh.svg[width="24px"]|Refresh|Reload the list of staged and unstaged files.|F5
|image:icons/git-add.svg[width="24px"]|Add|Stage the checked files. They move to 'Staged' and become part of the next commit.|CTRL+ALT+A
|image:icons/git-unstage.svg[width="24px"]|Unstage|Take the checked files out of the next commit. They move back to 'Unstaged', your changes are kept.|CTRL+ALT+U
