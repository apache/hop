...
|`CTRL-LEFT` / `CTRL-RIGHT`|Any text field. On macOS, `OPTION-LEFT` / `OPTION-RIGHT`|Move to the previous or next word. Hold Shift to extend the selection. `CMD-LEFT` / `CMD-RIGHT` on macOS still go to the start or end of the line
|`CTRL-C` / `CMD-C`|Any text field|Copy the selection, or the current line when nothing is selected
|`CTRL-X` / `CMD-X`|Any text field|Cut the selection, or the current line when nothing is selected. Read-only text is not cut
...
|`CTRL-ENTER` / `Cmd-Enter`|AI Assistant question field|Send the question (same as the Hop logo button)
|===

See xref:metadata-types/naming-scheme.adoc[Naming Scheme] for the *N* indicator, types, and the TableView bulk toolbar.

The AI Assistant perspective uses `CTRL-SHIFT-A` (`Cmd-Shift-A` on macOS). See xref:hop-gui/perspective-ai-advisor.adoc[AI Assistant].
