...
* **Plugin** options provide configuration options to control the behavior of your available Apache Hop plugins (including Explorer, xref:hop-gui/perspective-search.adoc[Search] limits, and xref:hop-gui/perspective-ai-advisor.adoc[AI Assistant] enablement).
...
a|Show the embedded terminal in the bottom panel. Clear this on desktops where that terminal misbehaves. Search, the database workbench, and the VFS file explorer stay available. Hop Web has no embedded terminal. Excluding `40010-menu-tools-terminal` in xref:hop-gui/disable-ui-elements.adoc[disabledGuiElements.xml] forces it off.
|yes
...
See xref:hop-gui/canvas-mouse.adoc[Canvas mouse gestures] for every click in both modes.
|no

|Use right click for the context dialog
|A right click (Ctrl-click or two-finger click on macOS) opens the context dialog of whatever is under the pointer, and a left click never does: it selects, clears the selection, or presses a badge. Names and badges keep opening their editors on a left click.

See xref:hop-gui/canvas-mouse.adoc[Canvas mouse gestures] for every click in both modes.
|no

|Use menus instead of the context dialog
|Transforms, actions, hops and notes show their actions in a pop-up menu with a submenu per category instead of the xref:hop-gui/hop-gui-popup-dialog.adoc[context dialog]. A click on the empty canvas keeps the context dialog while the xref:hop-gui/hop-gui-popup-dialog.adoc#palette-tree[palette tree] is hidden, since that is where new transforms and actions are searched for; with the palette shown the empty canvas gets a menu of the pipeline/workflow actions too. The click that opens the menu is the same one that opens the context dialog (left click, or right click with the option above).
|no
...
|Tooltips
a|A group of its own in the *Pipeline and Workflow canvas* section: one checkbox per kind of tooltip the canvas shows when the mouse rests on something. Untick a kind to stop only that one; *Show tooltip when hovering over a hop or Action/Transform* in the *Tooltips* section of the General tab still switches all of them off at once.

* *Edit hint on transform and action names*: the "Click to edit" hint on a name.
* *Transform and action descriptions*: the description on an icon and its info badge, along with the transform partitioning and whether a transform can start a pipeline.
* *Deprecation warnings*: the warning on the icon of a deprecated transform or action, and its replacement.
* *Hop details*: the source, target and status of a hop when the mouse rests on its line, and the badges on it: info, error, copies, row distribution, target streams and the conditions of workflow hops.
* *Execution results*: after a run, the failure logs, the output rows a transform or hop holds, the result of an action and where a workflow restarted from a checkpoint.
* *Note links*: where a hyperlink in a note leads.
* *Plugin tooltips*: tooltips that plugins add to the canvas, such as the custom debug levels.
* *Selection cleared notice*: the short "Selection cleared" notice after a click on the empty canvas while something was selected.
|all on
...
|Allow dialogs to open on any screen (macOS only)|Off: transform, action and metadata dialogs follow the Hop window and stay above it. On: they open as windows of their own that you can move to another screen. The macOS section only appears on macOS.|no
