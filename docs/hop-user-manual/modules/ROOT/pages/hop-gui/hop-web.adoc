...
Per-user audit data (open tabs, history) continues to be stored under `HOP_AUDIT_FOLDER/users/<username>/` when a principal is available.
...
| Role claim
| Claim path for groups/roles (e.g. `groups`, `roles`, `realm_access.roles` for Keycloak)

| Username claim
| Default `preferred_username` (falls back to `email`, then `sub`)
|===

Environment variables (applied at startup): `HOP_WEB_SECURITY_MODE=OAUTH2`, `HOP_WEB_OAUTH_ISSUER`, `HOP_WEB_OAUTH_CLIENT_ID`, `HOP_WEB_OAUTH_CLIENT_SECRET`, `HOP_WEB_OAUTH_REDIRECT_URI`, `HOP_WEB_OAUTH_SCOPES`, `HOP_WEB_OAUTH_ROLE_CLAIM`, `HOP_WEB_OAUTH_USERNAME_CLAIM`.
...
Map IdP groups to Hop roles with the *Container role mappings* table (same as EXTERNAL), e.g. `hop-admin` → `admin`.
