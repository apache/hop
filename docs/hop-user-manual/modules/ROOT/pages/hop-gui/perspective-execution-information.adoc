...
The execution information displayed in this perspective is not collected automatically.  You need to specify to which xref:metadata-types/execution-information-location.adoc[location] you want to send execution information in the xref:metadata-types/pipeline-run-config.adoc[Pipeline run confirmation] or xref:metadata-types/workflow-run-config.adoc[Workflow run configuration] of your choice.

== Navigation

You can navigate to it by clicking the image:icons/run.svg[width="24px"] icon from the vertical perspective toolbar in the Hop GUI.
