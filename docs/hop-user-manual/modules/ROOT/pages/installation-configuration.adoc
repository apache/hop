...
TIP: Take a look at the xref:tech-manual::docker-container.adoc[Docker] page for `apache/hop` (hop-run and Hop Server), and xref:tech-manual::hop-web-docker.adoc[Hop Web in Docker] for persisting projects and config across image upgrades.
...
Apache Hop works well with these 64-bit java runtimes for version 21.
...
TIP: Instead of changing your system’s JAVA_HOME, you can set the HOP_JAVA_HOME environment variable to point to a specific Java JRE or JDK for Apache Hop. This allows Hop to use a different Java version than your system default, without affecting other applications.
...
The easiest way is xref:hop-tools/hop-setup.adoc[hop setup]: *Tools → Configure Hop environment...* in Hop Gui, or `./hop setup apply --defaults` from the install directory. Restart Hop afterwards.

TIP: if you set the environment variables below after you've been using Apache Hop for a while, you can move the contents of your `hop/config/` folder to your new `HOP_CONFIG_FOLDER` location. `hop setup apply --defaults --copy-existing` does that copy when the new folder is empty.
...
TIP: For Hop Web in Docker, persist `HOP_CONFIG_FOLDER` and project homes on volumes and replace the image. See xref:tech-manual::hop-web-docker.adoc[Hop Web in Docker: persistence and upgrades].
