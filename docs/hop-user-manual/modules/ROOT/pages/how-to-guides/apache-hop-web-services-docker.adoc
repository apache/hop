...
|HOP_SHARED_JDBC_FOLDERS|/files/jdbc|Directory containing all required JDBC drivers (e.g. for MySQL, Oracle) that are not included in the standard delivery. This is a comma separated list defaulting to lib/jdbc
...
On the xref:docker-container.adoc[Hop in Docker] page, further parameters are listed (e.g. when using SSL), which have been omitted here for simplification.
...
To bake the whole project into an image in CI and roll Hop Server on Docker, Compose, Kubernetes or OpenShift, see xref:hop-server/deploy-project-image.adoc[Project in a Docker image].
