...
* *Timed window*: set *Max consume duration (ms)* when the topic never goes idle (a low continuous trickle) but you still want a scheduled run to process for a fixed time and then finish.
