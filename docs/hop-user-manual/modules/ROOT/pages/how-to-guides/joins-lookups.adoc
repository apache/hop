...
**example***: you receive a daily file that needs to be processed. Compare today's file with yesterday's file, so you can ignore the identical data and only process the changes. That pattern is snapshot xref:how-to-guides/cdc-snapshot-comparison.adoc[Change Data Capture].
...
Contrary to joins, most lookups work on the current pipeline stream and never change the number of rows you're processing.

NOTE: xref:pipeline/transforms/fuzzymatch.adoc[Fuzzy match] is an exception when *Match mode* is set to *All matches (separate rows)*: one main stream row can produce multiple output rows (Top-K).
