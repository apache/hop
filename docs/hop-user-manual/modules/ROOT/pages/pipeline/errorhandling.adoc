...
To find *which rows, columns and values* a database would reject on insert, put xref:pipeline/transforms/databasevaluevalidation.adoc[Database value validation] in front of Table Output (or a bulk loader) and send its error hop to a reject file or table.
