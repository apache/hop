...
- **Truncate table**: (Y/N) Empty the target table before loading. This is the master switch for truncate.
- **Only when rows are received**: (Y/N) Only used when **Truncate table** is checked. If checked, truncate only when at least one input row arrives. If unchecked, always truncate (including when the input is empty).
