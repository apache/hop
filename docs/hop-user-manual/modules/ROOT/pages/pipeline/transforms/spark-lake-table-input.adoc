...
This transform is **not** available on the local Hop engine or on Beam engines. Connector JARs are optional and must be installed as described in xref:pipeline/spark/lakehouse.adoc[Lakehouse tables on the native Spark engine].
