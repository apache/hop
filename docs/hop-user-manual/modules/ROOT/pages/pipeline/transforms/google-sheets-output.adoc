...
|Time out (minutes) |lets you specify an https timeout (in minutes, defaults to 5).
|Retry attempts|Number of retry attempts for retryable Google Sheets API errors (HTTP 429/500/503). Defaults to 3.
|Retry delay (seconds)|Initial delay in seconds between retries for retryable Google Sheets API errors. Defaults to 2 seconds and uses exponential backoff for subsequent retries.
