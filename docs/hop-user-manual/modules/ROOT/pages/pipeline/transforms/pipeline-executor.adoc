...
:page-engines: Hop Engine=yes, Single Threaded=yes, Native Spark=yes, Beam Spark=no, Beam Flink=maybe, Beam Dataflow=maybe
...
*Parameters*: Can map sub pipeline parameters to fields in your current pipeline. Only the parameters defined on the Parameters tab are set per data input row. The "Pass parent values to matching parameters" option decides what happens to the child's other parameters: with it on they are seeded from this pipeline's value of the same name, with it off they keep their own default. The child inherits this pipeline's variables either way.
...
|Stop after (milliseconds)|Maximum time to wait for the child pipeline to complete. Empty or 0 means wait indefinitely. When the time is reached the child pipeline is stopped and the execution-results stream reports a failure (`ExecutionResult=N`, `ExecutionNrErrors>=1`). The value may contain a variable.
...
TIP: The child pipeline always inherits the variables defined in this pipeline. "Pass parent values to matching parameters" (checked by default) additionally seeds a parameter the child declares from this pipeline's value of the same name; unchecking it makes the child keep its own defaults.
