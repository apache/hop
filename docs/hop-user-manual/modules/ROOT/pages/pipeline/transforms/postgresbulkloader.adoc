...
COPY error messages rarely name the stream field and value. Put xref:pipeline/transforms/databasevaluevalidation.adoc[Database value validation] in front of this transform to catch over-long strings, encoding problems, numeric overflow and invalid UUID/JSON before COPY runs.
