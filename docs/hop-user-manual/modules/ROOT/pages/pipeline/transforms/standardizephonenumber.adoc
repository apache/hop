...
* NATIONAL
* RFC3966
|Number type|The result field indicates the type of a processed phone number, such as fixed line, mobile... Empty or unparseable numbers are set to `ERROR`.
|Is valid|The result field indicates whether a phone number is supposed to be valid. Empty or unparseable numbers are `false`.
|===
...
TIP: Output, number type, and is-valid field names may contain variables (for example `+${OUTPUT_FIELD}+`).

TIP: If a number cannot be parsed, the original value is written to the output field, the number type is `ERROR`, and is-valid is `false`. Numbers that parse but are not valid (for example a too-short national number) are still formatted; use the is-valid field to filter them.