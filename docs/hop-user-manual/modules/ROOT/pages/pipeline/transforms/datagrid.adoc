...
|===
|Meta tab|Field metadata (name, type and format masks) used to parse the strings on the Data tab.
See <<meta-tab>>.
|Data Tab|The grid of values.
Every cell is a string.
Hop converts each cell using the Type and Format of that column on the Meta tab.
|===
...
|Type|Hop data type.
Each Data-tab cell in this column is parsed into this type.
See xref:concepts.adoc#data-types[Data types].
|===

TIP: The Data tab has one column per Meta-tab field and one row per output row.
That is how you specify *several different values*.
Do not try to put comma-separated values in a single cell; that is one string.
