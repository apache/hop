...
|User that created the pipeline|Only filled in when Hop knew who created the pipeline, such as an authenticated Hop web session. `-` otherwise. | `User created`

|Date when the pipeline was created|The date the pipeline was first created. | `Date created`

|User that modified the pipeline last|Only filled in when Hop knew who saved the pipeline last, such as an authenticated Hop web session. `-` otherwise. | `User modified`

|Date when the pipeline was modified last|The date of the last save that changed the pipeline. | `Date modified`
