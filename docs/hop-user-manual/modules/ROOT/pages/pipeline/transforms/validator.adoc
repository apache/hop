...
:page-engines: Hop Engine=yes, Single Threaded=yes, Native Spark=yes, Beam Spark=no, Beam Flink=no, Beam Dataflow=no
...
To check values against a live database table (column lengths, types, encoding) instead of hand-authored rules, use xref:pipeline/transforms/databasevaluevalidation.adoc[Database value validation].
...
|When enabled, the failed field and row values are left out of the validation error messages and shown as `?` instead. Use this to keep sensitive values out of the log and out of the error stream. Error rows are still sent to the error handling hop.
...
|Grouping symbol
|The expected thousands grouping symbol, not a row group.
