...
All rows can be specified either from a field (`F` column) or as a static value (`Static value` column). Toggle between field or static value with the `Value in Field?` option.
