...
Do not use Y to hide daylight-saving gaps — see xref:pipeline/formatting-values.adoc#string-to-date[String to Date and Date to String].
...
Leave blank to use the JVM default (`TimeZone.getDefault()`).
Set this to `UTC` when the string is a timezone-less UTC timestamp, or include `Z` / `X` in the Format mask.
...
|Grouping|Thousands grouping symbol (`.` or `,`).
