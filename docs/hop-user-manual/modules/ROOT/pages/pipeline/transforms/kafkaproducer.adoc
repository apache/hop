...
A Kafka producer transform publishes a stream of records to one Kafka topic, or to a topic read per row from a field in the incoming stream.
...
Disabled when the topic is read from a field.

|Get topic from field
|Read the topic from a field in the incoming stream instead of publishing every row to a fixed topic.
This lets a single transform route rows across topics, rather than needing one Kafka producer transform per topic.

|Topic field
|The field holding the topic name for each row, used when *Get topic from field* is enabled.
Rows where this field is empty cause the transform to fail.

|Headers field
|Optional.
A field holding the record headers to attach to each message, as a JSON array of `{"name":..,"value":..}` objects — the same shape the Kafka Consumer transform produces, so headers survive a consume-then-produce round trip.
An array is used rather than a JSON object because Kafka keeps headers ordered and allows a name to repeat.
Leave empty to send messages without headers.
