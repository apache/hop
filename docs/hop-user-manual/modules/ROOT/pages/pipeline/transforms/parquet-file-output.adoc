...
== Notes
[options="header"]
Currently, if the _Accept from field from file_ flag is checked the _Include transform copy number_ and _Split into parts and include number_ flags are temporarily left disabled. This functionality will be implemented soon.