...
The Join Rows transform allows you to produce combinations (https://en.wikipedia.org/wiki/Cartesian_product[Cartesian product^]) of all rows in multiple input streams. However, you will need to join on a key. If you want to join input streams without a key, see the "Join rows (cartesian product)" transform.
