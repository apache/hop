...
A Mapping transform allows you to re-use a series of transforms in the form of a "re-usable pipeline" also known as a "Mapping".  In the "Simple Mapping" transform you can specify one xref:pipeline/transforms/mapping-input.adoc[Mapping Input] transform to pass data to and one xref:pipeline/transforms/mapping-output.adoc[Mapping Output] transform to retrieve rows of data from.
...
* Input tab: specify which fields to pass to the xref:pipeline/transforms/mapping-input.adoc[Mapping Input] transform in the mapping.  You can map (hence the name) field names.
