...
. Select one transform, or a chain of transforms connected by hops (`A → B → C`).
. Right-click a selected transform and choose *Create mapping*.
. Save the new mapping pipeline (Mapping input, the selected transforms, Mapping output).
. Hop replaces the selection with a Simple Mapping transform at the location of the first selected transform and rewires the surrounding hops.

The selection must be a single path with at most one incoming hop and one outgoing hop. Splits, joins, extra info hops, and error hops that leave the selection are not supported; use Multi Mapping for those cases. Cancelling the save dialog leaves the original pipeline unchanged.
...
Input and output mappings reuse the same nested fields, so injection keys are prefixed to keep them unique.

* Top-level: `filename`, `runConfiguration`, `INPUT_SOURCE_TRANSFORM`, `INPUT_TARGET_TRANSFORM`, `INPUT_DESCRIPTION`, `INPUT_MAIN_PATH`, `INPUT_RENAME_ON_OUTPUT`, and the matching `OUTPUT_*` keys, plus `PARAMETERS_INHERIT_ALL_VARIABLES`
* Group `INPUT_FIELDS`: `INPUT_SOURCE_FIELD`, `INPUT_TARGET_FIELD`
* Group `OUTPUT_FIELDS`: `OUTPUT_SOURCE_FIELD`, `OUTPUT_TARGET_FIELD`
* Group `PARAMETERS_MAPPINGS`: `PARAMETERS_VARIABLE`, `PARAMETERS_VALUE`
...
For the exact rules that decide which value a parameter ends up with, see xref:variables/parameter-passing.adoc[Passing parameters to a child pipeline or workflow].
