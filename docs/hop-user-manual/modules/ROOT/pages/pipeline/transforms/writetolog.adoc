...
If no fields are specified it will write out all fields to the log. However, parameters and variables must be specified in the Log message section.  

Example to print a parameter or variable in the Log Message section: ``COUNTER: ${COUNTER}``

Set the Log detail level to Basic if you want variables to be printed out.

If no fields exist, you may have to use a “Generate rows” transform to create a dummy row in order to print variables or parameters.
