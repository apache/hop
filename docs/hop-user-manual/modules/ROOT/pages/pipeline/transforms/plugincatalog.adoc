...
|`metadata_fields`|String|JSON array of `{field, xml_key, java_type, password, group, group_key}` objects, one per property.
|===
...
|`property_group`|String|Name of the containing property group for a nested property, empty for top-level properties. This is *nesting*, not the serialization wrapper.
|`property_group_key`|String|The `groupKey` wrapper element a list or map is serialized under, for example `fields` in `<fields><field>…`. Empty when the property has no wrapper.
|===
