...
[NOTE]
====
The transform supports both Neo4j 4.x and 5.x syntax. Use the "Neo4j version" option 
to select the appropriate version. Neo4j 5.x introduced changes to the `neo4j-admin` 
command syntax, and the transform automatically adjusts the generated command accordingly.

The transform also supports running `neo4j-admin` commands inside Docker containers 
...
|Neo4j version|4.x|Select the Neo4j version (4.x or 5.x) to determine the correct command syntax.
Neo4j 5.x introduced changes to the `neo4j-admin import` command syntax, and this option 
ensures the correct command format is used.
