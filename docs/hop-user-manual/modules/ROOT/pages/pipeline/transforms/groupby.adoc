...
When *Include all rows* is enabled, ignored rows are still written to the output and receive the group aggregate values.
|Ignore field name|Name of the boolean field that marks rows to exclude from aggregation.
This field is only used when *Ignore rows for aggregation* is enabled.
