...
|Password a|The password to open a password protected (encrypted) workbook.
Leave this empty for regular files.

Only the two Excel (POI) engines can decrypt a workbook, the Open Office ODS engine will report an error when a password is set.
Note that with the streaming engine an encrypted workbook has to be decrypted as a whole before it can be read, so for those files the streaming engine loses its low memory advantage.
A password on a file that is not encrypted is ignored, which allows a single transform to read a directory holding both protected and regular files.
