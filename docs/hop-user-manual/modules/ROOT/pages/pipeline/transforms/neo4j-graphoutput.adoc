...
* Select all defined labels (default): when updating nodes all the labels from the graph model are selected.
* Use a field value to select a specific label: for the specified node we look at the input field you like and if it matches the value the label is selected.
* Select the first label: for the specified node we only use the first label from the graph model.
|===
