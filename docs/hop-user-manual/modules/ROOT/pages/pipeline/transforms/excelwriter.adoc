...
|`.ods`
|ODFDOM
|LibreOffice Calc compatible; see <<ods-limitations>> below
...
|Format|The *Excel* number/date format to use in the sheet, not Hop's DecimalFormat / SimpleDateFormat mask.
...
Hop field-metadata formats are described in xref:pipeline/formatting-values.adoc[Formatting numbers and dates].
