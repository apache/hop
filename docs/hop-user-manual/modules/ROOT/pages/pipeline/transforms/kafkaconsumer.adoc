...
For example, you can run the parent pipeline on a timed schedule, drain a topic once per hour with stop-when-idle, or abort the sub-pipeline if sensor data exceeds a preset range.
