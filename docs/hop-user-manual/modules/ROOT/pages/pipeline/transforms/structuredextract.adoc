...
|Name|The output field name, and the name the model is asked for. Surrounding spaces are removed, and names that differ only in case count as duplicates.
...
|Allowed values|Optional comma separated list. The model is then constrained to exactly these values, which is the reliable way to classify: severity as `low, medium, high` rather than whatever adjective the model prefers today. An answer outside the list is an error, also on providers that do not enforce the schema.
|===
