...
Use a VFS path; variables in the path (for example `+${PROJECT_HOME}/sql/lookup_records.sql+`) are resolved.
...
|Replace variables|Substitute Hop variables (for example `+${schema}+`) in the SQL before the statement is prepared. Applies to inline SQL and to SQL loaded from a file.
