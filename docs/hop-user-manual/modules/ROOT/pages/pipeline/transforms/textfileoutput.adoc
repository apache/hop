...
Field formats are ignored, so a Boolean is written as `true` or `false`.
...
|Format|Select a format to apply to the field value. Applies to numbers, dates and Booleans.
For a Boolean, the format is the text for true, a slash and the text for false, so `Ja/Nein` writes German words instead of `Y` / `N`.
You can type your own pair, see xref:pipeline/formatting-values.adoc#boolean-own-format[Your own true and false text].
...
|Group|The thousands separator in numbers. This can be a "," (10,000.00) or "." (5.000,00).
This is *not* a row group.
...
|Rounding Type a| When writing numbers to a file you can specify the Rounding type being used by default `Half Even` is used, for more information look at the Rounding Types section.
