...
[NOTE]
====
xref:pipeline/pipeline-sources.adoc[Pipeline source] when *Execute for each row* is off (the default). Enable that option to run the script once per incoming row; hops are then required, and hops into one-shot mode are refused.
====
