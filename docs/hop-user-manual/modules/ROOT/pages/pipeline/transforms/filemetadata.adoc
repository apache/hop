...
|Enclosure candidates|List of enclosure characters to try while detecting the file layout.
...
An enclosure opens a field when it is the first character of the field.
It closes the field when it is followed by the delimiter or the end of the line.
Inside an enclosed field:

* delimiters are part of the value: `"Smith, Bob"` is one field.
* a doubled enclosure is an escaped enclosure: `"BOB ""ROBERT"" SMITH"`.
* any other enclosure is part of the value as well: `"BOB "ROBERT" SMITH"` is one field.

An enclosure in the middle of a field that is not enclosed, such as `5" pipe`, does not open a field, and the line does not fit that enclosure.
The same goes for a line that ends inside an enclosed field: fields spanning several lines are not supported.

=== Numbers

A number column gets the precision of the value with the most decimals, read with the detected decimal symbol.
The suggested mask keeps all of those decimals: `123.58` and `90524.10` give the mask `#.00` and a precision of 2.
