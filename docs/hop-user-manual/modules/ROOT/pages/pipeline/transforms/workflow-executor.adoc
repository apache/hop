...
|Stop after (milliseconds)|Maximum time to wait for the child workflow to complete. Empty or 0 means wait indefinitely. When the time is reached the child workflow is stopped and the execution-results stream reports a failure (`ExecutionResult=N`, `ExecutionNrErrors>=1`). The value may contain a variable.
|===
...
TIP: The child workflow always inherits the variables defined in this pipeline. "Pass parent values to matching parameters" (checked by default) additionally seeds a parameter the child declares from this pipeline's value of the same name; unchecking it makes the child keep its own defaults.
