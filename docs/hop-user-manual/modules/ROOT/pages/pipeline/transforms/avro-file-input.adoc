...
[WARNING]
====
Avro file input does not run on the native Spark engine.
It produces an Avro Record field, and the native Spark engine has no Spark data type to carry that field from one transform to the next, so the pipeline is refused before it starts.
