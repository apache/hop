...
[NOTE]
====
xref:pipeline/pipeline-sources.adoc[Pipeline source] when reading files. Enable *Source is from a previous transform* to read JSON from an incoming field; hops are then required, and hops into file mode are refused.
====

[NOTE]
====
Use JSON Input when each column should come from its *own JsonPath* (or you are not *exploding* a list into many rows). For an *array of records* where each element becomes a row and *nested objects are flattened* into dotted column names, use xref:pipeline/transforms/jsonnormalizeinput.adoc[JSON Normalize Input] instead.
====
...
For Hop types of Date and Timestamp, use the Format column to specify the incoming format. E.g. for Date: yyyy-MM-dd. You can either select a format from the dropdown or enter format text directly.
See xref:pipeline/formatting-values.adoc[Formatting numbers and dates].
...
A function is applied to the value the Path points to. When the Path points to a single value, the function produces a single value and therefore a single output row, even when that value is itself a list, as with `keys()`.

When the Path contains a wildcard before the function, the function is applied to every match and produces one row per match. For example, `$.orders[*].lines.length()` returns the number of lines of each order. To aggregate over all matches instead, use a deep scan or pass the Path as a parameter to the function: `$..price.sum()` or `$.sum($.orders[*].price)`. Note that `$.orders[*].price.sum()` does not work: the sum is applied to each single price.

Combining a single-value function field with a field whose Path matches several elements is not supported: the transform reports a structure error because the two Paths do not yield the same number of rows. Read such a value in a dedicated JSON Input transform when the other fields return multiple rows.

When a function cannot be evaluated, for example `sum()` or `first()` on an empty array, the value is treated as missing: it is empty when *Ignore missing path* is enabled, otherwise the transform fails with the reason.
