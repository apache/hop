...
The Apache Spark Runner can be used to execute Beam pipelines using Apache Spark 3.x.
...
Beam's upstream documentation historically pins the Spark runner to the 3.2.x line. Hop ships Beam's `beam-runners-spark-3` artifact and measures empirical compatibility with Spark 3.2–3.5 via the `integration-tests/spark` suite and `integration-tests/scripts/run-spark-matrix.sh`. Prefer the xref:pipeline/beam/getting-started-with-beam.adoc#supportedversions[supported versions table] (and the matrix report) over assumptions.
