...
*Measured support (Hop 2.19 / Beam 2.74, client submit):* *Spark 3.5.x* (validated 3.5.7). Hop ships Spark client libraries at the beam engine `spark.version` (currently 3.5.7); the cluster minor must match for `spark://master` client mode. Spark 3.2–3.4 failed the integration matrix on Java 21 and/or client/cluster version skew — see `integration-tests/spark/MATRIX-REPORT.md` and re-run with `integration-tests/scripts/run-spark-matrix.sh`.

Beam's upstream documentation may still pin the Spark runner to an older line (historically 3.2.x). Prefer the xref:pipeline/beam/getting-started-with-beam.adoc#supportedversions[supported versions table] and the matrix report over assumptions.
