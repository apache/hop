...
The name is looked up in the metadata this client sends along first, then in the server's.
...
Extra configuration files, JDBC drivers and plugins are *not* sent in the ZIP.
See xref:hop-server/deploy-export-resources.adoc[Remote run with export resources] for the full list of limitations and when to put the project on the server instead.
