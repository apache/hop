...
The search box at the top of this dialog matches transform names, descriptions and keywords.
Search for `pipeline source` to list transforms that can start without incoming hops (see xref:pipeline/pipeline-sources.adoc[Pipeline sources]).
...
|Create mapping|Extract the selected sequential transforms into a mapping pipeline and replace them with a Simple Mapping transform
...
|Preview & debug output| Run the pipeline to preview rows from this transform and optionally pause when a breakpoint condition matches.
...
|Status| Draft or production status. Deprecated: nothing in Hop acts on this value and it will be removed in a future release.
|Version|Description of the version. Deprecated: this label has to be maintained by hand and will be removed in a future release, use the version control system that holds the file instead.
|Created by| The user that created the pipeline. Only recorded when Hop knows who that is, such as an authenticated Hop web session, and shows `-` otherwise.
|Created at|Displays the date and time when the pipeline was created.
|Last modified by| The user that last modified the pipeline, recorded under the same conditions as `Created by`.
|Last modified at|Displays the date and time of the last save that changed the pipeline. A save that does not change anything leaves this untouched.
|Created with|The version of Hop that created the pipeline. Empty for pipelines created before Hop started recording this.
|Last modified with|The version of Hop that last saved a change to the pipeline.
|===
