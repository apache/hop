...
*Spark (measured):* With Hop 2.19 / Beam 2.74 and client-mode submit (`spark://…`), only *Spark 3.5.x* is validated (see `integration-tests/spark/MATRIX-REPORT.md` and `integration-tests/scripts/run-spark-matrix.sh`). Older 3.x lines fail either because Spark cannot start on Java 21 or because Hop ships Spark client libraries at `spark.version` (3.5.7) which do not interoperate with other minors over RPC. Beam upstream docs may still mention 3.2.x; treat that as Beam’s historical line, not Hop’s measured support.
====
