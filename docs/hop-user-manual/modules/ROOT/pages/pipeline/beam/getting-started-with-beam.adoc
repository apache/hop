...
*Spark:* Apache Beam's Spark runner documentation may list a narrower official range (historically 3.2.x) than what Hop tests. The Spark column below is the version Hop aims to exercise; validate current status with the Docker smoke suite (`PROJECT_NAME=spark`) or the multi-version matrix script `integration-tests/scripts/run-spark-matrix.sh`.
====
...
|2.19.0
|2.74.0
|3.2.x–3.5.x (scala 2.12; matrix-tested)
|1.19.x
