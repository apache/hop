...
TIP: You can also set or remove partitioning directly on any hop pointing to the target transform by clicking the hop and selecting **Set partitioning** (or **Remove partitioning**) under the **Data routing** category in the xref:hop-gui/hop-gui-popup-dialog.adoc#_hop_options[hop context dialog].
