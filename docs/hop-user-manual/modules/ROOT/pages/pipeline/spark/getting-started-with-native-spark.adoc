...
----
{
  "variables": [
    {
      "name": "PROJECT_HOME",
      "value": "s3://hop-project/",
      "description": "Project root on object storage for the cluster run"
    },
    {
      "name": "OUTPUT_ROOT",
      "value": "s3://hop-project/output/",
...
* Values such as `s3://hop-project/` only help if workers can actually read/write that location (correct Spark/Hadoop S3A configuration, IAM roles, etc.).
...
|File path
|File, directory, or glob visible to Spark (local path on `local[*]`; cluster URI on submit)
...
|File path
|Output directory/prefix Spark should write to
...
Connector JARs are **not** in the default Hop assembly — install them under `plugins/engines/spark/lib/{delta,iceberg}` (hop-run/GUI) or via `spark-submit --packages`, then **restart Hop**. Full packaging, session conf, PATH vs TABLE, and samples: xref:pipeline/spark/lakehouse.adoc[Lakehouse tables on the native Spark engine].
...
* [ ] Lakehouse (optional): Delta/Iceberg jars installed + Hop restarted; see xref:pipeline/spark/lakehouse.adoc[lakehouse guide]
