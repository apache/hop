...
:description: Open table formats (Delta Lake and Apache Iceberg) on the Apache Hop native Spark pipeline engine — packaging, session configuration, Lake Table transforms, catalogs, MERGE, and maintenance.
:toc:
:toclevels: 3
...
| Catalog tables | Identifier mode **TABLE** + xref:metadata-types/spark-catalog.adoc[Spark Catalog] metadata (Iceberg-first) |
...
*Not in v1:* Hudi, automated ASF-shipped connector zip packs, default assembly embedding of Delta/Iceberg JARs.

== Version pins

| Component | Coordinate / version |
|-----------|----------------------|
| Spark (Hop native engine) | `spark-sql_2.13` *4.1.2* (Scala *2.13*) |
...
== Install connectors (required)

Default Hop assemblies **do not** ship Delta or Iceberg JARs (size and license review). You install connectors yourself onto the **Spark engine plugin classloader** or the Spark submit classpath.

IMPORTANT: A sibling plugin folder (for example `plugins/engines/spark-delta`) is **not** enough. `SparkPipelineEngine` builds `SparkSession` under the engine plugin classloader, which only loads `plugins/engines/spark/lib` (recursive) plus folders from `plugins/engines/spark/dependencies.xml`.

=== hop-run / Hop Gui (`local[*]`)

. Resolve connector JARs (Maven Central, or the Maven profile below).
. Copy them under your Hop install:
+
----
$HOP_HOME/plugins/engines/spark/lib/delta/     # Delta jars
$HOP_HOME/plugins/engines/spark/lib/iceberg/  # Iceberg runtime jar
----
. **Restart Hop** (or hop-run). Plugin classloaders and jar caches do not hot-reload new files.
. Do **not** re-copy Jackson core/databind/annotations, Spark, Scala library, or slf4j bindings — Hop and the existing Spark lib already provide them.

From a Hop source checkout you can stage jars for copy:

[source,bash]
----
./mvnw -pl plugins/engines/spark -Plakehouse generate-test-resources
# jars appear under plugins/engines/spark/target/lakehouse-connectors/
----

==== Frozen jar list (manual overlay)

Re-run the `lakehouse` profile after pin changes and refresh this list if coordinates change.
...
Prefer `spark-submit --packages` on the cluster so Maven resolves the full tree.

=== spark-submit (cluster)

Use a **native-provided** fat jar (Hop only — do not embed Spark) and add connectors with `--packages` or site jars under `$SPARK_HOME/jars`:
...
If connectors are missing, Hop fails with an actionable `HopException` that repeats the install / `--packages` recipe (`SparkLakeConnectorProbe`).
...
|**PATH**
|Directory / table root Spark can open (`file:///…`, `s3a://…`, …)
...
| **Iceberg** | SQL over `hop_iceberg.\`uri\`` | `writeTo(hop_iceberg.\`uri\`).using("iceberg")` |

| Format | TABLE read | TABLE write |
|--------|------------|-------------|
| **Iceberg** (primary) | `SELECT * FROM catalog.ns.table` | `writeTo(id).using("iceberg")` |
...
|Jars under `plugins/engines/spark/lib/{delta,iceberg}`? Hop restarted? Cluster `--packages` / site jars?
...
* Sample walkthroughs: `integration-tests/spark-native/lakehouse/README.md` in the Hop source tree
