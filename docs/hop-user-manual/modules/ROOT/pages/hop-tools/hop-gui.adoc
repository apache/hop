...
[source,bash]
----
hop gui -j samples -f '${PROJECT_HOME}/transforms/add-a-checksum.hpl'
...
`--project-locations` and `--in-memory` are parsed by the `hop` command and are not available on `hop-gui.sh` / `hop-gui.bat`.
...
|`-f`|`--file`|The filename of the workflow or pipeline to open. Relative paths are resolved against the current directory, then `+${PROJECT_HOME}+`.
