...
use the generated install spec at the installation root:
...
* *Install spec* — edit a Hop install spec (`hop-env.yaml` / JSON) and validate or apply it
...
image::marketplace/marketplace-dialog-environment.png[Marketplace dialog Install spec tab,75%,align="left"]
...
. *Save As…* `+${PROJECT_HOME}/hop-env.yaml+` (default folder is the active project home)
...
* *Open…* — open a `hop-env.yaml` / `.yml` / `.json` file (Hop VFS, project variables allowed)
...
* *Install spec file* — path to a `hop-env.yaml` (variables allowed, for example under the project home). This is a Hop install spec, not a lifecycle environment config file.
...
| `-f`, `--file`
| Path to the install spec file. If omitted, Hop discovers a file (see <<hop-env-discovery>>).
...
* *0* — install spec matches (prints `OK: install spec matches …`)
...
| `repositories`
| No
| List of Maven base URLs used when *applying* this file. Order: first = primary, rest = fallbacks. Each item: `id`, `url`, optional `username` / `password`. A repository here only inherits configured credentials when it points at a repository manager that is already configured — see <<marketplace-spec-repository-credentials>>.
...
[[marketplace-spec-repository-credentials]]
=== Repositories declared by an install spec

An install spec carried by a project can declare its own `repositories`. Because that URL comes from the project rather than from the operator, the configured credentials are only reused for it when the project points at the *same* repository manager they belong to — the same scheme, host and port as a repository in `hop-config`. The path may differ, so a second repository on the same Nexus or Artifactory keeps working.

A spec repository on any other host gets only what the spec itself declares. Neither the `username` / `password` stored on a configured repository nor the global `HOP_MARKETPLACE_USERNAME` / `HOP_MARKETPLACE_PASSWORD` pair is sent to it, so enabling a project cannot make Hop hand the operator's repository credentials to an address of the project's choosing. The log says so at basic level when credentials are withheld this way.

To give such a repository credentials, set `username` / `password` on its entry in the spec (prefer a variable over a literal secret), or set the repository-scoped `HOP_MARKETPLACE_<ID>_USERNAME` / `_PASSWORD` variables for the `id` the spec uses — those name one repository, so they still apply.
...
|`auto` (default)|Basic when a username and a password both resolve, nothing otherwise|Nexus, Forgejo, anonymous public repositories
|`none`|Nothing, ever|A public repository that must ignore globally configured credentials
|`basic`|`Authorization: Basic`|Any repository manager that accepts a username and password
|`token`|`Authorization: Bearer`|Repositories issuing standalone access tokens, such as JFrog Artifactory
|===
...
[source,json]
----
{ "id": "artifactory", "url": "https://acme.jfrog.io/artifactory/hop-plugins/", "authType": "token", "password": "${ARTIFACTORY_TOKEN}" }
----
...
./hop marketplace repo add --id artifactory \
  ...
  --auth-type token --password '${ARTIFACTORY_TOKEN}'
----

When a request comes back `401` or `403`, the error names the type that was used and, if nothing was sent, why not — an `authType` whose credentials do not resolve reports that instead of silently falling back to anonymous.
...
|`jfrog`|`/api/search/aql` or `/api/storage`|JFrog Artifactory
|===
...
* When credentials resolve, it uses *AQL* (`POST /api/search/aql`), which finds every plugin zip in the repository in one request. AQL requires an authenticated user, so it is not attempted anonymously.
* Otherwise — and whenever AQL is refused, for a token without that permission or an instance where it is disabled — it *walks the repository* with `+GET /api/storage/{repo}/{path}+`, one request per folder, stopping at each folder that holds a zip. This works for any user with read access, anonymous included.
