...
Place naming-scheme JSON files in `<output>/metadata/naming-scheme/` before running the command (or create them in the target project in Hop Gui). The unique *Hop metadata* scheme, or a unique *General* scheme, is applied to imported relational connection names. Use `--naming-scheme` to pick one by name.
