...
Hop writes each of these columns with the JDBC call the driver expects for it, so a transform such as xref:pipeline/transforms/tableoutput.adoc[Table Output] needs nothing configured for national text to survive the round trip.

To choose the right call Hop has to know the column type, which the rows on their way into a table do not carry -- a string is a string whether the column is `VARCHAR2` or `NVARCHAR2`.
It reads the column types from the target table once, while the `INSERT` is being prepared.
That costs one extra round trip per prepared statement, not per row, and it means the target table has to exist and be readable by the connection's user before the write starts.

TIP: If a national column still arrives mangled, check that the table Hop is writing to is the one it read the types from.
A synonym or a view over a table in another schema can resolve differently for the write and for the metadata read; writing to the table directly, or granting the connection's user access to it, settles it.
