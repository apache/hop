...
----
Unable to find service with name 'test'
org.apache.hop.core.exception.HopException : Unable to find service with name 'test'
  at org.apache.hop.rest.v1.resources.ExecutionResource.executeSynchronously(ExecutionResource.java:76)
----
...
|Example output
|The name of the element that was saved:
...
|Arguments
|`typeClassName` : the class name of the plugin type, as returned by <<List all the plugin type classes>>. Any other class name is rejected.
...
|Produces
|The content type configured on the xref:hop-server/web-service.adoc[Web Service] metadata element, `text/plain` when that option is left empty.

|Example call
|`curl -X POST  http://localhost:8080/hop/api/v1/execute/sync/  -H 'Content-Type: application/json'  -d '{ "service" : "test", "runConfig" : "local", "variables" : { "VAR1" : "value1", "VAR2" : "value2" }, "bodyContent" : "This is body content" }'`

|Example output
|The response body is the concatenation of the values of the output field, for every row written by the transform configured in the Web Service metadata element.
A pipeline writing a single JSON document in that field produces output like this:
...
NOTE: only synchronous xref:hop-server/web-service.adoc[Web Service] elements can be executed here.
The xref:hop-server/async-web-service.adoc[Asynchronous Web Service] endpoints `hop/asyncRun` and `hop/asyncStatus` are served by xref:hop-server/index.adoc[Hop Server], not by this application.
...
|Arguments
|`locationName` : the name of the execution information location to register the execution with.
...
|Example call
|`curl -X POST  http://localhost:8080/hop/api/v1/location/executions/local/  -H 'Content-Type: application/json'  -d '{ "id" : "df84cbc2-0166-4dea-956f-72b73cf66d0d", "name" : "pipeline-name", "filename" : "/path/filename.hpl", "executionType" : "Pipeline", "parentId" : null, "runConfigurationName" : "local", "registrationDate" : 1678191016156, "executionStartDate" : 1678191016156 }'`
