...
|===
|Variable | Default | Description and Example Use Cases
...
*Example use case*: Dynamically load external configuration or data files from the same directory as the workflow.
|${Internal.Workflow.Filename.Name} | N | The name of the current workflow file (without extension). Helpful when creating log files or reports based on the workflow name.
*Example use case*: Automatically name log files or result outputs based on the workflow name for easy identification.
|${Internal.Workflow.Name} | N | The name of the workflow as defined in the Hop GUI. Useful when you want to display or log the workflow name dynamically.
*Example use case*: Include the workflow name in logging information or in reports generated during the execution.
|${Internal.Pipeline.Filename.Folder} | N | The full directory path where the current pipeline (.hpl) file is stored. Useful for referencing file paths dynamically within the same folder as the pipeline.
*Example use case*: Loading or saving files to the pipeline’s folder for easy file management.
|${Internal.Pipeline.Filename.Name} | N | The name of the current pipeline file (without extension). This is useful for logging or dynamically naming outputs based on the pipeline.
*Example use case*: Name output files (like CSV or XML) based on the pipeline’s name to easily associate them with the correct pipeline.
|${Internal.Pipeline.Name} | N | The name of the pipeline as defined in the Hop GUI. Useful for including the pipeline name in logs or reports.
*Example use case*: Use in log entries to trace which pipeline produced specific outputs or errors.
|${Internal.Transform.CopyNr} | N | The current copy number of a transform when multiple copies of it are running in parallel. Helps differentiate between the different parallel instances.
*Example use case*: In a parallel pipeline setup, use this variable to split the workload or log actions for each transform copy. This ensures you can track and manage each parallel instance independently, such as when processing large datasets or distributing the task across multiple threads.
|===
