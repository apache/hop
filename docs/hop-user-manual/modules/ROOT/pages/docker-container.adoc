...
under the License.
////
:description: This page describes the standard Apache Hop docker image for long (Hop Server) and short-lived (hop-run) containers.
...
OpenJDK version 21 is then used to execute Apache Hop.
...
The project is only created when ```HOP_PROJECT_FOLDER``` is set as well.
A name the image already knows, such as `default` or `samples`, is not created again, so ```HOP_PROJECT_FOLDER``` is ignored for it: give your project its own name.
Without ```HOP_PROJECT_FOLDER``` the container runs the `default` project of its configuration.
...
|
| Comma separated list of JDBC drivers to download and install on container start, before Hop is launched.
Each entry is a driver id (run `hop driver list` to see them), optionally followed by `:version`, for example `oracle,mariadb:3.4.1,mysql`.
See <<Downloading JDBC drivers>>.

|```HOP_DRIVERS_ACCEPT_LICENSE```
|
| Set to `true` to accept the vendor license of restricted (Category X) drivers such as Oracle, MySQL, MariaDB and DB2.
Required to download those drivers, since Apache Hop does not distribute them.

|```HOP_DRIVERS_MAVEN_REPO```
|
| Optional Maven repository base URL to download the drivers from (defaults to Maven Central).
Point this at an internal Nexus or Artifactory for air-gapped environments.
...
|```HOP_COMMAND```
|
| This is the Hop sub-command to execute like for example `python`, `arrow`, `encrypt`, or `doc`.

| ```HOP_COMMAND_PARAMETERS```
|
| The parameters (options) to specify for the Hop command you want to execute, for example the host and port options for the Python and Arrow servers.
...
| `0`
| The maximum number of seconds to wait for running pipelines and workflows to finish when the server shuts down (for example on a `SIGTERM` from the container runtime). `0` shuts down immediately.
...
=== Running the short-lived container

The most common use case will be that you run a **short-lived container** to just complete one Hop workflow or pipeline.
If CI already baked the project into the image, this is often all you need in production: a scheduler starts the container, hop-run finishes, the container exits -- no long-lived Hop Server.
See xref:hop-server/deploy-project-image.adoc#skip_hop_server_short_lived_containers[Skip Hop Server: short-lived containers] and xref:how-to-guides/scheduling-workflows-and-pipelines.adoc[Scheduling workflows and pipelines].
...
docker run -it --rm \
  ...
  --env HOP_PROJECT_NAME=mounted-samples \
...
For more information on the long-lived container please also see the xref:hop-server/index.adoc[Hop Server] documentation as it describes what can be done using the Hop Server.

To ship a git-hosted project *with* the image (CI builds `FROM apache/hop`, copies the project to `/your-project`, then rolls the container) see xref:hop-server/deploy-project-image.adoc[Project in a Docker image].
...
If you want to use the web-services functionality additional information on how to configure your webserver can be found on the user manual  xref:hop-server/web-service.adoc[Web Service] page. For this to work properly the `HOP_SERVER_METADATA_FOLDER` variable has to be set too.

When started can then access the hop-server UI from your host at `http://0.0.0.0:8181` or `http://localhost:8181`

=== Running a Hop command

You can run any Hop command with the variables `HOP_COMMAND` and `HOP_COMMAND_PARAMETERS`.  Here is an example of running the Python Hop server:

[source,bash]
----
docker run \
  -p 25333:25333 \
  -v /home/hop/projects/demo:/demo \
  -e HOP_PROJECT_FOLDER=/demo \
  -e HOP_PROJECT_NAME=demo \
  -e HOP_COMMAND=python \
  -e HOP_COMMAND_PARAMETERS="--gateway-ip-address 0.0.0.0 --gateway-port 25333" \
  apache/hop:<tag>
----

In this example, a project is created in the container and exposed in the Hop Python server.  This gives you access to project metadata or similarly environment configurations.
...
The container can do this for you on start-up. Set `HOP_DRIVERS_DOWNLOAD` to a comma separated list of driver ids and the entrypoint runs `hop driver install` for each one *before* Hop is launched, so the drivers are picked up by the normal `lib/jdbc` scan. Each entry is a driver id, optionally followed by `:version`. Run `hop driver list` (inside the container or a local Hop install) to see the available ids.

Because the restricted drivers are downloaded from the vendor / Maven Central under the vendor's license, you must explicitly accept that license by setting `HOP_DRIVERS_ACCEPT_LICENSE=true`. Without it, restricted drivers are skipped with an error. Apache Hop neither hosts nor redistributes these drivers; obtaining them this way is your action under the vendor's terms.

.Download the Oracle and MariaDB drivers in a short-lived container
[source,shell]
----
docker run -it --rm \
  --env HOP_DRIVERS_DOWNLOAD="oracle,mariadb:3.4.1" \
  --env HOP_DRIVERS_ACCEPT_LICENSE=true \
...
For air-gapped environments, point the download at an internal Maven mirror with `HOP_DRIVERS_MAVEN_REPO`:

[source,shell]
----
docker run -it --rm \
  --env HOP_DRIVERS_DOWNLOAD="oracle" \
  --env HOP_DRIVERS_ACCEPT_LICENSE=true \
  --env HOP_DRIVERS_MAVEN_REPO=https://nexus.internal/repository/maven-public/ \
  ... \
  apache/hop
----

The download is supported by the client/server image (running pipelines, workflows or `hop-server`) and the Hop web image. The drivers and their transitive dependencies are written to `HOP_SHARED_JDBC_FOLDERS` (default `lib/jdbc`). If the container filesystem is read-only, make sure that folder is writable, for example by mounting a volume over it.
