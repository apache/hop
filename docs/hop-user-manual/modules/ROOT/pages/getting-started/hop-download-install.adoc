...
Hop official java version is 21. Check the https://java.com/en/download/help/download_options.html[Java docs] to download and install a Java runtime for your operating system.
...
Recommended next step: point Hop at a user-level config folder so upgrades keep your projects and settings. In Hop Gui use *Tools → Configure Hop environment...*, or from the install directory run `./hop setup apply --defaults` (then restart). See xref:hop-tools/hop-setup.adoc[Hop setup].
