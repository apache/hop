...
Start with a xref:pipeline/pipeline-sources.adoc[pipeline source] (a transform that does not need incoming hops).
Search for `pipeline source` in this dialog, or pick Generate Rows.
