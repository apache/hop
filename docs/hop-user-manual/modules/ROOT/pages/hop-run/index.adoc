...
|```-au```
|```--allow-unsupported```
a|Bypass the engine-compatibility gate and run pipelines or workflows that contain transforms or actions marked UNSUPPORTED on the selected engine.

Strict by default — when omitted, Hop Run refuses to start such a run and prints the list of incompatible elements. This flag is the CLI equivalent of the "Run anyway" button in the GUI run dialog. Under the hood it sets the `HOP_ALLOW_UNSUPPORTED=Y` variable on the execution, which propagates to every nested child pipeline / workflow so the override applies to the full call tree.
...
|```-pl```
|```--project-locations```
|Comma-separated project locations to register for this process only (`name=/path` or a zip/jar). See xref:projects/advanced.adoc#_use_a_project_without_hop_config_json[Use a project without hop-config.json].
...
See xref:projects/advanced.adoc#_use_a_project_without_hop_config_json[Use a project without hop-config.json] for the location and environment formats.
...
See xref:how-to-guides/scheduling-workflows-and-pipelines.adoc[Scheduling workflows and pipelines] for cron, `at`, systemd, Windows Task Scheduler, Jenkins, Kubernetes CronJob and Apache Airflow.
