...
|Stop after (milliseconds)|Maximum time to wait for the pipeline to complete. Empty or 0 means wait indefinitely. When the time is reached the pipeline is stopped and this action fails. Only used when wait-to-finish is enabled. The value may contain a variable.
...
*Pass params downstream*: On the Parameters tab, tick `Pass parent values to matching parameters`. The parameter must already exist in the pipeline (in pipeline properties for example) or alternatively, on the Parameters tab, you can specify new parameters.
A value you fill in here overrides the sub-pipeline's own default for this execution. Leaving the value empty does *not* blank the parameter out: an empty value means nothing was passed, so the sub-pipeline's own default applies. The exact rules are in xref:variables/parameter-passing.adoc[Passing parameters to a child pipeline or workflow].
...
|Pass parent values to matching parameters|Seed a parameter the sub-pipeline declares, but that is not listed above, from this workflow's value of the same name. With the option off the sub-pipeline keeps its own default. Variables are inherited by the sub-pipeline either way.
