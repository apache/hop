...
The selected value is stored with the workflow (XML tag `valueimaplist`, for example `imaplistunread`). Reopening the action keeps the same Retrieve option.
|Retrieve the first...emails|specify the number of messages to retrieve
|After retrieved a|specify the action to perform on email messages after retrieval:
...
* IMAP **Retrieve** (unread, new, flagged, and the other list options) is stored with the workflow. Saving and reopening keeps the selected option.
* Workflows saved in Hop 2.13 still load: date conditions such as `ignore` and IMAP list codes such as `imaplistunread` are accepted, not only integer indexes.
