...
|Arguments table a|Specify the strings to use as arguments for the shell script.

The **Hidden (Y/N)** column controls logging only: when set to `Y`, the argument is replaced by `***` in the Shell action log. The script still receives the real value.
|===
