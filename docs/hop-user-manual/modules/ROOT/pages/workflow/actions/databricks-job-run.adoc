...
|Deploy & run
|Uploads fat jar, pipeline `.hpl`, and exported metadata JSON to **DBFS base directory**, then creates or updates a JAR job (`org.apache.hop.spark.run.MainSpark`) on an **existing cluster** and runs it.
|===
...
|Deploy mode: local fat jar (prefer `hop-conf --generate-fat-jar … --spark-client-version=native-provided`), pipeline path, and Native Spark run configuration name present in the metadata export

|DBFS base directory
|Upload root, default `dbfs:/FileStore/hop` (files written as `hop-native.jar`, `pipeline.hpl`, `metadata.json`)

|Existing cluster ID
|Databricks cluster that already has a compatible Spark runtime

|Job name / Update existing job
|Name for create; if **Update existing job** is checked, **Job ID** is reset with the new settings
|===
...
* Large jar uploads use the DBFS create / add-block / close API and can take several minutes.
