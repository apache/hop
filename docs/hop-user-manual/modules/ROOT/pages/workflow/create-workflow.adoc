...
|Created by| The user that created the workflow. Only recorded when Hop knows who that is, such as an authenticated Hop web session, and shows `-` otherwise.
|Created at|Displays the date and time when the workflow was created.
|Last modified by| The user that last modified the workflow, recorded under the same conditions as `Created by`.
|Last modified at|Displays the date and time of the last save that changed the workflow. A save that does not change anything leaves this untouched.
|Created with|The version of Hop that created the workflow. Empty for workflows created before Hop started recording this.
|Last modified with|The version of Hop that last saved a change to the workflow.
|===
