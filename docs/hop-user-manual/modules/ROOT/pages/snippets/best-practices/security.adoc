...
==== AI Assistant: secrets, personal information, and minimum send

The xref:hop-gui/perspective-ai-advisor.adoc[AI Assistant] sends the open pipeline or workflow context to a language-model service.
Anything hard-coded in that file (or in its logs and check results) can leave your machine.

Set up the project *before* you enable AI:

* Never hard-code secrets or personal information in pipelines, workflows, metadata, notes, SQL, or sample rows.
* Put values in xref:projects/projects-environments.adoc[environment] configuration files as variables. Keep those files outside the project repository.
* Prefer a keystore or vault, referenced with a xref:metadata-types/variable-resolver/index.adoc[variable resolver].
* Name secret variables so they contain `PASSWORD`, `SECRET`, `TOKEN` or `PASSWD`. Hop Gui masks those in the `Ctrl-Space` picker, and the assistant redacts matching XML/JSON keys.
* Keep personal records (names, emails, national ids, row samples) out of notes, Data Grids and log messages.

On each question, use the session *context inclusion* checkboxes to send only what is needed: check results, plugin catalog, execution log, full XML.
That cuts token cost and the chance of sharing sensitive data.
See xref:hop-gui/perspective-ai-advisor.adoc#context-inclusions[Context inclusions].
...
The JavaScript, Script, and other coding transforms are powerful but can execute arbitrary code, potentially leading to
code injection or data exfiltration risks.

[WARNING]
====
Scripting transforms can execute arbitrary code. If you're reading data from untrusted sources or allowing user input
to influence script logic, you risk code injection attacks, data exfiltration, or unintended side effects on your system.
====
