...
* Never hard-code secrets. Put them in those environment files as variables, or better in a keystore read with a xref:metadata-types/variable-resolver/index.adoc[variable resolver]. Name secret variables with `PASSWORD`, `SECRET` or `TOKEN` so they are recognizable. See xref:../snippets/best-practices/security.adoc[Security].
