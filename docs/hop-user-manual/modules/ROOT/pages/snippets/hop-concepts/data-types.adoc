...
Hop binds a vector to a prepared statement using its text form, which a PostgreSQL `vector` column (provided by the pgvector extension) accepts directly.
No dialect currently emits a native vector column type when Hop generates DDL, so a generated `CREATE TABLE` produces a text column that has to be adjusted by hand.
