...
/**
 * Registers pipeline executions for drill-down support when the GUI is active. Skips registration
...
@ExtensionPoint(
    ...
    description = "Registers pipeline executions for drill-down support when GUI is active")
public class DrillDownRegistrationExtensionPoint
  ...
  public void callExtensionPoint(
      ...
      throws HopException {
    if (pipeline == null || !"GUI".equalsIgnoreCase(Const.getHopPlatformRuntime())) {
      return;
    }
    DrillDownGuiPlugin.registerRunningPipeline(pipeline.getLogChannelId(), pipeline);
