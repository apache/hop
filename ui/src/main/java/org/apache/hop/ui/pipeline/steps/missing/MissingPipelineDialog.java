 ...
 *
 ******************************************************************************/
package org.apache.hop.ui.pipeline.steps.missing;
...
public class MissingPipelineDialog extends BaseStepDialog implements StepDialogInterface {
  ...
  public static final int MISSING_PIPELINE_STEPS = 1;
  public static final int MISSING_PIPELINE_STEP_ID = 2;

  public MissingPipelineDialog( Shell parent, List<Missing> missingPipeline, StepMetaInterface baseStepMeta,
                                PipelineMeta pipelineMeta, String stepname ) {
    super( parent, baseStepMeta, pipelineMeta, stepname );
    this.shellParent = parent;
    this.missingPipeline = missingPipeline;
    this.mode = MISSING_PIPELINE_STEPS;
  }

  public MissingPipelineDialog( Shell parent, Object in, PipelineMeta pipelineMeta, String stepname ) {
    super( parent, (BaseStepMeta) in, pipelineMeta, stepname );
    this.shellParent = parent;
    this.mode = MISSING_PIPELINE_STEP_ID;
  }

  private String getErrorMessage( List<Missing> missingPipeline, int mode ) {
    String message = "";
    if ( mode == MISSING_PIPELINE_STEPS ) {
      ...
      message = BaseMessages.getString( PKG, "MissingPipelineDialog.MissingPipelineSteps", entries.toString() );
    }

    if ( mode == MISSING_PIPELINE_STEP_ID ) {
      message =
        BaseMessages.getString( PKG, "MissingPipelineDialog.MissingPipelineStepId", stepname + " - "
          + ( (Missing) baseStepMeta ).getMissingPluginId() );
    }
  ...
  public String open() {
    ...
    if ( this.mode == MISSING_PIPELINE_STEPS ) {
