...
public class TransGraph extends AbstractGraph implements XulEventHandler, Redrawable, TabItemInterface,
  ...
  public TransGraph( Composite parent, final HopUi hopUi, final TransMeta transMeta ) {
    ...
    transGridDelegate = new TransGridDelegate( hopUi, this );
    transHistoryDelegate = new TransHistoryDelegate( hopUi, this );
    transPerfDelegate = new TransPerfDelegate( hopUi, this );
    transMetricsDelegate = new TransMetricsDelegate( hopUi, this );
    transPreviewDelegate = new TransPreviewDelegate( hopUi, this );
    ...
    try {
      ...
      HopUiPluginManager.getInstance().applyPluginsForContainer( "trans-graph", xulDomContainer );
        ...
        try {
          ...
          switch ( container.getType() ) {
            ...
            case DragAndDropContainer.TYPE_BASE_STEP_TYPE:
              ...
              stepMeta = hopUi.newStep( transMeta, id, name, name, false, true );
              if ( stepMeta != null ) {
                newstep = true;
              } else {
            ...
            case DragAndDropContainer.TYPE_DATABASE_CONNECTION:
              ...
              if ( hopUi.editStep( transMeta, stepMeta ) != null ) {
          ...
          if ( newstep ) {
            hopUi.addUndoNew( transMeta, new StepMeta[] { stepMeta }, new int[] { transMeta.indexOfStep( stepMeta ) } );
          } else {
            hopUi.addUndoChange( transMeta, new StepMeta[] { before }, new StepMeta[] { (StepMeta) stepMeta.clone() },
          ...
          //
          if ( newstep && transMeta.nrSteps() > 1 && transMeta.nrSteps() < 5 && hopUi.props.isShowingHelpToolTips() ) {
          ...
          for ( AreaOwner areaOwner : areaOwners ) {
            if ( areaOwner.contains( real.x, real.y ) ) {
              if ( areaOwner.getParent() instanceof StepMeta
                ...
                hopUi.editPartitioning( transMeta, step );
                hit = true;
                break;
              }
            }
  ...
  public void mouseDown( MouseEvent e ) {
    ...
    try {
      ExtensionPointHandler.callExtensionPoint( LogChannel.GENERAL, HopExtensionPoint.TransGraphMouseDown.id,
        new TransGraphExtension( this, e, real ) );
    } catch ( Exception ex ) {
      ...
      if ( areaOwner != null && areaOwner.getAreaType() != null ) {
        switch ( areaOwner.getAreaType() ) {
          ...
          case HOP_ERROR_ICON:
            ...
            hopUi.editStepErrorHandling( transMeta, stepMeta );
            break;
        ...
        if ( hop != null ) {
          ...
          hopUi.addUndoChange( transMeta, new TransHopMeta[] { before }, new TransHopMeta[] { after },
    ...
    if ( !hopUi.props.getAutoSplit() ) {
      MessageDialogWithToggle md =
        new MessageDialogWithToggle( shell, BaseMessages.getString( PKG, "TransGraph.Dialog.SplitHop.Title" ), null,
          ...
          "TransGraph.Dialog.Option.SplitHop.DoNotAskAgain" ), hopUi.props.getAutoSplit() );
    ...
    if ( ( id & 0xFF ) == 0 ) { // Means: "Yes" button clicked!
      ...
      if ( !caExists && !bcExists ) {
        ...
        if ( transMeta.findTransHop( newhop1 ) == null ) {
          ...
          hopUi.addUndoNew( transMeta, new TransHopMeta[] { newhop1, },
        ...
        if ( transMeta.findTransHop( newhop2 ) == null ) {
          ...
          hopUi.addUndoNew( transMeta, new TransHopMeta[] { newhop2 },
            new int[] { transMeta.indexOfTransHop( newhop2 ) }, true );
        }
        int idx = transMeta.indexOfTransHop( hi );
        hopUi.addUndoDelete( transMeta, new TransHopMeta[] { hi }, new int[] { idx }, true );
  ...
  public void mouseMove( MouseEvent e ) {
    ...
    try {
      ...
      ExtensionPointHandler.callExtensionPoint(
        LogChannel.GENERAL, HopExtensionPoint.TransGraphMouseMoved.id, ext );
  ...
  protected void addHop( StreamInterface stream ) {
    switch ( stream.getStreamType() ) {
      case ERROR:
        ...
        hopUi.newHop( transMeta, candidate );
        break;
      case INPUT:
        hopUi.newHop( transMeta, candidate );
        break;
  ...
  public void checkSelectedSteps() {
    hopUi.checkTrans( transMeta, true );
  }
  ...
  public void clustering() {
    ...
    if ( selected != null && selected.size() > 0 ) {
      hopUi.editClustering( transMeta, transMeta.getSelectedSteps() );
    } else {
      hopUi.editClustering( transMeta, getCurrentStep() );
    }
  }

  public void errorHandling() {
    hopUi.editStepErrorHandling( transMeta, getCurrentStep() );
  }
  ...
  public void dupeStep() {
    try {
      ...
      if ( steps.size() <= 1 ) {
        hopUi.dupeStep( transMeta, getCurrentStep() );
      } else {
        for ( StepMeta stepMeta : steps ) {
          hopUi.dupeStep( transMeta, stepMeta );
        }
      }
    } catch ( Exception ex ) {
  ...
  public void copyStep() {
    hopUi.copySelected( transMeta, transMeta.getSelectedSteps(), transMeta.getSelectedNotes() );
  }
  ...
  public void enableDisableHopsDownstream( boolean enabled ) {
    ...
    hopUi.addUndoChange( transMeta, new TransHopMeta[] { before }, new TransHopMeta[] { after }, new int[] { transMeta
  ...
  public void deleteNote() {
    ...
    if ( idx >= 0 ) {
      ...
      hopUi.addUndoDelete( transMeta, new NotePadMeta[] { (NotePadMeta) ni.clone() }, new int[] { idx } );
      redraw();
    }
  }
  ...
  public void newNote() {
    ...
    if ( n != null ) {
      ...
      hopUi.addUndoNew( transMeta, new NotePadMeta[] { npi }, new int[] { transMeta.indexOfNote( npi ) } );
      redraw();
    }
  }
  ...
  public void newStep( String description ) {
    StepMeta stepMeta = hopUi.newStep( transMeta, description, description, false, true );
  ...
  protected void initializeXulMenu( Document doc, List<StepMeta> selection, StepMeta stepMeta ) throws HopException {
    ...
    item.setDisabled( hopUi.getPartitionSchemasNames( transMeta ).isEmpty() );
  ...
  public void delSelected( StepMeta stMeta ) {
    List<StepMeta> selection = transMeta.getSelectedSteps();
    if ( selection.size() == 0 ) {
      hopUi.delStep( transMeta, stMeta );
      return;
    }
    ...
    hopUi.delSteps( transMeta, steps );
  }
  ...
  private void editStep( StepMeta stepMeta ) {
    hopUi.editStep( transMeta, stepMeta );
  }
  ...
  private void editNote( NotePadMeta ni ) {
    ...
    if ( n != null ) {
      ...
      hopUi.addUndoChange( transMeta, new NotePadMeta[] { before }, new NotePadMeta[] { after }, new int[] { transMeta
        .indexOfNote( ni ) } );
      hopUi.refreshGraph();
    }
  }
  ...
  private void editHop( TransHopMeta transHopMeta ) {
    ...
    hopUi.editHop( transMeta, transHopMeta );
  }
  ...
  private SnapAllignDistribute createSnapAllignDistribute() {
    ...
    return new SnapAllignDistribute( transMeta, selection, indices, hopUi, this );
  }
  ...
  private void detach( StepMeta stepMeta ) {
    ...
    for ( int i = transMeta.nrTransHops() - 1; i >= 0; i-- ) {
      ...
      if ( stepMeta.equals( hop.getFromStep() ) || stepMeta.equals( hop.getToStep() ) ) {
        ...
        //
        hopUi.addUndoNew( transMeta, new TransHopMeta[] { hop }, new int[] { i } );
  ...
  public boolean applyChanges() throws HopException {
    return hopUi.saveToFile( transMeta );
  }
 ...
 */
  ...
  public void addAllTabs() {
    ...
    List<HopUiExtenderPluginInterface> relevantExtenders =
      HopUiExtenderPluginType.getInstance().getRelevantExtenders( TransGraph.class, LOAD_TAB );
  ...
  public void openMapping( StepMeta stepMeta, int index ) {
    try {
      ...
      if ( subTrans != null
        ...
        ExtensionPointHandler.callExtensionPoint( log, HopExtensionPoint.OpenMapping.id, objectArray );
      } else {
      ...
      if ( referencedMeta instanceof TransMeta ) {
        ...
        hopUi.addTransGraph( mappingMeta );
        TransGraph subTransGraph = hopUi.getActiveTransGraph();
      ...
      if ( hopUi.props.getAutoSave() ) {
        ...
        MessageDialogWithToggle md =
          new MessageDialogWithToggle( shell, BaseMessages.getString( PKG, "TransLog.Dialog.FileHasChanged.Title" ),
            ...
            MessageDialog.QUESTION, new String[] { BaseMessages.getString( PKG, "System.Button.Yes" ),
            BaseMessages.getString( PKG, "System.Button.No" ) }, 0, BaseMessages.getString( PKG,
            "TransLog.Dialog.Option.AutoSaveTransformation" ), hopUi.props.getAutoSave() );
        MessageDialogWithToggle.setDefaultImage( GUIResource.getInstance().getImageHopUi() );
        int answer = md.open();
        if ( ( answer & 0xFF ) == 0 ) {
          hopUi.saveToFile( transMeta );
  ...
  public void addStepToChain( PluginInterface stepPlugin, boolean shift ) {
    ...
    StepMeta newStep =
      hopUi.newStep( transMeta, stepPlugin.getIds()[ 0 ], stepPlugin.getName(), stepPlugin.getName(), false, true );
    if ( newStep == null ) {
      return;
    }
