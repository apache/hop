...
package org.apache.hop.ui.job.entries.pipeline;
...
/**
 * This dialog allows you to edit the pipeline job entry (JobEntryPipeline)
 *
 * @author Matt
 * @since 19-06-2003
 */
public class JobEntryPipelineDialog extends JobEntryBaseDialog implements IJobEntryDialog {
  ...
  private void getParameters( PipelineMeta inputPipelineMeta ) {
    try {
      if ( inputPipelineMeta == null ) {
        JobEntryPipeline jet = new JobEntryPipeline();
        getInfo( jet );
        inputPipelineMeta = jet.getPipelineMeta( metaStore, jobMeta );
      }
      ...
      new ErrorDialog( shell,
        BaseMessages.getString( PKG, "JobEntryPipelineDialog.Exception.UnableToLoadPipeline.Title" ),
        BaseMessages.getString( PKG, "JobEntryPipelineDialog.Exception.UnableToLoadPipeline.Message" ), e );
    }
  ...
  protected void pickFileVFS() {
    ...
    try {
      parentFolder = HopVFS.getFilename( HopVFS.getFileObject( jobMeta.environmentSubstitute( jobMeta.getFilename() ) ).getParent() );
    } catch ( Exception e ) {
      // not that important
    }
  ...
  private void getInfo( JobEntryPipeline jet ) throws HopException {
