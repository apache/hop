...
package org.apache.hop.ui.repository.filerep;
...
public class HopFileRepositoryDialog implements RepositoryDialogInterface {
  ...
  private HopFileRepositoryMeta input;
  // private RepositoriesMeta repositories;
  private RepositoriesMeta masterRepositoriesMeta;

  private String masterRepositoryName;

  public HopFileRepositoryDialog( Shell parent, int style, RepositoryMeta repositoryMeta,
    RepositoriesMeta repositoriesMeta ) {
    this.display = parent.getDisplay();
    this.props = PropsUI.getInstance();
    this.input = (HopFileRepositoryMeta) repositoryMeta;
    ...
    shell.setText( BaseMessages.getString( PKG, "HopFileRepositoryDialog.Dialog.Main.Title" ) );
  }

  public HopFileRepositoryMeta open( final MODE mode ) {
    this.mode = mode;
    props.setLook( shell );
    ...
    wlBaseDir.setText( BaseMessages.getString( PKG, "HopFileRepositoryDialog.Label.BaseDirectory" ) );
    ...
    wlReadOnly.setText( BaseMessages.getString( PKG, "HopFileRepositoryDialog.Label.ReadOnly" ) );
    ...
    wlHidesHiddenFiles
      .setText( BaseMessages.getString( PKG, "HopFileRepositoryDialog.Label.HidesHiddenFiles" ) );
    ...
    wlName.setText( BaseMessages.getString( PKG, "HopFileRepositoryDialog.Label.Name" ) );
    ...
    wlId.setText( BaseMessages.getString( PKG, "HopFileRepositoryDialog.Label.Description" ) );
    props.setLook( wlId );
    fdlId = new FormData();
  ...
  private void getInfo( HopFileRepositoryMeta info ) {
    ...
    if ( input.getBaseDirectory() != null && input.getBaseDirectory().length() > 0 ) {
      if ( input.getName() != null && input.getName().length() > 0 ) {
        if ( input.getDescription() != null && input.getDescription().length() > 0 ) {
          ...
          box.setMessage( BaseMessages.getString( PKG, "HopFileRepositoryDialog.Dialog.ErrorNoName.Message" ) );
          box.setText( BaseMessages.getString( PKG, "HopFileRepositoryDialog.Dialog.ErrorNoName.Title" ) );
          box.open();
        }
      } else {
        MessageBox box = new MessageBox( shell, SWT.ICON_ERROR | SWT.OK );
        box.setMessage( BaseMessages.getString( PKG, "HopFileRepositoryDialog.Dialog.ErrorNoId.Message" ) );
        box.setText( BaseMessages.getString( PKG, "HopFileRepositoryDialog.Dialog.ErrorNoId.Title" ) );
        box.open();
      }
    } else {
      MessageBox box = new MessageBox( shell, SWT.ICON_ERROR | SWT.OK );
      box.setMessage( BaseMessages.getString( PKG, "HopFileRepositoryDialog.Dialog.ErrorNoBaseDir.Message" ) );
      box.setText( BaseMessages.getString( PKG, "HopFileRepositoryDialog.Dialog.ErrorNoBaseDir.Title" ) );
      box.open();
    }
  }
