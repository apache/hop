...
import org.apache.hop.ui.repository.dialog.RepositoryDialogInterface.MODE;
import org.apache.hop.ui.repository.model.RepositoriesModel;
...
public class RepositoriesHelper {
    ...
    try {
      try {
        ...
        if ( input.getErrorMessage() != null && input.getErrorMessage().length() > 0 ) {
          throw new HopException( input.getErrorMessage() );
        }
  ...
  public void loginToRepository() throws HopException, HopSecurityException {
    if ( model != null && model.getSelectedRepository() != null ) {
      ...
      throw new HopException( BaseMessages.getString( PKG, "RepositoryLogin.ErrorLoginToRepository" ) );
    }
  }
