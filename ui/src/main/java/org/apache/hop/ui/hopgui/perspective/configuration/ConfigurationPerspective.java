...
public class ConfigurationPerspective implements IHopPerspective {
  ...
  private TreeItem pluginsTreeItem;
  ...
  public void initialize(HopGui hopGui, Composite parent) {
    ...
    searchBox = new Text(composite, SWT.SEARCH | SWT.ICON_SEARCH | SWT.ICON_CANCEL | SWT.BORDER);
    PropsUi.setLook(searchBox);
    searchBox.setMessage("Search settings...");
              ...
              if (parentItem != null) {
                ...
                if (pluginTab != null) {
                  ...
                  if (tabControl instanceof Composite) {
                    Composite pluginComposite = findPluginComposite((Composite) tabControl);
    ...
    // Set sash weights (20% tree, 80% content)
    ...
    if (tabsList != null) {
      ...
      for (CTabItem tab : allTabs) {
        ...
        if (categoryName.equalsIgnoreCase("Plugins") || categoryName.contains("plugin")) {
          ...
          pluginsTreeItem = new TreeItem(categoryTree, SWT.NONE);
    ...
    for (TreeItem item : categoryTree.getItems()) {
      ...
      if (tab != null && !tab.isDisposed()) {
        ...
        if (control instanceof Composite) {
          List<Control> matches = searchInComposite((Composite) control, lowerSearch);
          if (!matches.isEmpty()) {
  ...
  private void highlightControl(Control control, Color highlightColor) {
    ...
    if (!originalBackgrounds.containsKey(control)) {
      originalBackgrounds.put(control, control.getBackground());
    }

    // Store original font if not already stored
    if (!originalFonts.containsKey(control) && control.getFont() != null) {
      originalFonts.put(control, control.getFont());
    }
  ...
  private Composite findPluginComposite(Composite parent) {
    ...
    for (Control child : parent.getChildren()) {
      if (child instanceof Composite composite) {
        ...
        Composite found = findPluginComposite(composite);
        if (found != null) {
          return found;
        }
      }
