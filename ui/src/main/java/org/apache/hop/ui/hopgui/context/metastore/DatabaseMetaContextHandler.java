...
public class DatabaseMetaContextHandler implements IGuiContextHandler {
  ...
  @Override public List<IGuiAction> getSupportedActions() {
    ...
    IGuiAction newAction = new GuiAction( "create-database-meta", GuiActionType.Create, "Create Database Connection", "Create a new database connection", "ui/images/CNC.svg",
      (parameters) -> {
        hopGui.databaseMetaManager.newMetadata();
      });
    actions.add(newAction);

    IGuiAction editAction = new GuiAction( "edit-database-meta", GuiActionType.Modify, "Edit Database Connection", "Edit a database connection", "ui/images/CNC.svg",
