...
public class BaseGuiWidgets {
  ...
  /**
   * Every set of widgets (toolbar, composite, menu, ...) gets its own unique ID
   * It will cause a new object to be created per unique ID for the listener or GUI plugins if this plugin wasn't registered yet.
   */
  ...
  /**
   * Let the GUI plugin system know that there is no need to instantiate new objects for the given class.
   * Instead this object can be taken.
   * Make sure to call dispose() to prevent a (slow) memory leak.
   * Call this method before creating the widgets themselves.
   *
