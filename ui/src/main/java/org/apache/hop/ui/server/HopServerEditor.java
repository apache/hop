...
public class HopServerEditor extends MetadataEditor<HopServer> {
  ...
  public void test() {
    ...
    try {
      String xml = "<sample/>";
      String reply = server.sendXml( xml, RegisterPipelineServlet.CONTEXT_PATH );

      String message =
        BaseMessages.getString( PKG, "HopServer.Replay.Info1" )
          + server.constructUrl( RegisterPipelineServlet.CONTEXT_PATH ) + Const.CR
      ...
      EnterTextDialog dialog =
        new EnterTextDialog(getShell(), "XML", BaseMessages.getString( PKG, "HopServer.RetournedXMLInfo" ), message );
      dialog.open();
    } catch ( Exception e ) {
      new ErrorDialog( getShell(), BaseMessages.getString( PKG, "HopServer.ExceptionError" ), BaseMessages.getString(
