...
public class HopUiSlave extends Composite implements TabItemInterface {
  ...
  public HopUiSlave( Composite parent, int style, final HopUi hopUi, SlaveServer slaveServer ) {
    super( parent, style );
    ...
    Rectangle bounds = hopUi.tabfolder.getSwtTabset().getBounds();
    ...
    if ( item.getData( "transStatus" ) != null ) {
      ...
      try {
        ...
        SlaveServerTransStatus ts =
          HopUiSlave.this.slaveServer.getTransStatus(
        ...
        if ( log.isDetailed() ) {
          log.logDetailed( "Finished receiving transformation status for [{0}] from server [{1}]", transStatus
            .getTransName(), HopUiSlave.this.slaveServer );
        }
  ...
  private void engageViewAndLogUpdateTimer() {
    if ( timer == null ) {
      timer = new Timer( "HopUiSlave: " + getMeta().getName() );
    }
