...
public class TransformsMetricsMeta extends BaseTransformMeta implements TransformMetaInterface {
  ...
  public void getFields( RowMetaInterface r, String name, RowMetaInterface[] info, TransformMeta nextTransform,
    ...
    r.clear();
    String transformName = space.environmentSubstitute( transformnamefield );
    if ( !Utils.isEmpty( transformName ) ) {
      ValueMetaInterface v = new ValueMetaString( transformName );
      v.setOrigin( name );
      r.addValueMeta( v );
    }
    String transformid = space.environmentSubstitute( transformidfield );
    if ( !Utils.isEmpty( transformid ) ) {
      ValueMetaInterface v = new ValueMetaString( transformid );
      v.setOrigin( name );
      v.setLength( ValueMetaInterface.DEFAULT_INTEGER_LENGTH, 0 );
      r.addValueMeta( v );
    }
    String transformlinesinput = space.environmentSubstitute( transformlinesinputfield );
    if ( !Utils.isEmpty( transformlinesinput ) ) {
      ValueMetaInterface v = new ValueMetaInteger( transformlinesinput );
      v.setOrigin( name );
      v.setLength( ValueMetaInterface.DEFAULT_INTEGER_LENGTH, 0 );
      r.addValueMeta( v );
    }
    String transformlinesoutput = space.environmentSubstitute( transformlinesoutputfield );
    if ( !Utils.isEmpty( transformlinesoutput ) ) {
      ValueMetaInterface v = new ValueMetaInteger( transformlinesoutput );
      v.setOrigin( name );
      v.setLength( ValueMetaInterface.DEFAULT_INTEGER_LENGTH, 0 );
      r.addValueMeta( v );
    }
    String transformlinesread = space.environmentSubstitute( transformlinesreadfield );
    if ( !Utils.isEmpty( transformlinesread ) ) {
      ValueMetaInterface v = new ValueMetaInteger( transformlinesread );
      v.setOrigin( name );
      v.setLength( ValueMetaInterface.DEFAULT_INTEGER_LENGTH, 0 );
      r.addValueMeta( v );
    }
    String transformlinesupdated = space.environmentSubstitute( transformlinesupdatedfield );
    if ( !Utils.isEmpty( transformlinesupdated ) ) {
      ValueMetaInterface v = new ValueMetaInteger( transformlinesupdated );
      v.setOrigin( name );
      v.setLength( ValueMetaInterface.DEFAULT_INTEGER_LENGTH, 0 );
      r.addValueMeta( v );
    }
    String transformlineswritten = space.environmentSubstitute( transformlineswrittentfield );
    if ( !Utils.isEmpty( transformlineswritten ) ) {
      ValueMetaInterface v = new ValueMetaInteger( transformlineswritten );
      v.setOrigin( name );
      v.setLength( ValueMetaInterface.DEFAULT_INTEGER_LENGTH, 0 );
      r.addValueMeta( v );
    }
    String transformlineserrors = space.environmentSubstitute( transformlineserrorsfield );
    if ( !Utils.isEmpty( transformlineserrors ) ) {
      ValueMetaInterface v = new ValueMetaInteger( transformlineserrors );
      v.setOrigin( name );
      v.setLength( ValueMetaInterface.DEFAULT_INTEGER_LENGTH, 0 );
      r.addValueMeta( v );
    }
    String transformseconds = space.environmentSubstitute( transformsecondsfield );
    if ( !Utils.isEmpty( transformseconds ) ) {
      ValueMetaInterface v = new ValueMetaInteger( transformseconds );
      v.setOrigin( name );
      r.addValueMeta( v );
    }
  ...
  public TransformInterface getTransform( TransformMeta transformMeta, TransformDataInterface transformDataInterface, int cnr, PipelineMeta tr,
                                Pipeline pipeline ) {
    return new TransformsMetrics( transformMeta, transformDataInterface, cnr, tr, pipeline );
  }
