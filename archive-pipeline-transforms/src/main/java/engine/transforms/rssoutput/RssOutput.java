...
public class RssOutput extends BaseTransform implements ITransform {
  ...
  public boolean processRow( TransformMetaInterface smi, ITransformData sdi ) throws HopException {
    ...
    if ( first ) {
      ...
      if ( meta.isFilenameInField() ) {
        ...
        if ( data.indexOfFieldfilename < 0 ) {
          ...
          throw new HopException( BaseMessages.getString( PKG, "RssOutput.Log.ErrorFindingField", meta
            .getFileNameField() ) );
        }
      ...
      if ( Utils.isEmpty( data.filename ) ) {
        ...
        throw new HopException( BaseMessages.getString( PKG, "RssOutput.Log.FilenameEmpty" ) );
      }
      ...
      if ( meta.isCreateParentFolder() ) {
        ...
        try {
          // Get parent folder
          parentfolder = HopVFS.getFileObject( data.filename, getPipelineMeta() ).getParent();
      ...
      if ( !meta.isCustomRss() ) {
        ...
        if ( meta.AddGeoRSS() ) {
          if ( Utils.isEmpty( meta.getGeoPointLong() ) ) {
            throw new HopException( BaseMessages.getString( PKG, "RssOutput.Log.GeoPointLatEmpty" ) );
          }
          if ( Utils.isEmpty( meta.getGeoPointLong() ) ) {
            throw new HopException( BaseMessages.getString( PKG, "RssOutput.Log.GeoPointLongEmpty" ) );
          }
        ...
        for ( int i = 0; i < meta.getChannelCustomFields().length; i++ ) {
          ...
          if ( data.customchannels[ i ] < 0 ) { // couldn't find field!
            ...
            throw new HopTransformException( BaseMessages.getString( PKG, "RssOutput.Exception.FieldRequired", meta
    ...
    if ( !meta.isCustomRss() ) {
      ...
      if ( !createEntry(
        ...
        itemgeopointy ) ) {
        throw new HopException( "Error adding item to feed" );
      }
    } else {
    ...
    try {
      ...
      logError( BaseMessages.getString( PKG, "RssOutputMeta.Log.ErrorInTransform" ) + e.getMessage() );
      setErrors( 1 );
      stopAll();
  ...
  public String buildFilename() throws HopTransformException {
