 ...
 * <p>
 * Based on Sven Boden's Oracle Bulk Loader transform
 *
 * @author matt
 * @since 14-apr-2009
 */
public class MySQLBulkLoader extends BaseTransform implements ITransform {
  ...
  public boolean execute( MySQLBulkLoaderMeta meta ) throws HopException {
    ...
    try {
      ...
      if ( meta.getDatabaseMeta() == null ) {
        logError( BaseMessages.getString( PKG, "MySQLBulkLoader.Init.ConnectionMissing", getTransformName() ) );
        return false;
      }
  ...
  private void writeRowToBulk( IRowMeta rowMeta, Object[] r ) throws HopException {
    ...
    try {
      ...
      try {
        ...
        throw new HopException( BaseMessages.getString( PKG, "MySQLBulkLoader.Message.ERRORSERIALIZING" ), loadEx );
      }

      // MySQL didn't finish, throw the generic "Pipe" exception.
      throw new HopException( BaseMessages.getString( PKG, "MySQLBulkLoader.Message.ERRORSERIALIZING" ), e );

    } catch ( Exception e2 ) {
      // Null pointer exceptions etc.
      throw new HopException( BaseMessages.getString( PKG, "MySQLBulkLoader.Message.ERRORSERIALIZING" ), e2 );
    }
  }
  ...
  protected void verifyDatabaseConnection() throws HopException {
    ...
    if ( meta.getDatabaseMeta() == null ) {
      throw new HopException( BaseMessages.getString( PKG, "MySQLBulkLoaderMeta.GetSQL.NoConnectionDefined" ) );
    }
  }
