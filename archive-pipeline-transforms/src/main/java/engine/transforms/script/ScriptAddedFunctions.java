...
public class ScriptAddedFunctions {
  ...
  public static Object getTransformationName( ScriptEngine actualContext, Bindings actualObject, Object[] ArgList,
                                              Object FunctionContext ) {
    try {
      Object objTranName = actualObject.get( "_PipelineName_" );
      return objTranName;
    } catch ( Exception e ) {
  ...
  public static void putRow( ScriptEngine actualContext, Bindings actualObject, Object[] ArgList,
    ...
    if ( ArgList.length == 1 ) {
      try {
        ...
        Object scmO = actualObject.get( "_transform_" );
        try {
          Script transform = (Script) scmO;
          transform.putRow( transform.getOutputRowMeta(), newRow );
        } catch ( Exception e ) {
          ScriptDummy transform = (ScriptDummy) scmO;
          transform.putRow( transform.getOutputRowMeta(), newRow );
        }

      } catch ( Exception e ) {
        throw new RuntimeException( "Unable to pass the new row to the next transform(s) because of an error: "
          + Const.CR + e.toString() );
      }
    } else {
