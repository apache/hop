...
@ExtensionPoint(
    id = "PipelineLintTransformPainterExtension",
    description = "Draw lint severity badges on pipeline transform icons",
    extensionPointId = "PipelinePainterTransform")
public class PipelineLintTransformPainterExtension
  ...
  public void callExtensionPoint(
    ...
    if (!LintCanvasOverlayHelper.isEnabled() || ext == null || ext.transformMeta == null) {
      return;
    }

    String filePath = LintPathUtils.normalizePath(ext.pipelineMeta.getFilename());
    if (Utils.isEmpty(filePath)) {
      return;
    }

    Map<String, List<LintResult>> byTransform =
        LintResultsManager.getInstance().getOverlayIndex(filePath, LintSourceRef.Kind.TRANSFORM);
    ...
    if (severity == null) {
      ...
      // than leaving the next reader to wonder whether this transform was checked at all.
      if (LintCanvasOverlayHelper.isShowingIgnoredMarkers()
        ...
        LintCanvasOverlayHelper.drawIgnoredOverlay(
            ext.gc, ext.x1, ext.y1, ext.iconSize, ext.transformMeta.isSelected());
      }
      return;
    }
