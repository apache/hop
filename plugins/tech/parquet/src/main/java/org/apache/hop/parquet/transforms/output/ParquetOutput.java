...
public class ParquetOutput extends BaseTransform<ParquetOutputMeta, ParquetOutputData>
    ...
    private void openNewFile() throws HopException {
        ...
        SchemaBuilder.FieldAssembler<Schema> fieldAssembler = SchemaBuilder.record("ApacheHopParquetSchema")
                .fields();
        ...
        for (int i = 0; i < meta.getFields().size(); i++) {
            ...
            switch (valueMeta.getType()) {
                case IValueMeta.TYPE_DATE:
                    ...
                    fieldAssembler = fieldAssembler.name(field.getTargetFieldName())
                            .type()
                            .unionOf()
                            .nullType().and().type(timestampMilliType)
                            .endUnion()
                            .noDefault();
                    break;
        ...
        try {
            ...
            data.writer =
                    new ParquetWriterBuilder(
                            messageType, data.avroSchema, data.outputFile, data.sourceFieldIndexes, meta.getFields())
