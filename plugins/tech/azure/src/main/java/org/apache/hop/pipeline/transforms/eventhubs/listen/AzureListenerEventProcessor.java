  ...
  public void onEvents( PartitionContext context, Iterable<EventData> events ) throws Exception {
    ...
    for ( EventData data : events ) {
      ...
      try {
        ...
        if (azureTransform.isDebug()) {
          azureTransform.logDebug("Event read and passed for PartitionId (" + context.getPartitionId() + "," + data.getSystemProperties().getOffset() + "," +
        ...
        if ( ( checkpointBatchingCount % checkpointBatchingSize ) == 0 ) {
          if (azureTransform.isDebug()) {
            azureTransform.logDebug( "Partition " + context.getPartitionId() + " checkpointing at " +
