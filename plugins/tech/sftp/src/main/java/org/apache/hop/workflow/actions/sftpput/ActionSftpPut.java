...
public class ActionSftpPut extends ActionBase implements Cloneable, IAction {
  ...
  /**
   * The name of an SFTP connection in the metadata. When it's set, the server settings below are
  ...
  public Result execute(Result result, int nr) throws HopException {
          ...
          if (file != null) {
            if (!file.exists()) {
              logError(
                  BaseMessages.getString(
                      PKG,
                      "ActionSftpPut.Log.FilefromPreviousNotFound",
              ...
              if (isDebug()) {
                logDebug(
                    BaseMessages.getString(
                        PKG, "ActionSftpPut.Log.FilenameFromResult", file.getPublicURIString()));
              }
            }
          }
        }
    ...
    try {
      ...
      if (isUsingConnection()) {
        // Everything about the server lives in the named SFTP connection.
        //
        sftpclient =
            SftpConnections.createClient(
                this, SftpConnections.load(getMetadataProvider(), resolve(connection)));
        if (isDetailed()) {
          logDetailed(
              BaseMessages.getString(
                  PKG, "ActionSftpPut.Log.OpenedNamedConnection", resolve(connection)));
        }
      } else {
        if (isUseKeyFilename()) {
      ...
      while (it.hasNext() && !parentWorkflow.isStopped()) {
        ...
        try {
          String localFilename = myFile.getPublicURIString();
