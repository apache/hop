  ...
  public Map<String, FileProvider> getProviders(IVariables variables) {
    ...
    try {
      ...
      for (WebDavConnection connection : connections) {
        ...
        String root = StringUtils.trimToEmpty(variables.resolve(connection.getRootUrl()));
        if (StringUtils.startsWithIgnoreCase(root, "webdav4s://")) {
          providers.put(name, new HopWebDav4sConnectionFileProvider(variables, connection));
        } else {
          providers.put(name, new HopWebDav4ConnectionFileProvider(variables, connection));
        }
      }
    } catch (Exception e) {
