...
/**
 * The Cypher generated here is not executed by Hop, it is presented in the Neo4j perspective for
 * the user to run in the Neo4j browser. It still has to be valid Cypher.
 */
class HistoryResultTest {

  private HistoryResult createWorkflowWithErrorPath() {
    HistoryResult pipeline = new HistoryResult();
    pipeline.setId("pipeline-id");
    pipeline.setName("load-customers");
    pipeline.setType("PIPELINE");

    HistoryResult transform = new HistoryResult();
    transform.setId("transform-id");
    transform.setName("Table output");
    transform.setType("TRANSFORM");

    HistoryResult workflow = new HistoryResult();
  ...
  @Test
  void executionInfoCommandLooksUpTheSubject() {
    assertTrue(
        createWorkflowWithErrorPath()
            .getExecutionInfoCommand()
            .contains(
                "MATCH(ex:Execution { name : \"main\", type : \"WORKFLOW\", id : \"workflow-id\"})"));
  }
}
