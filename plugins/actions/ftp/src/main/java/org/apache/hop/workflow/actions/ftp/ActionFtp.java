...
@Action(
  id = "FTP",
  i18nPackageName = "org.apache.hop.workflow.actions.ftp",
  name = "ActionFTP.Name",
  description = "ActionFTP.Description",
  image = "FTP.svg",
...
public class ActionFtp extends ActionBase implements Cloneable, IAction {
  private static Class<?> PKG = ActionFtp.class; // for i18n purposes, needed by Translator!!
      ...
      if ( !Utils.isEmpty( socksProxyHost ) ) {
        if ( !Utils.isEmpty( socksProxyPort ) ) {
          ...
          throw new FTPException( BaseMessages.getString(
            PKG, "ActionFTP.SocksProxy.PortMissingException", environmentSubstitute( socksProxyHost ),
            getName() ) );
        }
        ...
        if ( !Utils.isEmpty( socksProxyUsername ) && !Utils.isEmpty( socksProxyPassword ) ) {
          ...
          throw new FTPException( BaseMessages.getString(
            PKG, "ActionFTP.SocksProxy.IncompleteCredentials", environmentSubstitute( socksProxyHost ),
            getName() ) );
        }
      }
  ...
  public void check( List<ICheckResult> remarks, WorkflowMeta workflowMeta, IVariables variables,
    ...
    ActionValidatorUtils.andValidator().validate(
      this, "targetDirectory", remarks, AndValidator.putValidators( ActionValidatorUtils.notBlankValidator(),
