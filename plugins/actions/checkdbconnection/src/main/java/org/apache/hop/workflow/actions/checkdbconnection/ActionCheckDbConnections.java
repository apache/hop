...
@Action( id = "CHECK_DB_CONNECTIONS",
  ...
  documentationUrl = "https://hop.apache.org/manual/latest/plugins/actions/checkdbconnection.html"
)
public class ActionCheckDbConnections extends ActionBase implements Cloneable, IAction {
  ...
  public Result execute( Result previousResult, int nr ) {
    ...
    if ( connections != null ) {
      for ( int i = 0; i < connections.length && !parentWorkflow.isStopped(); i++ ) {
        ...
        try {
          ...
          if ( iMaximumTimeout > 0 ) {
            ...
            while ( continueLoop && !parentWorkflow.isStopped() ) {
              ...
              if ( ( now >= ( timeStart + iMaximumTimeout * multiple ) ) ) {
                ...
                if ( isDetailed() ) {
                  logDetailed( BaseMessages.getString(
                    PKG, "ActionCheckDbConnections.WaitTimeIsElapsed.Label", connections[ i ].getDatabaseName(),
          ...
          nrsuccess++;
          if ( isDetailed() ) {
            logDetailed( BaseMessages.getString( PKG, "ActionCheckDbConnections.ConnectionOK", connections[ i ]
              .getDatabaseName(), connections[ i ].getName() ) );
          }
        } catch ( HopDatabaseException e ) {
          nrerrors++;
          logError( BaseMessages.getString( PKG, "ActionCheckDbConnections.Exception", connections[ i ]
    ...
    if ( isDetailed() ) {
      logDetailed( "=======================================" );
      logDetailed( BaseMessages.getString( PKG, "ActionCheckDbConnections.Log.Info.ConnectionsInError", ""
        + nrerrors ) );
      logDetailed( BaseMessages.getString( PKG, "ActionCheckDbConnections.Log.Info.ConnectionsInSuccess", ""
        + nrsuccess ) );
      logDetailed( "=======================================" );
    }
