...
@Transform(
		...
		i18nPackageName="org.apache.hop.pipeline.transforms.streamschemamerge",
		...
		documentationUrl = "https://hop.apache.org/manual/latest/plugins/transforms/streamschemamerge.html"
)
public class StreamSchemaMeta extends BaseTransformMeta implements ITransformMeta<StreamSchema, StreamSchemaData> {
     ...
     * you try to run the pipeline in safe mode.
     * @return true
     */
	...
	/**
	 * This method is called by Hop Gui when a transform needs to serialize its configuration to XML. The expected
	 ...
	 * 
	 * Please use XmlHandler to conveniently read from the
	 * XML node passed in.
	 * 
    ...
    private void readData( Node transformNode) throws HopXmlException {
        try {
            ...
            for ( int i = 0; i < nrsteps; i++ ) {
                getTransformIOMeta().addStream(
                        new Stream( IStream.StreamType.INFO, null, "Streams to Merge", StreamIcon.INFO, null ) );
            }
	 ...
	 * the transform. This method must apply any changes the transform makes to the row stream. Usually a step adds fields to the
	 * row-stream.
	 * 
	...
	public void check(List<ICheckResult> remarks, PipelineMeta pipelineMeta, TransformMeta transformMeta,
		...
		if (input.length > 0) {
			cr = new CheckResult(CheckResult.TYPE_RESULT_OK, BaseMessages.getString(PKG, "StreamSchemaTransform.CheckResult.ReceivingRows.OK"), transformMeta);
			remarks.add(cr);
		} else {
			cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR, BaseMessages.getString(PKG, "StreamSchemaTransform.CheckResult.ReceivingRows.ERROR"), transformMeta);
			remarks.add(cr);
		}	
...
*/
	 ...
	 */
	@Override
	public ITransform createTransform(TransformMeta transformMeta, StreamSchemaData data, int copyNr, PipelineMeta pipelineMeta, Pipeline pipeline) {
