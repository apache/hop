...
@Transform( id = "TokenReplacementPlugin",
        ...
        description = "BaseTransform.TypeTooltipDesc.TokenReplacement",
        categoryDescription = "i18n:org.apache.hop.pipeline.transform:BaseTransform.Category.Transform",
        i18nPackageName = "i18n:org.apache.hop.pipeline.transforms.tokenreplacement",
        documentationUrl = "https://hop.apache.org/manual/latest/plugins/transforms/tokenreplacement.html"
)
@InjectionSupported( localizationPrefix = "TokenReplacement.Injection.", groups = { "OUTPUT_FIELDS" } )
public class TokenReplacementMeta
        extends BaseTransformMeta implements ITransformMeta<TokenReplacement, TokenReplacementData> {
  ...
  public void loadXml( Node transformNode, IHopMetadataProvider metadataProvider ) throws HopXmlException {
    try {
      ...
      addInputFileNameToResult = "Y".equalsIgnoreCase(
              Const.NVL( XmlHandler.getTagValue( transformNode, ADD_INPUT_FILENAME_TO_RESULT ), "" ) );
      ...
      outputFileEncoding = Const.NVL( XmlHandler.getTagValue( transformNode, OUTPUT_FILE_ENCODING ),
      ...
      includeStepNrInOutputFileName = "Y".equalsIgnoreCase(
              Const.NVL( XmlHandler.getTagValue( transformNode, INCLUDE_STEP_NR_IN_OUTPUT_FILENAME ), "" ) );
      includePartNrInOutputFileName = "Y".equalsIgnoreCase(
              Const.NVL( XmlHandler.getTagValue( transformNode, INCLUDE_PART_NR_IN_OUTPUT_FILENAME ), "" ) );
      includeDateInOutputFileName = "Y".equalsIgnoreCase(
              Const.NVL( XmlHandler.getTagValue( transformNode, INCLUDE_DATE_IN_OUTPUT_FILENAME ), "" ) );
      includeTimeInOutputFileName = "Y".equalsIgnoreCase(
              Const.NVL( XmlHandler.getTagValue( transformNode, INCLUDE_TIME_IN_OUTPUT_FILENAME ), "" ) );
      specifyDateFormatOutputFileName = "Y".equalsIgnoreCase(
              Const.NVL( XmlHandler.getTagValue( transformNode, SPECIFY_DATE_FORMAT_OUTPUT_FILENAME ), "" ) );
      ...
      addOutputFileNameToResult = "Y".equalsIgnoreCase(
              Const.NVL( XmlHandler.getTagValue( transformNode, ADD_OUTPUT_FILENAME_TO_RESULT ), "" ) );
  ...
  @Override
  public ITransform createTransform(TransformMeta transformMeta, TokenReplacementData data, int copyNr, PipelineMeta pipelineMeta, Pipeline pipeline) {
  ...
  public String buildFilename( String filename, IVariables variables, int transformnr, String partnr,
  ...
  public void check(List<ICheckResult> remarks, PipelineMeta pipelineMeta, TransformMeta transformMeta,
    ...
    if ( prev != null && prev.size() > 0 ) {
      cr =
        new CheckResult( ICheckResult.TYPE_RESULT_OK, BaseMessages.getString(
          PKG, "TokenReplacementMeta.CheckResult.FieldsReceived", "" + prev.size() ), transformMeta );
      remarks.add( cr );
      ...
      if ( error_found ) {
        ...
        cr =
          new CheckResult( ICheckResult.TYPE_RESULT_OK, BaseMessages.getString(
            PKG, "TokenReplacementMeta.CheckResult.AllFieldsFound" ), transformMeta );
        remarks.add( cr );
      }
    }
    ...
    if( Utils.isEmpty( tokenStartString ) || Utils.isEmpty( tokenEndString ) )
    {
      cr = new CheckResult( ICheckResult.TYPE_RESULT_ERROR, BaseMessages.getString(
        PKG, "TokenReplacementMeta.CheckResult.ExpectedTokenReplacementError" ), transformMeta );
      remarks.add( cr );
    } else {
      cr = new CheckResult( ICheckResult.TYPE_RESULT_OK, BaseMessages.getString(
        PKG, "TokenReplacementMeta.CheckResult.ExpectedTokenReplacementOk" ), transformMeta );
      remarks.add( cr );
    }
    ...
    if ( input.length > 0 ) {
      cr =
        new CheckResult( ICheckResult.TYPE_RESULT_OK, BaseMessages.getString(
          PKG, "TokenReplacementMeta.CheckResult.ExpectedInputOk" ), transformMeta );
      remarks.add( cr );
    } else {
      cr =
        new CheckResult( ICheckResult.TYPE_RESULT_ERROR, BaseMessages.getString(
          PKG, "TokenReplacementMeta.CheckResult.ExpectedInputError" ), transformMeta );
      remarks.add( cr );
    }

    cr =
      new CheckResult( ICheckResult.TYPE_RESULT_COMMENT, BaseMessages.getString(
        PKG, "TokenReplacementMeta.CheckResult.FilesNotChecked" ), transformMeta );
    remarks.add( cr );
  }
