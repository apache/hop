...
public class RedshiftBulkLoader
   ...
   * last row, run the put and copy commands to copy the data into Redshift.
   *
  ...
  public synchronized boolean processRow() throws HopException {
    ...
    if (row != null && first) {
      ...
      if (meta.isSpecifyFields()
          && meta.getDataType()
              .equals(
                  RedshiftBulkLoaderMeta.DATA_TYPE_CODES[RedshiftBulkLoaderMeta.DATA_TYPE_CSV])) {
        // Get input field mapping
        data.fieldnrs = new HashMap<>();
        getDbFields();
        for (int i = 0; i < meta.getRedshiftBulkLoaderFields().size(); i++) {
          int streamFieldLocation =
              data.outputRowMeta.indexOfValue(
                  meta.getRedshiftBulkLoaderFields().get(i).getStreamField());
          ...
          for (int e = 0; e < data.dbFields.size(); e++) {
            ...
            if (field[0].equalsIgnoreCase(
                meta.getRedshiftBulkLoaderFields().get(i).getTableField())) {
              dbFieldLocation = e;
              break;
            }
          }
          ...
          data.fieldnrs.put(
              meta.getRedshiftBulkLoaderFields().get(i).getTableField().toUpperCase(),
              streamFieldLocation);
        }
      } else if (meta.getDataType()
          .equals(
              RedshiftBulkLoaderMeta.DATA_TYPE_CODES[RedshiftBulkLoaderMeta.DATA_TYPE_JSON])) {
        data.fieldnrs = new HashMap<>();
  ...
  /**
   * Runs the commands to put the data to the Redshift stage, the copy command to load the table,
          ...
          for (int i = 0; i < putRowMeta.getFieldNames().length; i++) {
            ...
            if (putRowMeta.getFieldNames()[i].equalsIgnoreCase("status")
              ...
              throw new HopDatabaseException(
                  "Error putting file to Redshift stage \n"
  ...
  private void writeRowToFile(IRowMeta rowMeta, Object[] row) throws HopTransformException {
    try {
      if (meta.getDataTypeId() == RedshiftBulkLoaderMeta.DATA_TYPE_CSV
      ...
      } else if (meta.getDataTypeId() == RedshiftBulkLoaderMeta.DATA_TYPE_CSV) {
        /*
  ...
  private void openNewFile(String baseFilename) throws HopException {
    if (baseFilename == null) {
      throw new HopFileException(
          BaseMessages.getString(PKG, "RedshiftBulkLoader.Exception.FileNameNotSet"));
    }
  ...
  public void dispose() {
    ...
    if (!Boolean.parseBoolean(resolve(RedshiftBulkLoaderMeta.DEBUG_MODE_VAR))) {
