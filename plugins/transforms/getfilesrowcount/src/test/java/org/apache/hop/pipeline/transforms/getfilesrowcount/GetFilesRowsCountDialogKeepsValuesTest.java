...
/**
 * The filename field of {@link GetFilesRowsCountDialog} is filled with the incoming fields when the
 ...
 */
@Tag("uitest")
class GetFilesRowsCountDialogKeepsValuesTest extends SwtBotTestBase {

  private static final String TRANSFORM_NAME = "getfilesrowcount";
  private static final String TITLE =
      BaseMessages.getString(GetFilesRowsCountMeta.class, "GetFilesRowsCountDialog.DialogTitle");

  @Test
  void filenameFieldSurvivesFailingUpstream() {
    GetFilesRowsCountMeta meta = configured("configured_field");
    ...
    assertEquals(
        ...
        "OK must keep the configured filename field");
  }

  @Test
  void filenameFieldSurvivesUpstreamWithFields() {
    GetFilesRowsCountMeta meta = configured("renamed_field");
  ...
  private void focusFieldAndPressOk(GetFilesRowsCountMeta meta, PipelineMeta pipelineMeta) {
    AtomicReference<GetFilesRowsCountDialog> dialog = new AtomicReference<>();
    withDialog(
        parent -> {
          GetFilesRowsCountDialog d =
              new GetFilesRowsCountDialog(parent, new Variables(), meta, pipelineMeta);
          dialog.set(d);
          d.open();
        },
        bot -> {
  ...
  private static Widget field(GetFilesRowsCountDialog dialog) {
    try {
  ...
  private static GetFilesRowsCountMeta configured(String fieldName) {
    GetFilesRowsCountMeta meta = new GetFilesRowsCountMeta();
    meta.setDefault();
    meta.setFileFromField(true);
    meta.setOutputFilenameField(fieldName);
    return meta;
  }
}
