...
@Transform(
        ...
        documentationUrl = "https://hop.apache.org/manual/latest/plugins/transforms/changefileencoding.html"
)
    ...
    if ( input.length > 0 ) {
      cr =
        new CheckResult( CheckResult.TYPE_RESULT_OK,
          BaseMessages.getString( PKG, "ChangeFileEncodingMeta.CheckResult.ReceivingInfoFromOtherTransforms" ),
          transformMeta );
      remarks.add( cr );
    } else {
      cr =
        new CheckResult( CheckResult.TYPE_RESULT_ERROR,
          BaseMessages.getString( PKG, "ChangeFileEncodingMeta.CheckResult.NoInpuReceived" ), transformMeta );
      remarks.add( cr );
    }
