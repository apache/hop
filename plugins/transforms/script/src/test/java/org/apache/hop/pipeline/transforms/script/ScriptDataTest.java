...
class ScriptDataTest {
  ...
  void defaultConstructor_setsNullFields() {
    ...
    assertNull(data.compiledScript);
    assertNull(data.rawScript);
    assertNull(data.fieldsUsed);
    assertNull(data.valuesUsed);
    assertNull(data.outputRowMeta);
    assertNull(data.replaceIndex);
  }
}
