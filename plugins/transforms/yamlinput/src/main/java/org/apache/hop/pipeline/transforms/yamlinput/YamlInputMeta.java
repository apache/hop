...
@Transform(
        id = "YamlInput",
        image = "yamlinput.svg",
        i18nPackageName = "org.apache.hop.pipeline.transforms.YamlInput",
        name = "YamlInput.Name",
...
public class YamlInputMeta extends BaseTransformMeta implements ITransformMeta<YamlInput,YamlInputData> {
  ...
  @Override
  public ITransform createTransform(TransformMeta transformMeta, YamlInputData data, int copyNr, PipelineMeta pipelineMeta, Pipeline pipeline) {
  ...
  public void check( List<ICheckResult> remarks, PipelineMeta pipelineMeta, TransformMeta transformMeta,
    ...
    if ( input.length <= 0 ) {
      cr =
        new CheckResult( CheckResult.TYPE_RESULT_ERROR, BaseMessages.getString(
          PKG, "YamlInputMeta.CheckResult.NoInputExpected" ), transformMeta );
      remarks.add( cr );
    } else {
      cr =
        new CheckResult( CheckResult.TYPE_RESULT_OK, BaseMessages.getString(
          PKG, "YamlInputMeta.CheckResult.NoInput" ), transformMeta );
      remarks.add( cr );
    }
    ...
    if ( getInputFields().length <= 0 ) {
      cr =
        new CheckResult( CheckResult.TYPE_RESULT_ERROR, BaseMessages.getString(
          PKG, "YamlInputMeta.CheckResult.NoInputField" ), transformMeta );
      remarks.add( cr );
    }
    ...
    if ( isInFields() ) {
      if ( Utils.isEmpty( getYamlField() ) ) {
        cr =
          new CheckResult( CheckResult.TYPE_RESULT_ERROR, BaseMessages.getString(
            PKG, "YamlInputMeta.CheckResult.NoField" ), transformMeta );
        remarks.add( cr );
      } else {
        cr =
          new CheckResult( CheckResult.TYPE_RESULT_OK, BaseMessages.getString(
            PKG, "YamlInputMeta.CheckResult.FieldOk" ), transformMeta );
        remarks.add( cr );
      }
    } else {
      ...
      if ( fileInputList == null || fileInputList.getFiles().size() == 0 ) {
        cr =
          new CheckResult( CheckResult.TYPE_RESULT_ERROR, BaseMessages.getString(
            PKG, "YamlInputMeta.CheckResult.NoFiles" ), transformMeta );
        remarks.add( cr );
      } else {
        cr =
          new CheckResult( CheckResult.TYPE_RESULT_OK, BaseMessages.getString(
            PKG, "YamlInputMeta.CheckResult.FilesOk", "" + fileInputList.getFiles().size() ), transformMeta );
        remarks.add( cr );
      }
    }
  }
