...
public class LdapInputSearchScopeConverter implements IIntCodeConverter {
  ...
  public String getCode(int type) {
    if (type < 0 || type >= LdapInputMeta.searchScopeCode.length) {
      return LdapInputMeta.searchScopeCode[0];
    }
    return LdapInputMeta.searchScopeCode[type];
  }

  @Override
  public int getType(String code) {
    return LdapInputMeta.getSearchScopeByCode(code);
  }
}
