...
public class LdapInput extends BaseTransform<LdapInputMeta, LdapInputData>
    ...
    private boolean dynamicSearch() throws HopException {
        ...
        if (first) {
            ...
            if (meta.isDynamicFilter()) {
                data.indexOfFilterField =
                        getInputRowMeta().indexOfValue(meta.getDynamicFilterFieldName());
    ...
    private Object[] getOneRow() throws HopException {
        ...
        if (data.dynamic) {
            while (data.readRow == null
                    || (data.attributes = data.connection.getAttributes()) == null) {
                ...
                if (!dynamicSearch()) {
                    ...
                    if (log.isDetailed()) {
                        logDetailed(
                                BaseMessages.getString(PKG, "LdapInput.Log.FinishedProcessing"));
                    }
                    return null;
                }
            }
        ...
        try {
            ...
            for (int i = 0; i < meta.getInputFields().length; i++) {
                ...
                if (attr != null) {
                    ...
                    outputRowData[index] =
                            getAttributeValue(field, attr, index, outputRowData[index]);
                }
            ...
            throw new HopException(
                    BaseMessages.getString(PKG, "LdapInput.Exception.CanNotReadLDAP"), e);
        }
    ...
    private void connectServerLdap() throws HopException {
        ...
        // Set the page size?
        if (meta.isPaging()) {
            data.connection.setPagingSize(
                    Const.toInt(environmentSubstitute(meta.getPageSize()), -1));
        }
    }
    ...
    public void dispose() {
        ...
        if (data.connection != null) {
            try {
                ...
                logError(
                        BaseMessages.getString(
                                PKG, "LdapInput.Exception.ErrorDisconecting", e.toString()));
            }
        }
