 ...
 * @since 21-09-2007
 */
public class LdapInput extends BaseTransform<LdapInputMeta, LdapInputData> implements ITransform<LdapInputMeta, LdapInputData> {
  private static Class<?> PKG = LdapInputMeta.class; // for i18n purposes, needed by Translator!!

  public LdapInput( TransformMeta transformMeta, LdapInputMeta meta,  LdapInputData data, int copyNr, PipelineMeta pipelineMeta,
                    Pipeline pipeline ) {
    ...
    if ( first ) {
      ...
      if ( meta.isDynamicSearch() ) {
        ...
        if ( data.indexOfSearchBaseField < 0 ) {
          ...
          throw new HopException( BaseMessages.getString( PKG, "LdapInput.Exception.CouldnotFindField", meta
