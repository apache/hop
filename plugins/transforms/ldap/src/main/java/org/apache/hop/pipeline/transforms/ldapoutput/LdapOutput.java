        ...
        if (first) {
            ...
            if (meta.getOperationType() != LdapOutputMeta.OPERATION_TYPE_DELETE
                ...
                for (int i = 0; i < data.nrFields; i++) {
                    ...
                    data.fieldStream[i] =
                            getInputRowMeta()
                                    .indexOfValue(environmentSubstitute(meta.getUpdateStream()[i]));
            ...
            if (meta.getOperationType() == LdapOutputMeta.OPERATION_TYPE_RENAME) {
                ...
                if (Utils.isEmpty(dnField)) {
                    throw new HopException(
                            BaseMessages.getString(
                                    classFromResourcesPackage, "LdapOutput.Error.DNFieldMissing"));
                }
        ...
        try {
            if (meta.getOperationType() != LdapOutputMeta.OPERATION_TYPE_RENAME) {
                ...
                if (isDebug()) {
                    logDebug(
                            BaseMessages.getString(
                                    classFromResourcesPackage, "LdapOutput.ProcessDn", dn));
                }
                ...
                if (meta.getOperationType() != LdapOutputMeta.OPERATION_TYPE_DELETE) {
                    ...
                    for (int i = 0; i < data.nrFields; i++) {
                        data.attributes[i] =
                                getInputRowMeta().getString(outputRowData, data.fieldStream[i]);
                    }
                }
            }
            switch (meta.getOperationType()) {
                case LdapOutputMeta.OPERATION_TYPE_UPSERT:
                    ...
                    for (int i = 0; i < data.nrFieldsToUpdate; i++) {
                        data.attributesToUpdate[i] =
                                getInputRowMeta()
                                        .getString(outputRowData, data.fieldStreamToUpdate[i]);
                    }
                    int status =
                ...
                case LdapOutputMeta.OPERATION_TYPE_RENAME:
                    String oldDn =
                            getInputRowMeta().getString(outputRowData, data.indexOfOldDNField);
                    String newDn =
                            getInputRowMeta().getString(outputRowData, data.indexOfNewDNField);
                ...
                default:
                    ...
                    data.connection.insert(
                            dn, data.fieldsAttribute, data.attributes, data.separator);
                    incrementLinesOutput();
                    break;
            }
    ...
    public boolean init() {
        if (super.init()) {
            try {
                ...
                logError(
                        BaseMessages.getString(
                                classFromResourcesPackage, "LdapOutput.Error.Init", e));
                stopAll();
                setErrors(1);
                return false;
            }
        ...
        if (data.connection != null) {
            try {
                ...
                logError(
                        BaseMessages.getString(
                                classFromResourcesPackage,
                                "LdapOutput.Exception.ErrorDisconecting",
                                e.toString()));
            }
        }
