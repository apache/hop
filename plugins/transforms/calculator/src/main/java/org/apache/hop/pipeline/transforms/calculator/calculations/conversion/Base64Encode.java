...
public class Base64Encode implements ICalculation {
  ...
  public CalculationOutput calculate(CalculationInput in) throws HopValueException {
    ...
    if (in.dataA != null) {
      byte[] bytes =
          (in.metaA.getType() == IValueMeta.TYPE_BINARY)
              ? in.metaA.getBinary(in.dataA)
              : in.metaA.getString(in.dataA).getBytes();

      value = Base64.getEncoder().withoutPadding().encodeToString(bytes);
    }
