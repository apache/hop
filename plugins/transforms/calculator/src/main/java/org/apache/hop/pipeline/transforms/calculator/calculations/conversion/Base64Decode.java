...
public class Base64Decode implements ICalculation {
  ...
  public CalculationOutput calculate(CalculationInput in) throws HopValueException {
    ...
    if (in.dataA != null) {
      ...
      if (in.targetMeta.getType() == IValueMeta.TYPE_BINARY) {
        value = tmpDecoded;
      } else {
        String tmpDecodedString = new String(tmpDecoded);
        value = in.targetMeta.convertData(in.metaA, tmpDecodedString);
      }
    }
