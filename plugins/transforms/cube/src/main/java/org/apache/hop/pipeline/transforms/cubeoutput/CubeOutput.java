...
public class CubeOutput extends BaseTransform<CubeOutputMeta, CubeOutputData> {
  ...
  public boolean processRow() throws HopException {
    ...
    if (first) { // Always run this code once, even if stream is empty (r==null)
      ...
      } else if (data.outputMeta == null) {
        // If the stream is empty, then row metadata hasn't been received. Use the design-time
        // layout unless init() already resolved it before opening the file.
        data.outputMeta = lookupDesignTimeLayout();
  ...
  /**
   * The layout of the stream as the pipeline knows it at design time. Some input transforms have to
   * interrogate their source to answer (Cube Input reads the cube file, Table Input runs the query
   * metadata), so this can fail; in that case the pipeline itself reports the problem.
   *
   * @return the layout, or null when it could not be determined
   */
  private IRowMeta lookupDesignTimeLayout() {
    try {
      return getPipelineMeta().getPrevTransformFields(this, getTransformMeta());
    } catch (HopTransformException e) {
      logDetailed(
          BaseMessages.getString(
              PKG, "CubeOutput.Log.UnableToLookupDesignTimeLayout", e.getMessage()));
      return null;
    }
  }
  ...
  public boolean init() {
    if (super.init()) {
      if (!meta.isDoNotOpenNewFileInit()) {
        // Resolve the layout of the incoming stream before opening the output file. Opening
        // truncates it, and when the pipeline refreshes a cube in place the Cube Input upstream can
        // no longer read that file to tell us its layout at the end of an empty run.
        //
        data.outputMeta = lookupDesignTimeLayout();
        try {
          prepareFile();
