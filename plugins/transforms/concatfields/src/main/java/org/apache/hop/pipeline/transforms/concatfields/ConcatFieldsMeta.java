...
@Transform(
        ...
        i18nPackageName = "org.apache.hop.pipeline.transforms.concatfields",
...
//    createparentfolder = true; // Default createparentfolder to true
  ...
  public void getFields( IRowMeta row, String name, IRowMeta[] info, TransformMeta nextTransform,
    ...
    // add targetFieldName
    IValueMeta vValue =
      new ValueMetaString( targetFieldName, targetFieldLength, 0 );
  ...
  public void loadXml(Node transformNode, IHopMetadataProvider metadataProvider ) throws HopXmlException {
    ...
    targetFieldName = XmlHandler.getTagValue( transformNode, ConcatFieldsNodeNameSpace, "targetFieldName" );
    targetFieldLength =
      Const.toInt( XmlHandler.getTagValue( transformNode, ConcatFieldsNodeNameSpace, "targetFieldLength" ), 0 );
    ...
    for ( int i = 0; i < nrfields; i++ ) {
      ...
      outputFields[i].setTrimType( ValueMetaBase.getTrimTypeByCode( XmlHandler.getTagValue( fnode, "trim_type" ) ) );
...
//    String retval = super.getXml();
  ...
  public void check(List<ICheckResult> remarks, PipelineMeta transMeta, TransformMeta transformMeta,
    ...
    if ( StringUtil.isEmpty( targetFieldName ) ) {
      cr =
        new CheckResult( ICheckResult.TYPE_RESULT_ERROR, BaseMessages.getString(
          PKG, "ConcatFieldsMeta.CheckResult.TargetFieldNameMissing" ), transformMeta );
      remarks.add( cr );
    }
    ...
    if ( targetFieldLength <= 0 ) {
      cr =
        new CheckResult( ICheckResult.TYPE_RESULT_WARNING, BaseMessages.getString(
          PKG, "ConcatFieldsMeta.CheckResult.TargetFieldLengthMissingFastDataDump" ), transformMeta );
      remarks.add( cr );
    }
    ...
    if ( prev != null && prev.size() > 0 ) {
      ...
      if ( errorFound ) {
        ...
        cr =
          new CheckResult( ICheckResult.TYPE_RESULT_OK, BaseMessages.getString(
            PKG, "ConcatFieldsMeta.CheckResult.AllFieldsFound" ), transformMeta );
        remarks.add( cr );
      }
    }
  ...
  @Override
  public ITransform createTransform(TransformMeta transformMeta, ConcatFieldsData data, int copyNr, PipelineMeta pipelineMeta, Pipeline pipeline) {
