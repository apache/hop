...
/**
 * The SQL field name of {@link ExecSqlRowDialog} is filled with the incoming fields when the user
 ...
 */
@Tag("uitest")
class ExecSqlRowDialogKeepsValuesTest extends SwtBotTestBase {

  private static final String TRANSFORM_NAME = "execsqlrow";
  private static final String TITLE =
      BaseMessages.getString(ExecSqlRowMeta.class, "ExecSqlRowDialog.Shell.Label");

  @Test
  void sqlFieldSurvivesFailingUpstream() {
    ExecSqlRowMeta meta = configured("configured_field");
  ...
  private void focusFieldAndPressOk(ExecSqlRowMeta meta, PipelineMeta pipelineMeta) {
    AtomicReference<ExecSqlRowDialog> dialog = new AtomicReference<>();
    withDialog(
        parent -> {
          ExecSqlRowDialog d = new ExecSqlRowDialog(parent, new Variables(), meta, pipelineMeta);
          dialog.set(d);
          d.open();
        },
        bot -> {
  ...
  private static Widget field(ExecSqlRowDialog dialog) {
    try {
  ...
  private static ExecSqlRowMeta configured(String fieldName) {
