...
:language: en_US
:page-alternativeEditUrl: https://github.com/project-hop/hop/edit/master/plugins/transforms/getserversequence/src/main/doc/getserversequence.adoc
= Get slave sequence
...
Once the configuration files are changed as shown above, hop servers receive a new servlet that can be called as follows (authentication required):http://hostname:port/kettle/nextSequence/?name=SequenceName&increment=NumberOfIDsToReserveIn
