 ...
 * @since 03-Juin-2008
 */
public class MailValidator extends BaseTransform<MailValidatorMeta, MailValidatorData>
    implements ITransform<MailValidatorMeta, MailValidatorData> {
  private static final Class<?> PKG = MailValidator.class; // For Translator

  public MailValidator(
      TransformMeta transformMeta,
      MailValidatorMeta meta,
  ...
  public boolean processRow() throws HopException {
    ...
    if (first) {
      ...
      if (Utils.isEmpty(data.realResultFieldName)) {
        throw new HopException(
            BaseMessages.getString(PKG, "MailValidator.Error.ResultFieldNameMissing"));
      }
      ...
      if (Utils.isEmpty(meta.getEmailField())) {
        throw new HopException(
            BaseMessages.getString(PKG, "MailValidator.Error.FilenameFieldMissing"));
      }
    ...
    try {
      ...
      if (getTransformMeta().isDoingErrorHandling()) {
        ...
        logError(
            BaseMessages.getString(PKG, "MailValidator.ErrorInTransformRunning") + e.getMessage());
        setErrors(1);
        stopAll();
      ...
      if (sendToErrorRow) {
        ...
        putError(
            getInputRowMeta(), r, 1, errorMessage, meta.getResultFieldName(), "MailValidator001");
      }
    }
