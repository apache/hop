...
import org.apache.hop.pipeline.steps.textfileoutput.TextFileOutput;
...
public class ConcatFields extends TextFileOutput implements StepInterface {
  ...
  private static Class<?> PKG = ConcatFields.class; // for i18n purposes, needed by Translator!!

  public ConcatFieldsMeta meta;
  public ConcatFieldsData data;

  public ConcatFields( StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, PipelineMeta pipelineMeta,
                       Pipeline pipeline ) {
    super( stepMeta, stepDataInterface, copyNr, pipelineMeta, pipeline ); // allocate TextFileOutput
  }
  ...
  void checkAndWriteHeader() throws HopStepException {
  ...
  void prepareForReMap() throws HopStepException {
    ...
    if ( targetBinary != null ) {
      if ( !data.hasEncoding ) {
        ...
        try {
          ...
          throw new HopStepException( BaseMessages.getString( PKG, "ConcatFields.Error.UnsupportedEncoding", ""
            + meta.getEncoding() ) );
        }
      }
    } else {
  ...
  Object[] putRowFastDataDump( Object[] r ) throws HopStepException {
