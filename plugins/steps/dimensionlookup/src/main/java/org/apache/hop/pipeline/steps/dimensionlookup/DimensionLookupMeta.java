...
package org.apache.hop.pipeline.steps.dimensionlookup;
...
@Step(
        ...
        i18nPackageName = "org.apache.hop.pipeline.steps.dimensionlookup",
        name = "BaseStep.TypeLongDesc.DimensionUpdate",
        description = "BaseStep.TypeTooltipDesc.DimensionUpdate",
        categoryDescription = "i18n:org.apache.hop.pipeline.step:BaseStep.Category.DataWarehouse"
)
...
public class DimensionLookupMeta extends BaseStepMeta implements StepMetaInterface, ProvidesModelerMeta {
  ...
  public void getFields( RowMetaInterface row, String name, RowMetaInterface[] info, StepMeta nextStep,
    ...
    if ( !update && fieldLookup.length > 0 ) {
      ...
      try {
        ...
        logError( message );
        throw new HopStepException( message, e );
      } finally {
        if ( db != null ) {
          db.disconnect();
        }
  ...
  @Override
  public void analyseImpact( List<DatabaseImpact> impact, PipelineMeta pipelineMeta, StepMeta stepMeta, RowMetaInterface prev,
    ...
    if ( prev != null ) {
      if ( !update ) {
        ...
        for ( int i = 0; i < keyLookup.length; i++ ) {
          ...
          DatabaseImpact ii =
            new DatabaseImpact( DatabaseImpact.TYPE_IMPACT_READ, pipelineMeta.getName(), stepMeta.getName(), databaseMeta
  ...
  public StepInterface getStep( StepMeta stepMeta, StepDataInterface stepDataInterface, int cnr, PipelineMeta tr,
                                Pipeline pipeline ) {
    return new DimensionLookup( stepMeta, stepDataInterface, cnr, tr, pipeline );
  }
