...
public class TableOutput extends BaseStep implements StepInterface {
  ...
  public TableOutput( StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, PipelineMeta pipelineMeta,
                      Pipeline pipeline ) {
  ...
  protected Object[] writeToTable( RowMetaInterface rowMeta, Object[] r ) throws HopException {
    ...
    if ( meta.isTableNameInField() ) {
      ...
      if ( data.indexOfPartitioningField < 0 ) {
        ...
        if ( data.indexOfPartitioningField < 0 ) {
          throw new HopStepException( "Unable to find field ["
    ...
    if ( Utils.isEmpty( tableName ) ) {
      throw new HopStepException( "The tablename is not defined (empty)" );
    }
    ...
    try {
      ...
      if ( meta.isReturningGeneratedKeys() ) {
        ...
        if ( extraKeys.getRowMeta().size() > 0 ) {
          ...
          throw new HopStepException( "No generated keys while \"return generated keys\" is active!" );
        }
      }
