...
import org.apache.hop.pipeline.steps.file.BaseFileInputAdditionalField;
import org.apache.hop.pipeline.steps.file.BaseFileInputFiles;
import org.apache.hop.pipeline.steps.file.BaseFileInputMeta;
...
@Step(
        ...
        i18nPackageName = "org.apache.hop.pipeline.steps.fileinput.text",
...
public class TextFileInputMeta extends BaseFileInputMeta<BaseFileInputAdditionalField, BaseFileInputFiles, BaseFileField>
  ...
  public StepInterface getStep( StepMeta stepMeta, StepDataInterface stepDataInterface, int cnr, PipelineMeta pipelineMeta,
                                Pipeline pipeline ) {
    return new TextFileInput( stepMeta, stepDataInterface, cnr, pipelineMeta, pipeline );
  }
   ...
   * For testing
   */
  FileObject getFileObject( String vfsFileName, VariableSpace variableSpace ) throws HopFileException {
    return HopVFS.getFileObject( variableSpace.environmentSubstitute( vfsFileName ), variableSpace );
  }
  ...
  public void resolve() {
    for ( int i = 0; i < inputFiles.fileName.length; i++ ) {
      if ( inputFiles.fileName[ i ] != null && !inputFiles.fileName[ i ].isEmpty() ) {
        try {
          FileObject fileObject = HopVFS.getFileObject( getParentStepMeta().getParentPipelineMeta().environmentSubstitute( inputFiles.fileName[ i ] ) );
