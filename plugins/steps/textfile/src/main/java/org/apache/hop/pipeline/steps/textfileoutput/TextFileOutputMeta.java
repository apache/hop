...
import org.apache.hop.pipeline.steps.file.BaseFileOutputMeta;
...
@Step(
        ...
        i18nPackageName = "org.apache.hop.pipeline.steps.textfileoutput",
        name = "BaseStep.TypeLongDesc.TextFileOutput",
        description = "BaseStep.TypeTooltipDesc.TextFileOutput",
        categoryDescription = "i18n:org.apache.hop.pipeline.step:BaseStep.Category.Output"
)
...
public class TextFileOutputMeta extends BaseFileOutputMeta implements StepMetaInterface, ResolvableResource {
  ...
  public StepInterface getStep( StepMeta stepMeta, StepDataInterface stepDataInterface, int cnr, PipelineMeta pipelineMeta,
                                Pipeline pipeline ) {
    return new TextFileOutput( stepMeta, stepDataInterface, cnr, pipelineMeta, pipeline );
  }
  ...
  public void resolve() {
    if ( fileName != null && !fileName.isEmpty() ) {
      try {
        FileObject fileObject = HopVFS.getFileObject( getParentStepMeta().getParentPipelineMeta().environmentSubstitute( fileName ) );
