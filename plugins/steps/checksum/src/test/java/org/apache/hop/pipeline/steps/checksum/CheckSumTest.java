...
package org.apache.hop.pipeline.steps.checksum;
...
public class CheckSumTest {
  ...
  private Pipeline buildHexadecimalChecksumPipeline( int checkSumType, boolean compatibilityMode, boolean oldChecksumBehaviour ) throws Exception {
    ...
    pipelineMeta.addStep( checkSumStep );
    ...
    pipelineMeta.addStep( dummyStep );

    // Create a hop from CheckSum to Output
    PipelineHopMeta hop = new PipelineHopMeta( checkSumStep, dummyStep );
  ...
  private MockRowListener executeHexTest( int checkSumType, boolean compatibilityMode, Object input, ValueMetaInterface meta, boolean oldChecksumBehaviour ) throws Exception {
    ...
    StepInterface output = pipeline.getRunThread( "Output", 0 );
    ...
    ( (BaseStep) pipeline.getRunThread( "CheckSum", 0 ) ).setInputRowMeta( inputRowMeta );
  ...
  public void testHexOutput_md5() throws Exception {
    ...
    byte[] input = IOUtils.toByteArray( getFile( "/org/apache/hop/pipeline/steps/loadfileinput/files/pentaho_splash.png" ).getContent().getInputStream() );
