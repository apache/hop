...
public class CubeOutput extends BaseStep implements StepInterface {
  ...
  public CubeOutput( StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, PipelineMeta pipelineMeta,
                     Pipeline pipeline ) {
  ...
  public boolean processRow( StepMetaInterface smi, StepDataInterface sdi ) throws HopException {
    ...
    if ( first ) { // Always run this code once, even if stream is empty (r==null)
      if ( getInputRowMeta() != null ) {
        ...
        data.outputMeta = getPipelineMeta().getPrevStepFields( getStepMeta() );
      }
