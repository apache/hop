...
@JobEntry(
  id = "FTPS_GET",
  i18nPackageName = "org.apache.hop.job.entries.ftpsget",
  name = "JobEntryFTPSGet.Name",
  description = "JobEntryFTPSGet.Description",
  image = "FTPSGet.svg",
  categoryDescription = "i18n:org.apache.hop.job:JobCategory.Category.FileTransfer"
)
public class JobEntryFTPSGet extends JobEntryBase implements Cloneable, IJobEntry {
  ...
  public void loadXML( Node entrynode,
    ...
    try {
      ...
      throw new HopXMLException( "Unable to load job entry of type 'FTPS' from XML node", xe );
    }
  }
  ...
  private void downloadFiles( FTPSConnection connection, String folder, Pattern pattern, Result result ) throws HopException {
    ...
    for ( int i = 0; i < fileList.size(); i++ ) {
      ...
      if ( parentJob.isStopped() ) {
        throw new HopException( BaseMessages.getString( PKG, "JobEntryFTPS.JobStopped" ) );
      }

      if ( successConditionBroken ) {
        throw new HopException( BaseMessages.getString( PKG, "JobEntryFTPS.SuccesConditionBroken", NrErrors ) );
      }
