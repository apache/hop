...
public class JobEntryICheckDbConnections extends JobEntryBase implements Cloneable, IJobEntry {
  private static final Class<?> PKG = JobEntryICheckDbConnections.class; // for i18n purposes, needed by Translator!!
  ...
  public JobEntryICheckDbConnections( String name ) {
    super( name, "" );
    connections = null;
    waitfors = null;
    waittimes = null;
  ...
  public JobEntryICheckDbConnections() {
    this( "" );
  }

  public Object clone() {
    JobEntryICheckDbConnections je = (JobEntryICheckDbConnections) super.clone();
    return je;
  }
      ...
      for ( int i = 0; i < connections.length && !parentJob.isStopped(); i++ ) {
        ...
        try {
          ...
          if ( iMaximumTimeout > 0 ) {
            ...
            switch ( waittimes[ i ] ) {
              case JobEntryICheckDbConnections.UNIT_TIME_MILLI_SECOND:
                multiple = 1;
                waitTimeMessage = unitTimeDesc[ 0 ];
                break;
              case JobEntryICheckDbConnections.UNIT_TIME_SECOND:
                multiple = 1000; // Second
                waitTimeMessage = unitTimeDesc[ 1 ];
                break;
              case JobEntryICheckDbConnections.UNIT_TIME_MINUTE:
                multiple = 60000; // Minute
                waitTimeMessage = unitTimeDesc[ 2 ];
                break;
              case JobEntryICheckDbConnections.UNIT_TIME_HOUR:
