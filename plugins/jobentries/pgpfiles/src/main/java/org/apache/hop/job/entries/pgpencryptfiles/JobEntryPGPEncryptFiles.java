...
@JobEntry(
  ...
  i18nPackageName = "org.apache.hop.job.entries.pgpencryptfiles",
  name = "JobEntryPGPEncryptFiles.Name",
  description = "JobEntryPGPEncryptFiles.Description",
  image = "PGPEncryptFiles.svg",
  categoryDescription = "i18n:org.apache.hop.job:JobCategory.Category.FileEncryption"
)
public class JobEntryPGPEncryptFiles extends JobEntryBase implements Cloneable, IJobEntry {
  ...
  private void addFileToResultFilenames( String fileaddentry, Result result, Job parentJob ) {
    try {
      ResultFile resultFile =
        new ResultFile( ResultFile.FILE_TYPE_GENERAL, HopVFS.getFileObject( fileaddentry ), parentJob
          .getJobname(), toString() );
