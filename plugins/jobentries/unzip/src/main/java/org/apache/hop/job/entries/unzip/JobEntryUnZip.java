...
@JobEntry(
  id = "UNZIP",
  i18nPackageName = "org.apache.hop.job.entries.unzip",
  name = "JobEntryUnZip.Name",
  description = "JobEntryUnZip.Description",
  image = "UnZip.svg",
...
public class JobEntryUnZip extends JobEntryBase implements Cloneable, JobEntryInterface {
  ...
  public void loadXML( Node entrynode,
    ...
    try {
      ...
      throw new HopXMLException( "Unable to load job entry of type 'unzip' from XML node", xe );
    }
  }
  ...
  private void addFilenameToResultFilenames( Result result, Job parentJob, String newfile ) throws Exception {
    if ( addfiletoresult ) {
      ...
      ResultFile resultFile =
        new ResultFile( ResultFile.FILE_TYPE_GENERAL, HopVFS.getFileObject( newfile, this ), parentJob
          .getJobname(), toString() );
