...
package org.apache.hop.pipeline.steps.textfileoutput;
...
public class TextFileOutputTest {
  ...
  public void setUp() throws Exception {
    ...
    Mockito.when( stepMockHelper.pipeline.isRunning() ).thenReturn( true );
    Mockito.verify( stepMockHelper.pipeline, Mockito.never() ).stopAll();
    ...
    Mockito.when( stepMockHelper.pipelineMeta.listVariables() ).thenReturn( new String[ 0 ] );
  }
  ...
  public void testCloseFileDataOutIsNullCase() {
    textFileOutput =
      new TextFileOutput( stepMockHelper.stepMeta, stepMockHelper.stepDataInterface, 0, stepMockHelper.pipelineMeta,
  ...
  private FileObject createTemplateFile() {
    String path =
      TestUtils.createRamFile( getClass().getSimpleName() + "/" + TEXT_FILE_OUTPUT_PREFIX + new Random().nextLong()
        + TEXT_FILE_OUTPUT_EXTENSION, stepMockHelper.pipelineMeta );
    return TestUtils.getFileObject( path, stepMockHelper.pipelineMeta );
  }
   ...
   * pipeline does not pass any rows to the file input step, then NO output file should be created.
   */
  @Test
  ...
  private List<Throwable> doOutput( TextFileField[] textFileField, List<Object[]> rows, String pathToFile,
    ...
    TextFileOutputTestHandler textFileOutput =
      new TextFileOutputTestHandler( stepMockHelper.stepMeta, textFileOutputData, 0, stepMockHelper.pipelineMeta,
        stepMockHelper.pipeline );
  ...
  public void testEndedLineVar() throws Exception {
    ...
    TextFileOutput textFileOutput =
      new TextFileOutputTestHandler( stepMockHelper.stepMeta, data, 0, stepMockHelper.pipelineMeta,
        stepMockHelper.pipeline );
  ...
  private void assertNotInvokedTwice( TextFileField field ) {
    TextFileOutput step =
      new TextFileOutput( stepMockHelper.stepMeta, stepMockHelper.stepDataInterface, 1, stepMockHelper.pipelineMeta,
        stepMockHelper.pipeline );
  ...
  public void testProcessRule_2() throws Exception {
    ...
    textFileOutput =
      new TextFileOutputTestHandler( stepMockHelper.stepMeta, stepMockHelper.stepDataInterface, 0, stepMockHelper.pipelineMeta,
        stepMockHelper.pipeline );
