...
public class MemoryGroupByAggregationTest {
  ...
  private RowMetaAndData runStep() throws HopException {
    // Allocate meta
    ...
    when( pipelineMeta.findStep( STEP_NAME ) ).thenReturn( stepMeta );

    // Spy on step, regrettable but we need to easily inject rows
    MemoryGroupBy step = spy( new MemoryGroupBy( stepMeta, data, 0, pipelineMeta, mock( Pipeline.class ) ) );
