...
package org.apache.hop.pipeline.steps.databaselookup;
...
import org.apache.hop.pipeline.steps.databaselookup.readallcache.ReadAllCache;
...
public class DatabaseLookupUTest {
  ...
  private StepMockHelper<DatabaseLookupMeta, DatabaseLookupData> createMockHelper() {
    ...
    when( mockHelper.pipelineMeta.findNextSteps( Matchers.any( StepMeta.class ) ) )
      .thenReturn( Collections.singletonList( mock( StepMeta.class ) ) );
    when( mockHelper.pipelineMeta.findPreviousSteps( any( StepMeta.class ), anyBoolean() ) )
      .thenReturn( Collections.singletonList( mock( StepMeta.class ) ) );
  ...
  private DatabaseLookup spyLookup( StepMockHelper<DatabaseLookupMeta, DatabaseLookupData> mocks, Database db,
    ...
    DatabaseLookup lookup =
      new DatabaseLookup( mocks.stepMeta, mocks.stepDataInterface, 1, mocks.pipelineMeta, mocks.pipeline );
  ...
  public void testEqualsAndIsNullAreCached() throws Exception {
    ...
    DatabaseLookup look =
      new MockDatabaseLookup( mockHelper.stepMeta, mockHelper.stepDataInterface, 0, mockHelper.pipelineMeta,
        mockHelper.pipeline );
  ...
  public void getRowInCacheTest() throws HopException {
    ...
    DatabaseLookup look =
      new DatabaseLookup( mockHelper.stepMeta, mockHelper.stepDataInterface, 0, mockHelper.pipelineMeta,
        mockHelper.pipeline );
    DatabaseLookupData lookData = new DatabaseLookupData();
    lookData.cache = DefaultCache.newCache( lookData, 0 );
    lookData.lookupMeta = new RowMeta();
  ...
  public class MockDatabaseLookup extends DatabaseLookup {
    public MockDatabaseLookup( StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, PipelineMeta pipelineMeta, Pipeline pipeline ) {
