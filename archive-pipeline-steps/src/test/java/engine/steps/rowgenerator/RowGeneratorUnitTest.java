...
package org.apache.hop.pipeline.steps.rowgenerator;
...
public class RowGeneratorUnitTest {
  ...
  public void setUp() throws HopException {
    ...
    when( pipelineMeta.findStep( anyString() ) ).thenReturn( stepMeta );
    ...
    rowGenerator = spy( new RowGenerator( stepMeta, stepDataInterface, 0, pipelineMeta, pipeline ) );
  ...
  public void doesNotWriteRowOnTimeWhenStopped() throws HopException, InterruptedException {
    ...
    pipeline.getSteps().get( 1 ).step.addRowListener( new RowAdapter() {
    ...
    assertEquals( 1, pipeline.getSteps().get( 0 ).step.getLinesWritten() );
    assertEquals( 1, pipeline.getSteps().get( 1 ).step.getLinesRead() );
  }
}
