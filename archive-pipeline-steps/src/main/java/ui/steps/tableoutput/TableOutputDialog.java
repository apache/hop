...
package org.apache.hop.ui.pipeline.steps.tableoutput;
...
import org.apache.hop.pipeline.steps.tableoutput.TableOutputMeta;
  ...
  private void sql() {
    try {
      ...
      if ( isValidRowMeta( prev ) ) {
        SQLStatement sql = info.getSQLStatements( pipelineMeta, stepMeta, prev, pk, autoInc, pk );
        if ( !sql.hasError() ) {
          if ( sql.hasSQL() ) {
            SQLEditor sqledit =
