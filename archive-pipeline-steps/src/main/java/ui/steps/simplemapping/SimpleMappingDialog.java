...
package org.apache.hop.ui.pipeline.steps.simplemapping;
...
import org.apache.hop.pipeline.steps.simplemapping.SimpleMappingMeta;
...
public class SimpleMappingDialog extends BaseStepDialog implements StepDialogInterface {
  ...
  public RowMetaInterface getFieldsFromStep( boolean parent, boolean input ) throws HopException {
    if ( input ) {
      // INPUT
      //
      if ( parent ) {
        return pipelineMeta.getPrevStepFields( stepMeta );
      } else {
        if ( mappingPipelineMeta == null ) {
          throw new HopException( BaseMessages.getString(
            PKG, "SimpleMappingDialog.Exception.NoMappingSpecified" ) );
        }
        StepMeta mappingInputStepMeta = mappingPipelineMeta.findMappingInputStep( null );
        return mappingPipelineMeta.getStepFields( mappingInputStepMeta );
      }
    } else {
      // OUTPUT
      //
      StepMeta mappingOutputStepMeta = mappingPipelineMeta.findMappingOutputStep( null );
      return mappingPipelineMeta.getStepFields( mappingOutputStepMeta );
    }
  }
