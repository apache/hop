...
package org.apache.hop.ui.pipeline.steps.update;
...
import org.apache.hop.pipeline.steps.update.UpdateMeta;
...
public class UpdateDialog extends BaseStepDialog implements StepDialogInterface {
  ...
  public void setFlags() {
    ...
    boolean hasErrorHandling = pipelineMeta.findStep( stepname ).isDoingErrorHandling();
  ...
  private void create() {
    try {
      ...
      SQLStatement sql = info.getSQLStatements( pipelineMeta, stepinfo, prev, metaStore );
      if ( !sql.hasError() ) {
        if ( sql.hasSQL() ) {
          SQLEditor sqledit =
