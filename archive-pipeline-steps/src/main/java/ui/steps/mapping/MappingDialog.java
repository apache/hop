...
package org.apache.hop.ui.pipeline.steps.mapping;
...
import org.apache.hop.pipeline.steps.mapping.MappingParameters;
...
public class MappingDialog extends BaseStepDialog implements StepDialogInterface {
  ...
  protected String selectTransformationStepname( boolean getTransformationStep, boolean mappingInput ) {
    ...
    if ( getTransformationStep ) {
      dialogTitle = BaseMessages.getString( PKG, "MappingDialog.SelectPipelineStep.Title" );
      dialogMessage = BaseMessages.getString( PKG, "MappingDialog.SelectPipelineStep.Message" );
      String[] stepnames;
      if ( mappingInput ) {
        stepnames = pipelineMeta.getPrevStepNames( stepMeta );
      } else {
        stepnames = pipelineMeta.getNextStepNames( stepMeta );
      }
      ...
      String[] stepnames = getMappingSteps( mappingPipelineMeta, mappingInput );
      EnterSelectionDialog dialog = new EnterSelectionDialog( shell, stepnames, dialogTitle, dialogMessage );
      return dialog.open();
    }
  }

  public static String[] getMappingSteps( PipelineMeta mappingPipelineMeta, boolean mappingInput ) {
  ...
  public RowMetaInterface getFieldsFromStep( String stepname, boolean getTransformationStep, boolean mappingInput ) throws HopException {
    if ( !( mappingInput ^ getTransformationStep ) ) {
      if ( Utils.isEmpty( stepname ) ) {
        // If we don't have a specified stepname we return the input row
        // metadata
        //
        return pipelineMeta.getPrevStepFields( this.stepname );
      } else {
        ...
        return pipelineMeta.getStepFields( stepMeta );
      }
      ...
      if ( Utils.isEmpty( stepname ) ) {
        ...
        return mappingPipelineMeta.getStepFields( stepnames[ 0 ] );
      } else {
        // OK, a fieldname is specified...
        // See if we can find it...
        StepMeta stepMeta = mappingPipelineMeta.findStep( stepname );
        if ( stepMeta == null ) {
          throw new HopException( BaseMessages.getString(
            PKG, "MappingDialog.Exception.SpecifiedStepWasNotFound", stepname ) );
        }
        return mappingPipelineMeta.getStepFields( stepMeta );
      }
    }
  }
   ...
   * @throws HopException
   */
  private void enableMappingButton( final Button button, boolean input, String sourceStepname, String targetStepname ) throws HopException {
    if ( input ) {
    ...
    if ( mappingPipelineMeta != null ) {
      StepMeta mappingInputStep = mappingPipelineMeta.findMappingInputStep( sourceStepname );
      if ( mappingInputStep != null ) {
        StepMeta mappingOutputStep = pipelineMeta.findMappingOutputStep( targetStepname );
        RowMetaInterface requiredFields = mappingOutputStep.getStepMetaInterface().getRequiredFields( pipelineMeta );
  ...
  private void ok() {
    ...
    mappingMeta.searchInfoAndTargetSteps( pipelineMeta.getSteps() );
