...
package org.apache.hop.ui.pipeline.steps.script;
...
import org.apache.hop.pipeline.steps.rowgenerator.RowGeneratorMeta;
import org.apache.hop.pipeline.steps.script.ScriptAddedFunctions;
import org.apache.hop.pipeline.steps.script.ScriptDummy;
import org.apache.hop.pipeline.steps.script.ScriptMeta;
import org.apache.hop.pipeline.steps.script.ScriptValuesScript;
...
public class ScriptDialog extends BaseStepDialog implements StepDialogInterface {
  ...
  public String open() {
    ...
    final Runnable runnable = new Runnable() {
      public void run() {
        ...
        if ( stepMeta != null ) {
          try {
            rowPrevStepFields = pipelineMeta.getPrevStepFields( stepMeta );
  ...
  public void getData() {
    ...
    org.apache.hop.pipeline.steps.script.ScriptValuesScript[] jsScripts = input.getJSScripts();
  ...
  private boolean newTest() {
    ...
    try {
      ...
      //
      RowMetaInterface rowMeta = pipelineMeta.getPrevStepFields( stepname ).clone();
      if ( rowMeta != null ) {
        ...
        //
        PipelineHopMeta hop = new PipelineHopMeta( genStep, scriptStep );

        // Generate a new test pipeline...
        //
        PipelineMeta pipelineMeta = new PipelineMeta();
        pipelineMeta.setName( wStepname.getText() + " - PREVIEW" );
        pipelineMeta.addStep( genStep );
        pipelineMeta.addStep( scriptStep );
        pipelineMeta.addPipelineHop( hop );

        // OK, now we ask the user to edit this dialog...
        //
        if ( HopUi.getInstance().editStep( pipelineMeta, genStep ) != null ) {
          // Now run this pipeline and grab the results...
          //
          PipelinePreviewProgressDialog progressDialog =
            new PipelinePreviewProgressDialog(
              shell, pipelineMeta, new String[] { scriptStepName, }, new int[] { Const.toInt( genMeta
              .getRowLimit(), 10 ), } );
          progressDialog.open();
          ...
          if ( previewRowsMeta != null && previewRows != null && previewRows.size() > 0 ) {
            PreviewRowsDialog prd =
              new PreviewRowsDialog(
                shell, pipelineMeta, SWT.NONE, wStepname.getText(), previewRowsMeta, previewRows, loggingText );
            prd.open();
          }
        }
        ...
        throw new HopException( BaseMessages.getString( PKG, "ScriptDialog.Exception.CouldNotGetFields" ) );
      }
    } catch ( Exception e ) {
  ...
  private boolean test( boolean getvars, boolean popup ) {
    ...
    jsscope.put( "_PipelineName_", jsscope, this.stepname );

    try {
      RowMetaInterface rowMeta = pipelineMeta.getPrevStepFields( stepname );
      if ( rowMeta != null ) {

        ScriptDummy dummyStep = new ScriptDummy( rowMeta, pipelineMeta.getStepFields( stepname ) );
        ...
        try {
          ...
          testException =
            new HopException( BaseMessages.getString( PKG, "ScriptDialog.CouldNotAddToContext", e.toString() ) );
          retval = false;
        }
        ...
        try {
          ...
          testException =
            new HopException( BaseMessages.getString( PKG, "ScriptDialog.CouldNotAddDefaultFunctions", ex
              .toString() ) );
          retval = false;
        }
        ...
        try {
          ...
          testException =
            new HopException( BaseMessages.getString(
              PKG, "ScriptDialog.CouldNotAddPipelineConstants", ex.toString() ) );
          retval = false;
        }
        ...
        try {
          ...
          testException =
            new HopException( BaseMessages.getString( PKG, "ScriptDialog.CouldNotAddInputFields", ev
              .toString() ) );
          retval = false;
        }
        ...
        try {
          ...
          testException =
            new HopException( BaseMessages.getString( PKG, "ScriptDialog.CouldProcessStartScript", e
              .toString() ) );
          retval = false;
        }
        ...
        testException =
          new HopException( BaseMessages.getString( PKG, "ScriptDialog.Exception.CouldNotGetFields" ) );
        retval = false;
      }
   ...
   *
   * RowMetaInterface r = pipelineMeta.getPrevStepFields(stepname); if (r!=null){ TreeItem item = null; for (TreeItem look
