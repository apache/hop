...
public class Calculator extends BaseStep implements StepInterface {
  ...
  public Calculator( StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, PipelineMeta pipelineMeta,
                     Pipeline pipeline ) {
  ...
  public boolean processRow( StepMetaInterface smi, StepDataInterface sdi ) throws HopException {
    ...
    if ( first ) {
      ...
      for ( int i = 0; i < meta.getCalculation().length; i++ ) {
        ...
        if ( !Utils.isEmpty( function.getFieldName() ) ) {
          ...
          throw new HopStepException( BaseMessages.getString( PKG, "Calculator.Error.NoNameField", ""
            + ( i + 1 ) ) );
        }
        ...
        if ( !Utils.isEmpty( function.getFieldA() ) ) {
          if ( function.getCalcType() != CalculatorMetaFunction.CALC_CONSTANT ) {
            ...
            if ( data.getFieldIndexes()[ i ].indexA < 0 ) {
              // Nope: throw an exception
              throw new HopStepException( "Unable to find the first argument field '"
          ...
          throw new HopStepException( "There is no first argument specified for calculated field #" + ( i + 1 ) );
        }
        ...
        if ( !Utils.isEmpty( function.getFieldB() ) ) {
          ...
          if ( data.getFieldIndexes()[ i ].indexB < 0 ) {
            // Nope: throw an exception
            throw new HopStepException( "Unable to find the second argument field '"
              + function.getFieldName() + " for calculation #" + ( i + 1 ) );
          }
        }
        ...
        if ( !Utils.isEmpty( function.getFieldC() ) ) {
          ...
          if ( data.getFieldIndexes()[ i ].indexC < 0 ) {
            // Nope: throw an exception
            throw new HopStepException( "Unable to find the third argument field '"
              + function.getFieldName() + " for calculation #" + ( i + 1 ) );
          }
        }
    ...
    try {
      ...
      throw new HopStepException( BaseMessages.getString( PKG, "Calculator.ErrorInStepRunning" ), e );
    }
    return true;
  }
