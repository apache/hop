...
package org.apache.hop.pipeline.steps.zipfile;
...
public class ZipFile extends BaseStep implements StepInterface {
  ...
  public ZipFile( StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, PipelineMeta pipelineMeta,
                  Pipeline pipeline ) {
  ...
  public boolean processRow( StepMetaInterface smi, StepDataInterface sdi ) throws HopException {
    ...
    if ( first ) {
      ...
      meta.getFields( data.outputRowMeta, getStepname(), null, null, this, getPipeline().getMetaStore() );
    ...
    try {
      ...
      data.sourceFile = HopVFS.getFileObject( sourceFilename, this );
      ...
      if ( !skip ) {
        ...
        data.zipFile = HopVFS.getFileObject( targetFilename, this );
  ...
  private void processFile( String folder ) throws Exception {
    ...
    switch ( meta.getOperationType() ) {
      case ZipFileMeta.OPERATION_TYPE_MOVE:
        ...
        try {
          // Move to folder
          moveToFolder = HopVFS.getFileObject( folder, this );
          ...
          file = HopVFS.getFileObject( targetfilename, this );
  ...
  private void addFilenameToResult() throws FileSystemException {
    if ( meta.isaddTargetFileNametoResult() ) {
      ...
      ResultFile resultFile =
        new ResultFile( ResultFile.FILE_TYPE_GENERAL, data.zipFile, getPipelineMeta().getName(), getStepname() );
