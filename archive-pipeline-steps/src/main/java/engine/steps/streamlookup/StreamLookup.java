...
public class StreamLookup extends BaseStep implements StepInterface {
  ...
  public StreamLookup( StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, PipelineMeta pipelineMeta,
                       Pipeline pipeline ) {
    ...
    if ( data.lookupColumnIndex == null ) {
      ...
      for ( int i = 0; i < names.length; i++ ) {
        ...
        if ( data.lookupColumnIndex[ i ] < 0 ) {
          ...
          throw new HopStepException( "The lookup column '" + names[ i ] + "' could not be found" );
        }
      }
    }
      ...
      for ( int i = 0; i < data.lookupMeta.size(); i++ ) {
        ...
        if ( inputValue.getType() != lookupValue.getType() ) {
          try {
            ...
            throw new HopStepException( "Error converting data while looking up value", e );
          }
        }
      }
    }
    ...
    if ( data.hasLookupRows ) {
      try {
        if ( meta.getKeystream().length > 0 ) {
          ...
          throw new HopStepException( BaseMessages.getString( PKG, "StreamLookup.Log.GotRowWithoutKeys" ) );
        }
      } catch ( Exception e ) {
  ...
  public boolean processRow( StepMetaInterface smi, StepDataInterface sdi ) throws HopException {
    ...
    if ( first ) {
      ...
      meta.getFields(
        data.outputRowMeta, getStepname(), new RowMetaInterface[] { data.infoMeta }, null, this, metaStore );
