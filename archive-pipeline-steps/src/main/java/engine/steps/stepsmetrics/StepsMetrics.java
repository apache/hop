...
package org.apache.hop.pipeline.steps.stepsmetrics;
...
public class StepsMetrics extends BaseStep implements StepInterface {
  ...
  public StepsMetrics( StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, PipelineMeta pipelineMeta,
                       Pipeline pipeline ) {
  ...
  public boolean processRow( StepMetaInterface smi, StepDataInterface sdi ) throws HopException {
    ...
    if ( first ) {
      ...
      if ( meta.getStepName() != null && meta.getStepName().length > 0 ) {
        ...
        // error
        throw new HopException( BaseMessages.getString( PKG, "StepsMetrics.Error.NotSteps" ) );
      }
      ...
      for ( int i = 0; i < stepnrs; i++ ) {
        ...
        StepInterface si = getPipeline().getStepInterface( stepnames[ i ], CopyNr );
        if ( si != null ) {
