...
public class MergeJoin extends BaseStep implements StepInterface {
  ...
  public MergeJoin( StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, PipelineMeta pipelineMeta,
                    Pipeline pipeline ) {
  ...
  public boolean processRow( StepMetaInterface smi, StepDataInterface sdi ) throws HopException {
    ...
    if ( first ) {
      ...
      if ( data.one != null ) {
        ...
        data.one = null;
        data.oneMeta = getPipelineMeta().getStepFields( infoStreams.get( 0 ).getStepname() );
      }
      ...
      if ( data.two != null ) {
        ...
        data.two = null;
        data.twoMeta = getPipelineMeta().getStepFields( infoStreams.get( 1 ).getStepname() );
      }
