...
public class HTTP extends BaseStep implements StepInterface {
  ...
  public HTTP( StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, PipelineMeta pipelineMeta, Pipeline pipeline ) {
    ...
    if ( first ) {
      ...
      for ( int i = 0; i < meta.getArgumentField().length; i++ ) {
        ...
        if ( data.argnrs[ i ] < 0 ) {
          ...
          throw new HopStepException( BaseMessages.getString( PKG, "HTTP.Exception.CouldnotFindField", meta
            .getArgumentField()[ i ] ) );
        }
      }
    }
    ...
    if ( first ) {
      ...
      for ( int i = 0; i < nrHeaders; i++ ) {
        ...
        if ( fieldIndex < 0 ) {
          ...
          throw new HopStepException( BaseMessages.getString( PKG, "HTTP.Exception.ErrorFindingField", meta
            .getHeaderField()[ i ] ) );
        }
