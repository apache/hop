...
package org.apache.hop.pipeline.steps.propertyinput;
...
public class PropertyInput extends BaseStep implements StepInterface {
  ...
  public PropertyInput( StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, PipelineMeta pipelineMeta,
                        Pipeline pipeline ) {
  ...
  private Object[] getOneRow() throws HopException {
    ...
    try {
      ...
      throw new HopException( BaseMessages.getString( PKG, "PropertyInput.Error.CanNotReadFromFile", data.file
        .toString() ), e );
    }
    ...
    try {
      ...
      if ( meta.isAddResultFile() ) {
        ...
        ResultFile resultFile =
          new ResultFile( ResultFile.FILE_TYPE_GENERAL, data.file, getPipelineMeta().getName(), getStepname() );
      ...
      if ( log.isDetailed() ) {
        ...
        logDetailed( BaseMessages.getString( PKG, "PropertyInput.log.TotalKey", ""
          + ( data.propfiles ? data.pro.size() : data.iniSection.size() ), HopVFS.getFilename( data.file ) ) );
      }
    } catch ( Exception e ) {
