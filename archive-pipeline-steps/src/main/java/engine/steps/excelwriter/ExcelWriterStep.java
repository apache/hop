...
package org.apache.hop.pipeline.steps.excelwriter;
...
public class ExcelWriterStep extends BaseStep implements StepInterface {
  ...
  public ExcelWriterStep( StepMeta s, StepDataInterface stepDataInterface, int c, PipelineMeta t, Pipeline dis ) {
  ...
  public void prepareNextOutputFile() throws HopException {
    try {
      ...
      if ( data.realSheetname != null && data.realSheetname.length() > 31 ) {
        throw new HopException( BaseMessages.getString( PKG, "ExcelWriterStep.Exception.MaxSheetName", data.realSheetname ) );
      }
      // clear style cache
      ...
      if ( meta.isAddToResultFiles() ) {
        ...
        ResultFile resultFile = new ResultFile( ResultFile.FILE_TYPE_GENERAL, data.file, getPipelineMeta().getName(), getStepname() );
      ...
      if ( data.wb.getSheet( data.realSheetname ) == null ) {
        if ( meta.isTemplateSheetEnabled() ) {
          ...
          if ( ts == null ) {
            throw new HopException( BaseMessages.getString( PKG, "ExcelWriterStep.Exception.TemplateNotFound", data.realTemplateSheetName ) );
          }
   ...
   *
   * @see org.apache.hop.pipeline.step.BaseStep#init(org.apache.hop.pipeline.step.StepMetaInterface,
   ...
   * pipeline run end
   *
   * @see org.apache.hop.pipeline.step.BaseStep#dispose(org.apache.hop.pipeline.step.StepMetaInterface,
   * org.apache.hop.pipeline.step.StepDataInterface)
   */
  @Override
