...
public class Delete extends BaseStep implements StepInterface {
  ...
  public Delete( StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, PipelineMeta pipelineMeta,
                 Pipeline pipeline ) {
    ...
    if ( first ) {
      ...
      for ( int i = 0; i < meta.getKeyStream().length; i++ ) {
        ...
        if ( data.keynrs[ i ] < 0 && // couldn't find field!
          ...
          throw new HopStepException( BaseMessages.getString( PKG, "Delete.Exception.FieldRequired", meta
            .getKeyStream()[ i ] ) );
        }
