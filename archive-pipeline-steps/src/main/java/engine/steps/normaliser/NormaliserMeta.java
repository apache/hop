...
public class NormaliserMeta extends BaseStepMeta implements StepMetaInterface {
  ...
  public void getFields( RowMetaInterface row, String name, RowMetaInterface[] info, StepMeta nextStep,
    ...
    for ( int i = 0; i < norm_occ.size(); i++ ) {
      ...
      if ( v != null ) {
        ...
        throw new HopStepException( BaseMessages.getString( PKG, "NormaliserMeta.Exception.UnableToFindField", fieldname ) );
      }
      v.setName( normname );
      v.setOrigin( name );
      row.addValueMeta( v );
  ...
  public StepInterface getStep( StepMeta stepMeta, StepDataInterface stepDataInterface, int cnr, PipelineMeta pipelineMeta,
                                Pipeline pipeline ) {
    return new Normaliser( stepMeta, stepDataInterface, cnr, pipelineMeta, pipeline );
  }
