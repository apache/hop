...
package org.apache.hop.pipeline.steps.systemdata;
...
public class SystemData extends BaseStep implements StepInterface {
  ...
  public SystemData( StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, PipelineMeta pipelineMeta,
                     Pipeline pipeline ) {
    ...
    for ( int i = 0, index = inputRowMeta.size(); i < meta.getFieldName().length; i++, index++ ) {
      ...
      switch ( meta.getFieldType()[ i ] ) {
        ...
        case TYPE_SYSTEM_INFO_PIPELINE_DATE_FROM:
          row[ index ] = getPipeline().getStartDate();
          break;
        case TYPE_SYSTEM_INFO_PIPELINE_DATE_TO:
          row[ index ] = getPipeline().getEndDate();
          break;
        case TYPE_SYSTEM_INFO_JOB_DATE_FROM:
          row[ index ] = getPipeline().getJobStartDate();
          break;
        case TYPE_SYSTEM_INFO_JOB_DATE_TO:
          row[ index ] = getPipeline().getJobEndDate();
          break;
  ...
  public boolean init( StepMetaInterface smi, StepDataInterface sdi ) {
    ...
    if ( super.init( smi, sdi ) ) {
      ...
      List<StepMeta> previous = getPipelineMeta().findPreviousSteps( getStepMeta() );
