...
package org.apache.hop.pipeline.steps.validator;
...
public class Validator extends BaseStep implements StepInterface {
  ...
  public Validator( StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, PipelineMeta pipelineMeta,
                    Pipeline pipeline ) {
      ...
      for ( int i = 0; i < meta.getValidations().size(); i++ ) {
        ...
        if ( !Utils.isEmpty( field.getFieldName() ) ) {
          ...
          if ( data.fieldIndexes[ i ] < 0 ) {
            ...
            throw new HopStepException( "Unable to find the specified fieldname '"
              + field.getFieldName() + "' for validation#" + ( i + 1 ) );
          }
        } else {
          throw new HopStepException( "There is no name specified for validator field #" + ( i + 1 ) );
        }
      }
    } else {
      // Read the row AFTER the info rows
  ...
  void readSourceValuesFromInfoSteps() throws HopStepException {
      ...
      if ( field.isSourcingValues() ) {
        ...
        while ( allowedRowData != null ) {
          ...
          if ( fieldIndex < 0 ) {
            ...
            if ( fieldIndex < 0 ) {
              throw new HopStepException( "Source field ["
  ...
  public boolean init( StepMetaInterface smi, StepDataInterface sdi ) {
    ...
    if ( super.init( smi, sdi ) ) {
      ...
      List<StepMetaDataCombi> pipelineSteps = getPipeline().getSteps();
      if ( pipelineSteps != null ) {
        for ( StepMetaDataCombi s : pipelineSteps ) {
