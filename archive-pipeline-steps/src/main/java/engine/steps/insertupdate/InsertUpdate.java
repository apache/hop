...
public class InsertUpdate extends BaseStep implements StepInterface {
  ...
  public InsertUpdate( StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, PipelineMeta pipelineMeta,
                       Pipeline pipeline ) {
    ...
    if ( first ) {
      ...
      for ( int i = 0; i < meta.getKeyStream().length; i++ ) {
        ...
        if ( keynr < 0 && // couldn't find field!
          ...
          throw new HopStepException( BaseMessages.getString( PKG, "InsertUpdate.Exception.FieldRequired", meta
            .getKeyStream()[ i ] ) );
        }
        keynrs.add( keynr );
      ...
      for ( int i = 0; i < meta.getUpdateLookup().length; i++ ) {
        ...
        if ( insValue == null ) {
          ...
          throw new HopStepException( "The same column can't be inserted into the target row twice: "
