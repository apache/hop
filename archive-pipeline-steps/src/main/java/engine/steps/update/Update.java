...
public class Update extends BaseStep implements StepInterface {
  ...
  public Update( StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, PipelineMeta pipelineMeta,
                 Pipeline pipeline ) {
    ...
    if ( first ) {
      ...
      for ( int i = 0; i < meta.getKeyStream().length; i++ ) {
        ...
        if ( keynr < 0 && // couldn't find field!
          ...
          throw new HopStepException( BaseMessages.getString( PKG, "Update.Exception.FieldRequired", meta
            .getKeyStream()[ i ] ) );
        }
        keynrs.add( keynr );
