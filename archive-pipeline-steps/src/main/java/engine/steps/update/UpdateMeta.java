...
package org.apache.hop.pipeline.steps.update;
...
import org.apache.hop.pipeline.step.utils.RowMetaUtils;
...
public class UpdateMeta extends BaseStepMeta implements StepMetaInterface {
  ...
  public void analyseImpact( List<DatabaseImpact> impact, PipelineMeta pipelineMeta, StepMeta stepMeta,
    ...
    if ( prev != null ) {
      ...
      for ( int i = 0; i < keyLookup.length; i++ ) {
        ...
        DatabaseImpact ii =
          new DatabaseImpact(
            DatabaseImpact.TYPE_IMPACT_READ, pipelineMeta.getName(), stepMeta.getName(), databaseMeta
      ...
      for ( int i = 0; i < updateLookup.length; i++ ) {
        ...
        DatabaseImpact ii =
          new DatabaseImpact(
            DatabaseImpact.TYPE_IMPACT_UPDATE, pipelineMeta.getName(), stepMeta.getName(), databaseMeta
  ...
  public StepInterface getStep( StepMeta stepMeta, StepDataInterface stepDataInterface, int cnr, PipelineMeta tr,
                                Pipeline pipeline ) {
    return new Update( stepMeta, stepDataInterface, cnr, tr, pipeline );
  }
