...
public class MultiMergeJoin extends BaseStep implements StepInterface {
  ...
  public MultiMergeJoin( StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, PipelineMeta pipelineMeta,
                         Pipeline pipeline ) {
    super( stepMeta, stepDataInterface, copyNr, pipelineMeta, pipeline );
  }

  private boolean processFirstRow( StepMetaInterface smi, StepDataInterface sdi ) throws HopException {
    ...
    for ( int i = 0; i < infoStreams.size(); i++ ) {
      ...
      //check the hop
      pipelineHopMeta = pipelineMeta.findPipelineHop( fromStepMeta, toStepMeta, true );
    ...
    for ( int i = 0, j = 0; i < inputStepNames.length; i++ ) {
      ...
      data.rows[ j ] = row;
      if ( row == null ) {
        rowMeta = getPipelineMeta().getStepFields( inputStepName );
  ...
  /**
   * @see StepInterface#init(org.apache.hop.pipeline.step.StepMetaInterface, org.apache.hop.pipeline.step.StepDataInterface)
   */
